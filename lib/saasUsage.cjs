...
async function countMonthlyReferralInvitesForClinic(supabase, clinicId, clinicCode, _monthStartIso, _monthEndIso) {
  ...
  const build = () => {
    let q = supabase
      ...
      .lt('created_at', end);
    if (codeUp) {
      return q.or(`clinic_id.eq.${cid},clinic_code.eq.${codeUp}`);
    }
    return q.eq('clinic_id', cid);
  };
  const out = await safeCount(build, 'referrals_monthly');
  console.log('COUNT TIMING:', 'referrals_monthly', Date.now() - t0, {
    clinicId: cid,
    clinicCodeOr: codeUp || null,
    value: out.value,
  });
  return out;
}
...
async function collectUsageDiagnostics(supabase, clinicUuid, clinicCode, period) {
  ...
  try {
    ...
    out.referrals.month_by_clinic_id = byId.value;
    if (codeUp) {
      const byCode = await safeCount(
        () =>
          supabase
            .from('referrals')
            .select('id', { count: 'exact', head: true })
      ...
      out.referrals.month_by_clinic_code = byCode.value;
    }
    const orQ = await safeCount(
      () =>
        codeUp
          ? supabase
              .from('referrals')
              ...
              .gte('created_at', start)
              .lt('created_at', end)
              .or(`clinic_id.eq.${cid},clinic_code.eq.${codeUp}`)
          : supabase
              .from('referrals')
              .select('id', { count: 'exact', head: true })
              .eq('clinic_id', cid)
    ...
    out.referrals.month_union_or = orQ.value;
    const allTime = await safeCount(
      () =>
        codeUp
          ? supabase
              .from('referrals')
              .select('id', { count: 'exact', head: true })
              .or(`clinic_id.eq.${cid},clinic_code.eq.${codeUp}`)
          : supabase.from('referrals').select('id', { count: 'exact', head: true }).eq('clinic_id', cid),
      'diag_ref_all'
    );
    out.referrals.rowsFoundAllTimeUnion = allTime.value;
  } catch (e) {
