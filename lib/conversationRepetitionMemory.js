...
const SOCIAL_ACK_RE =
  /^(tesekkur|tesekkurler|sagol|sag\s*ol|thanks|thank\s*you|thank\s*u|iyi\s+aksamlar|iyi\s+geceler|gunaydin|good\s+(evening|morning|night)|hello|hi|hey|merhaba|selam|selamlar|gamarjoba)[\s!.?,🙏]*$/iu;
...
function isConversationalShortReply(text) {
  return isSocialAcknowledgmentMessage(text);
}
...
function buildRepetitionSuppressionPromptBlock(memory, ctx = {}) {
  ...
  if (isSocialAcknowledgmentMessage(msg)) {
    lines.push(
      "* Patient message is only thanks, OK, tamam, anladım, or a greeting — reply in 1–2 short warm sentences. Do NOT repeat pricing, doctor counts, appointment pitches, or facts already stated in recent messages.",
