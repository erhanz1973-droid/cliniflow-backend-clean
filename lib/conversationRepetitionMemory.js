...
function topicsFromText(text, isAi = false) {
  ...
  if (textDiscussesReferralTopic(t)) topics.push("referral");
...
function buildRepetitionSuppressionPromptBlock(memory, ctx = {}) {
  ...
  if (memory.recentTopics.includes("referral")) {
    lines.push(
      "* Referral / invite-friend / discount thread was discussed recently — if the patient sends a short follow-up, continue referral (code, how to use) — do NOT pivot to unrelated clinical procedures.",
    );
  }
