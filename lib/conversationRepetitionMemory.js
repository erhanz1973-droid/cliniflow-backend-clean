...
/** Thanks / short ack — greetings handled by greetingIntent.js, not as «Tamam». */
const SOCIAL_ACK_RE =
  /^(tesekkur|tesekkurler|sagol|sag\s*ol|thanks|thank\s*you|thank\s*u|iyi\s+aksamlar|iyi\s+geceler|gunaydin|good\s+(evening|morning|night)|gamarjoba)[\s!.?,🙏]*$/iu;

/** Short conversational replies — should advance, not re-answer prior topic. */
const CONVERSATIONAL_SHORT_RE =
  /^(?:ok(?:ay)?|tamam|olur|peki|anlad[ıi]m|süper|super|harika|great|good|cool|nice|thanks?|teşekkür(?:ler)?|sağol|sağ\s*ol|thank\s*you|thank\s*u|got\s*it|i\s+understand|understood|evet|yes|yep|yeah|sure|alright|fine|makes\s+sense)[\s!.?,👍🙏👌]*$/iu;

/** «Tamam teşekkür ederim», «ok thanks» — compound thanks (not a new request). */
const THANKS_COMPOUND_RE =
  /^(?:tamam|ok|peki|olur|anlad[ıi]m|super|harika|great|yes|evet)[\s!.?,👍🙏👌]*(?:tesekkur|sagol|thank|ric|mersi)/iu;

const THANKS_PHRASE_RE =
  /\b(tesekkur\s*ederim|cok\s*tesekkur|thank\s*you\s*very\s*much|thanks\s*a\s*lot|sagolunuz|rica\s*ederim)\b/iu;
...
/** User-requested duplicate threshold for hard prevention. */
const REPEAT_PREVENT_SIM = 0.8;
...
function normalizeDiscussionMemory(persisted) {
  ...
  return {
    ...
    lastIntent: p.lastIntent ? String(p.lastIntent) : null,
    lastAssistantMessage: p.lastAssistantMessage ? String(p.lastAssistantMessage) : null,
    lastAction: p.lastAction ? String(p.lastAction) : null,
...
/**
 * Compound thanks after booking (e.g. tamam teşekkür ederim).
 * @param {string} text
 */
function isPostBookingThanksMessage(text) {
  const raw = String(text || "").trim();
  if (!raw || raw.length > 120) return false;
  if (/\b(randevu\s+almak|randevu\s+istiyorum|fiyat|appointment|book\s+a)\b/i.test(raw)) {
    return false;
  }
  try {
    const { normalizeForIntentMatch } = require("./greetingIntent");
    const t = normalizeForIntentMatch(raw);
    if (THANKS_COMPOUND_RE.test(t)) return true;
    if (THANKS_PHRASE_RE.test(t)) return true;
    if (/^(tamam|ok|peki)\b/.test(t) && /\b(tesekkur|thank|sagol|ric)\b/.test(t)) return true;
    return false;
  } catch (_) {
    const t = raw.toLocaleLowerCase("tr");
    return THANKS_COMPOUND_RE.test(t) || THANKS_PHRASE_RE.test(t);
  }
}
...
function isSocialAcknowledgmentMessage(text) {
  const raw = String(text || "").trim();
  if (!raw || raw.length > 120) return false;
  if (isPostBookingThanksMessage(raw)) return true;
  if (raw.length > 80) return false;
  try {
    const { isGreetingOnlyMessage, normalizeForIntentMatch } = require("./greetingIntent");
    if (isGreetingOnlyMessage(raw)) return true;
    const t = normalizeForIntentMatch(raw);
    return SOCIAL_ACK_RE.test(t) || CONVERSATIONAL_SHORT_RE.test(t);
  } catch (_) {
    const t = raw.toLocaleLowerCase("tr");
    return SOCIAL_ACK_RE.test(t) || CONVERSATIONAL_SHORT_RE.test(t);
  }
...
/**
 * OK / thanks / tamam / anladım — advance conversation, do not re-answer prior facts.
 * Pure greetings (merhaba, selam) are excluded — use greeting direct reply instead.
 * @param {string} text
 */
function isConversationalShortReply(text) {
  const raw = String(text || "").trim();
  if (!raw) return false;
  if (isPostBookingThanksMessage(raw)) return true;
  try {
    const { isGreetingOnlyMessage, normalizeForIntentMatch } = require("./greetingIntent");
    if (isGreetingOnlyMessage(raw)) return false;
    const t = normalizeForIntentMatch(raw);
    return CONVERSATIONAL_SHORT_RE.test(t) || SOCIAL_ACK_RE.test(t);
  } catch (_) {
    const t = raw.toLocaleLowerCase("tr");
    return CONVERSATIONAL_SHORT_RE.test(t) || SOCIAL_ACK_RE.test(t);
  }
 ...
 * @param {string} reply
 */
function detectAssistantActionType(reply) {
  const t = String(reply || "");
  if (!t.trim()) return "empty";
  if (PRICE_IN_TEXT.test(t)) return "pricing";
  if (/\b(available appointment times|müsait saat|müsait saatler|size uygun)\b/i.test(t) && /\b1\.\s/.test(t)) {
    return "booking_slots";
  }
  if (/\b(randevu|appointment|onaylıyor musunuz|confirm)\b/i.test(t)) return "booking_confirm";
  if (CTA_PATTERNS.some((c) => c.re.test(t))) return "cta";
  return "general";
}

/**
 * @param {string} patientMessage
 * @param {ReturnType<typeof buildDiscussionMemory>|null|undefined} discussionMemory
 */
function detectPatientTurnIntent(patientMessage, discussionMemory) {
  const msg = String(patientMessage || "").trim();
  if (!msg) return "empty";
  if (isConversationalShortReply(msg)) return "conversational_short";
  const commercial = detectPatientCommercialIntent(msg, {});
  if (commercial.asksPrice || commercial.isCommercialQuestion) return "pricing_question";
  if (/\b(randevu|appointment|book|schedule|müsait|musait)\b/i.test(msg)) return "booking";
  if (discussionMemory?.pricingAlreadyDiscussed && commercial.isCommercialQuestion === false) {
    return "follow_up";
  }
  return "general";
}
...
function shortSocialAckFallback(patientMessage, lang) {
  let t = "";
  try {
    const { normalizeForIntentMatch, isGreetingOnlyMessage, buildGreetingDirectReply } = require("./greetingIntent");
    t = normalizeForIntentMatch(patientMessage);
    if (isGreetingOnlyMessage(patientMessage)) {
      return buildGreetingDirectReply(patientMessage, lang);
    }
  } catch (_) {
    t = String(patientMessage || "").toLocaleLowerCase("tr");
  }
...
/**
 * Acknowledge briefly and move forward instead of repeating prior facts.
 * @param {{
 *   patientMessage?: string,
 *   language?: string|null,
 *   discussionMemory?: ReturnType<typeof buildDiscussionMemory>|null,
 *   recentTurns?: Array<{ role: string, text: string }>,
 * }} params
 */
function buildConversationForwardReply(params) {
  const patientMessage = String(params.patientMessage || "");
  const lang = params.language;
  const mem = params.discussionMemory;
  const hasActiveAppointment = params.hasActiveAppointment === true;

  if (hasActiveAppointment && isConversationalShortReply(patientMessage)) {
    try {
      const { buildPostBookingPatientAckReply } = require("./appointmentCoordinationSync");
      return buildPostBookingPatientAckReply(lang, params.operationalFlags || {});
    } catch (_) {
      const tr = isTurkishConversation(lang, patientMessage);
      return tr
        ? "Rica ederiz! Randevunuz kayıtlı. Başka bir sorunuz olursa yazabilirsiniz."
        : "You're welcome! Your appointment is already booked.";
    }
  }
  try {
    const { isGreetingOnlyMessage, buildGreetingDirectReply } = require("./greetingIntent");
    if (isGreetingOnlyMessage(patientMessage)) {
      const bookingTopic = (mem?.recentTopics || []).some((t) =>
        String(t).includes("consultation") ||
        String(t).includes("appointment") ||
        String(t).includes("booking"),
      );
      return buildGreetingDirectReply(patientMessage, lang, { bookingContinuity: bookingTopic });
    }
  } catch (_) {
    /* non-fatal */
  }
  const tr = isTurkishConversation(lang, patientMessage);
  const ka = !tr && isGeorgianConversation(lang, patientMessage);
  const pricingDone = mem?.pricingAlreadyDiscussed === true;
  const bookingTopic = (mem?.recentTopics || []).some((t) =>
    String(t).includes("consultation") || String(t).includes("appointment") || String(t).includes("booking"),
  );

  if (tr) {
    if (hasActiveAppointment) {
      return "Anladım. Randevunuz kayıtlı — saat değişikliği isterseniz yazın; yeniden randevu planlamaya gerek yok.";
    }
    if (pricingDone) {
      return "Tamam, not aldım. Fiyat konusunu netleştirdik — randevu planlamak veya başka bir tedavi hakkında bilgi almak ister misiniz?";
    }
    if (bookingTopic) {
      return "Anladım. Az önce paylaştığımız saatlerden birini seçebilir veya farklı bir gün/saat yazabilirsiniz.";
    }
    return "Tamam! Size nasıl devam edelim — randevu, tedavi detayı veya başka bir sorunuz var mı?";
  }
  if (ka) {
    return pricingDone
      ? "გასაგებია. ფასზე უკვე გითხარით — გსურთ ჩაწერა თუ სხვა კითხვა?"
      : "გასაგებია! რით გავაგრძელოთ — ჩაწერა, მკურნალობის დეტალი თუ სხვა კითხვა?";
  }
  if (hasActiveAppointment) {
    return "Understood. Your appointment is already booked — message us if you need to change the time.";
  }
  if (pricingDone) {
    return "Got it — we already covered pricing. Would you like to book a consultation or ask about another treatment?";
  }
  if (bookingTopic) {
    return "Understood. You can pick a number from the times we shared or suggest another day/time.";
  }
  return "Thanks! How would you like to continue — booking, treatment details, or another question?";
}

/**
 * Hard anti-repeat layer with structured logging.
 * @param {string} reply
 * @param {{
 *   patientMessage?: string,
 *   discussionMemory?: ReturnType<typeof buildDiscussionMemory>|null,
 ...
 * }} ctx
 */
function preventRepetitiveAssistantReply(reply, ctx = {}) {
  const patientMsg = String(ctx.patientMessage || "").trim();
  const recentTurns = ctx.recentTurns || [];
  const mem = ctx.discussionMemory;
  const logLabel = String(ctx.logLabel || "repeat_guard").trim();
  let out = String(reply || "").trim();
  const meta = {
    repeatDetected: false,
    repeatPrevented: false,
    reason: null,
    similarity: 0,
    lastIntent: detectPatientTurnIntent(patientMsg, mem),
    lastAction: detectAssistantActionType(out),
  };

  if (!out) return { reply: out, ...meta };

  const conversationalShort = isConversationalShortReply(patientMsg);
  const dupReply = detectNearDuplicateAssistantReply(out, recentTurns);
  const lastAssistant = recentTurnTextsByRole(recentTurns, "assistant", 1)[0] || null;

  if (dupReply.duplicate && dupReply.similarity >= REPEAT_PREVENT_SIM) {
    const recentDup = findRecentDuplicateWithAssistantReply(
      recentTurns,
      patientMsg,
      DUPLICATE_QUESTION_WINDOW_MS,
    );
    if (recentDup?.reply && ctx.reusePriorReplyWithin30s !== false) {
      meta.repeatDetected = true;
      meta.repeatPrevented = true;
      meta.similarity = dupReply.similarity;
      meta.reason = "reuse_prior_reply_within_30s";
      meta.lastAction = detectAssistantActionType(recentDup.reply);
      console.log("[repeat_prevented]", {
        label: logLabel,
        reason: meta.reason,
        similarity: Number(dupReply.similarity.toFixed(3)),
        patientPreview: patientMsg.slice(0, 48),
      });
      return { reply: recentDup.reply.trim(), ...meta };
    }
    meta.repeatDetected = true;
    meta.similarity = dupReply.similarity;
    meta.reason = "near_duplicate_reply";
    if (conversationalShort || mem?.pricingAlreadyDiscussed) {
      out = buildConversationForwardReply({
        patientMessage: patientMsg,
        language: ctx.conversationLanguage,
        discussionMemory: mem,
        recentTurns,
        hasActiveAppointment: ctx.hasActiveAppointment === true,
        operationalFlags: ctx.operationalFlags,
      });
    } else {
    ...
    meta.repeatPrevented = true;
    meta.lastAction = detectAssistantActionType(out);
    console.log("[repeat_detected]", {
      label: logLabel,
      reason: meta.reason,
      similarity: Number(dupReply.similarity.toFixed(3)),
      patientPreview: patientMsg.slice(0, 48),
    ...
    return { reply: out.trim(), ...meta };
  }

  if (
    conversationalShort &&
    (mem?.pricingAlreadyDiscussed || PRICE_IN_TEXT.test(lastAssistant || "")) &&
    (PRICE_IN_TEXT.test(out) || wordSetSimilarity(out, lastAssistant || "") >= 0.55)
  ) {
    meta.repeatDetected = true;
    meta.reason = "conversational_short_after_pricing";
    meta.similarity = wordSetSimilarity(out, lastAssistant || "");
    out = buildConversationForwardReply({
      patientMessage: patientMsg,
      language: ctx.conversationLanguage,
      discussionMemory: mem,
...
function applyConversationRepetitionGuardrails(reply, ctx = {}) {
  ...
  if (ctx.hasActiveAppointment && socialAck) {
    try {
      const {
        buildPostBookingPatientAckReply,
        replyOffersRedundantBooking,
      } = require("./appointmentCoordinationSync");
      if (replyOffersRedundantBooking(out) || /\brandevu\b/i.test(out) && out.length < 320) {
        out = buildPostBookingPatientAckReply(ctx.conversationLanguage, ctx.operationalFlags || {});
      }
    } catch (_) {
      /* non-fatal */
    }
  ...
  if (socialAck && PRICE_IN_TEXT.test(out)) {
    out = stripSentencesMatching(out, PRICE_IN_TEXT);
    out = stripSentencesMatching(
      out,
      /\b(dolgu|filling|implant|fiyat|price|try|tl|indirim|discount|clinifly|uygulama)\b/i,
    );
    if (!out || wordSetSimilarity(out, recentTurnTextsByRole(recentTurns, "assistant", 1)[0] || "") > 0.5) {
  ...
  if (dupReply.duplicate) {
    const strict =
      ...
      dupReply.similarity >= REPEAT_PREVENT_SIM ||
      (repeatedQ.repeated && dupReply.similarity >= NEAR_DUPLICATE_REPLY_SIM);
    if (strict) {
      if (socialAck && mem?.pricingAlreadyDiscussed) {
        out = buildConversationForwardReply({
          patientMessage: patientMsg,
          language: ctx.conversationLanguage,
          discussionMemory: mem,
  ...
  const prevented = preventRepetitiveAssistantReply(out, {
    ...ctx,
    logLabel: ctx.logLabel || "applyConversationRepetitionGuardrails",
  });
  return prevented.reply.trim();
}
...
function buildRepetitionSuppressionPromptBlock(memory, ctx = {}) {
  ...
  if (ctx?.hasActiveAppointment && isSocialAcknowledgmentMessage(msg)) {
    lines.push(
      "* Patient already has a CONFIRMED/SCHEDULED appointment in flags — reply with brief thanks only.",
      "* Do NOT offer to create/book/schedule another appointment or say «whenever you want you can book».",
      "* Confirm existing appointment time if helpful; invite questions about visit logistics only.",
    );
  }
  ...
  if (isSocialAcknowledgmentMessage(msg)) {
    const greetingOnly = (() => {
      try {
        ...
        return isGreetingOnlyMessage(msg);
      } catch {
        return false;
      }
    })();
    if (greetingOnly) {
      lines.push(
        "* Patient sent only a greeting (Merhaba, Selam, Hello, etc.) — reply with the SAME greeting word first (e.g. Merhaba!), then one warm sentence and how you can help. Do NOT reply with only «Tamam» or «OK».",
        "* Do NOT repeat pricing, doctor counts, or long clinic introductions.",
      );
    } else {
      lines.push(
        "* Patient message is only thanks, OK, tamam, or anladım — reply in 1–2 short warm sentences. Do NOT repeat pricing, doctor counts, appointment pitches, or facts already stated in recent messages.",
        "* Move the conversation forward with ONE next-step question (booking, another treatment, or clarifying question).",
      );
    }
  }
...
function updateDiscussionMemoryAfterTurn(memory, turn) {
  ...
  next.lastIntent = detectPatientTurnIntent(turn.patientMessage || "", next);
  next.lastAssistantMessage = String(turn.aiReply || "").trim().slice(0, 600) || null;
  next.lastAction = detectAssistantActionType(turn.aiReply || "");
  return next;
}
...
function mergeDiscussionMemoryIntoFlags(flags, memory) {
  ...
  return {
    ...
    discussionMemory: {
      ...
      lastAssistantMessage: memory.lastAssistantMessage,
...
module.exports = {
  ...
  detectPatientTurnIntent,
  detectAssistantActionType,
