...
/** @type {null | ((opts: Record<string, unknown>, row: Record<string, unknown>) => Promise<void>)} */
let doctorNotifyAfterMirrorFn = null;

/**
 * Wire socket emit + push (same path as in-app POST /messages).
 * @param {{ notifyAfterMirror?: typeof doctorNotifyAfterMirrorFn }} deps
 */
function setupOmnichannelMirrorDoctorNotify(deps) {
  doctorNotifyAfterMirrorFn = deps?.notifyAfterMirror || null;
}

/**
 * @param {Record<string, unknown>} opts
 * @param {Record<string, unknown>} insertedRow
 */
async function notifyDoctorAfterOmnichannelMirror(opts, insertedRow) {
  if (!doctorNotifyAfterMirrorFn || !insertedRow) return;
  await doctorNotifyAfterMirrorFn(opts, insertedRow);
}
...
async function mirrorOmnichannelInboundToPatientMessages(params) {
  ...
  const messageId =
    String(params.publicMessageId || "").trim() ||
    `omni_${channel}_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
  const baseRow = {
    patient_id: patientId,
    clinic_id: clinicId,
    thread_id: threadId,
  ...
  for (const from_role of fromRoles) {
    ...
    for (const payload of payloads) {
      ...
      if (!pr.error && pr.data) {
        ...
        const row = { ...pr.data, thread_id: threadId, clinic_id: clinicId };
        void (async () => {
          try {
            await supabase
              .from("patient_chat_threads")
        ...
        return { ok: true, row, channel, threadId };
      }
      lastError = pr.error;
...
module.exports = {
  mirrorOmnichannelInboundToPatientMessages,
  setupOmnichannelMirrorDoctorNotify,
  notifyDoctorAfterOmnichannelMirror,
};
