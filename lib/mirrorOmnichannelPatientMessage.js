 ...
 */
async function resolveThreadIdForMirror(patientId, clinicId) {
  const { resolveCanonicalChatThread } = require("./canonicalChatThread");
...
async function mirrorOmnichannelInboundToPatientMessages(params) {
  ...
  const threadId = await resolveThreadIdForMirror(patientId, clinicId);
