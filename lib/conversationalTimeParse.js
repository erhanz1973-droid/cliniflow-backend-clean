...
function parseConversationalTimeToMinutes(message) {
  ...
  // standalone "7" / "saat 7" (short replies)
  const short = t.match(/^(?:saat\s*)?(\d{1,2})$/);
  if (short && t.length <= 32) return toMinutes(short[1], 0);
...
function messageExpressesTimeIntent(message) {
  ...
  return /\b(saat|bucuk|yarim|gec|gece|kala|var)\b/.test(t);
}
