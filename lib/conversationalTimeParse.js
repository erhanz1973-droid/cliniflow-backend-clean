...
function parseConversationalTimeToMinutes(message) {
  ...
  m = t.match(/\b(\d{1,2})\s*[:.]\s*(\d{2})\s*(am|pm)\b/);
  if (m) {
    let h = Number(m[1]);
  ...
  // öğleden sonra 2 / öğleden sonra 2:15
  m = t.match(/\b(?:ogleden sonra|oglen sonrasi|afternoon)\s+(\d{1,2})(?:\s*[:.]\s*(\d{2}))?\b/);
  if (m) {
    let h = Number(m[1]);
    if (h >= 1 && h <= 11) h += 12;
...
/**
 * Patient asks what treatments/services the clinic offers (not appointment scheduling).
 * e.g. «Diş beyazlatma harici ne hizmetleriniz var?»
 * @param {string} message
 */
function isClinicServicesCatalogQuestion(message) {
  const t = normalizeTimeText(message);
  if (!t || t.length > 220) return false;

  const hasServiceNoun =
    /\b(hizmet\w*|tedavi\w*|prosedur\w*|procedure\w*|treatment\w*|service\w*|uygulama\w*)\b/.test(t);
  const hasOfferAsk =
    /\b(ne|neler|hangi|which|what|var|offer|sunuyor\w*|yapiyor\w*|yapıyor\w*|yapabilir\w*)\b/.test(t);

  if (hasServiceNoun && hasOfferAsk) return true;

  if (
    /\b(harici|haric|disinda|except|excluding|besides|other\s+than)\b/.test(t) &&
    hasServiceNoun
  ) {
    return true;
  }

  if (/\b(ne\s+yap\w*|neler\s+yap\w*|what\s+do\s+you\s+(offer|do|provide)|which\s+treatments)\b/.test(t)) {
    return true;
  }
