...
function getBundledCliniflySalesKb() {
  ...
  const iosUrl = getCliniflyIosAppStoreUrl();
  const androidUrl = getCliniflyAndroidPlayStoreUrl();
  const rows = [
    ["support.mobile_app_download", "mobile_app_download", 98, ["en", "tr", "ka", "ru"], ["download the app", "where can i download", "get the app", "iphone app", "android app", "clinifly app", "app store", "google play", "uygulama indir", "nereden indir", "nasil indir", "app link", "mobile app download"], `Facts: Clinifly mobile app (users/patients/doctors) — NOT clinic admin registration. iPhone App Store: ${iosUrl}. Android Google Play: ${androidUrl}. After install: create account, upload photos, AI recommendations, connect with clinics. App is free.`, "trial", ["app", "download", "mobile", "ios", "android"], "NEVER send admin-register.html or clinic admin registration URL when visitor asks to download/install the mobile app. Clinic registration URL is ONLY for explicit clinic sign-up requests."],
    ["sales.clinic_register", "clinic_registration", 96, ["en", "tr", "ka", "ru"], ["register clinic", "sign up", "join clinifly", "create account", "kayit ol", "klinik kaydi", "რეგისტრაცია", "დაწყება", "how to start"], `Facts: Free clinic registration, no credit card, self-service, immediate access. Registration URL: ${registerUrl}`, "trial", ["registration", "signup", "onboarding"], "Never use /sign-up or clinifly.net for clinic registration — only the URL above. Only when visitor explicitly asks to register their clinic — NOT for mobile app download questions."],
    ...
    ["ai.clinic_training", "ai_clinic_knowledge_training", 97, ["en", "tr", "ka", "ru"], ["teach prices to the ai", "train the ai with clinic", "can i teach my clinic prices", "treatment processes to the ai", "clinic-specific information ai", "ai training center", "treatment price list ai", "fiyatları ai öğret", "tedavi süreçlerini öğret", "klinik bilgilerini eğit", "ai asistanına öğret"], "Facts: YES — clinics train their Clinifly AI assistant with clinic-specific knowledge: Treatment Price List (pricing), AI Training Center (policies, tone, FAQs, workflows), working hours, materials/brands, international visitor procedures, transfer/accommodation, and clinic policies. The assistant uses this on WhatsApp, Messenger, and Clinifly chat.", "trial", ["ai_training", "pricing", "faq", "knowledge_base"], "NEVER say Clinifly does not allow teaching prices, treatment processes, or clinic FAQs to the AI — that is false and contradicts a core product feature."],
