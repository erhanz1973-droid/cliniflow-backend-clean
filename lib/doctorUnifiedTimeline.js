 ...
 */
async function fetchOfferThreadMessages(patientId, clinicId, coordinationOfferId) {
...
async function buildUnifiedSupervisionFeed(params) {
  const { profileId, clinicId, patientId, timeline, coordinationOfferId } = params;
  const seen = new Set();
  /** @type {Array<Record<string, unknown>>} */
  const merged = [];

  const [offerThreadMsgs, channelItems, clinicalItems, patientMsgs] = await Promise.all([
    fetchOfferThreadMessages(patientId, clinicId, coordinationOfferId),
