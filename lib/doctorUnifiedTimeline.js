...
async function fetchPatientClinicMessages(patientId, clinicId) {
  ...
  const selectAttempts = [
    "id, message, sender_type, from_role, sender_name, created_at, type, attachment_url, file_url",
    "id, message, from_role, created_at, attachment_url",
    "id, message, from_role, created_at",
    "id, message_text, sender_type, from_role, sender_name, created_at, type, attachment_url, file_url",
...
async function buildUnifiedSupervisionFeed(params) {
  ...
  const skipLegacyPatientMirror =
    workspaceMode &&
    (UUID_RE.test(effectiveOfferId) || UUID_RE.test(String(treatmentRequestId || "")));
  ...
  const [offerThreadMsgs, channelItems, clinicalItems, patientMsgs] = await Promise.all([
    ...
    skipLegacyPatientMirror ? Promise.resolve([]) : fetchChannelItems(profileId),
    fetchClinicalArtifacts(profileId),
    skipLegacyPatientMirror ? Promise.resolve([]) : fetchPatientClinicMessages(patientId, clinicId),
  ]);
  ...
  const chatEventItems = eventItems.filter((item) => {
    ...
    if (hasOfferThread && (item.kind === "message" || item.kind === "ai_draft")) return false;
    return (
      item.kind === "message" ||
      item.role === "patient" ||
      item.role === "ai" ||
