...
function dedupeKey(at, role, text, item = {}) {
  ...
  if (r === "doctor_intent" || item.kind === "doctor_intent") {
    return `meta|intent|${item.id || t.slice(0, 48)}`;
  }
  if (r === "ai_draft" || item.kind === "ai_draft") {
    return `meta|draft|${t}`;
  }
...
const OUTBOUND_ROLE_RANK = { doctor: 3, ai: 2, human: 1 };

/**
 * Same patient-visible text often appears as offer row + timeline/channel + pending draft.
 * Keep a single bubble; prefer doctor label over AI when both exist.
 * @param {Array<Record<string, unknown>>} items
 */
function pruneDuplicateSupervisionMessages(items) {
  const offerDoctorTexts = new Set(
    items
      .filter(
        (m) =>
          m.kind === "message" &&
          m.role === "doctor" &&
          String(m.source || "") === "offer_messages",
      )
      .map((m) => normalizeTextForDedupe(m.text)),
  );

  let out = items.filter((m) => {
    if (m.kind === "ai_draft" || m.role === "ai_draft") {
      const t = normalizeTextForDedupe(m.text);
      if (t && offerDoctorTexts.has(t)) return false;
    }
    return true;
  });
  ...
  for (const m of out) {
    ...
    if (!["ai", "doctor", "human"].includes(r)) continue;
    const ts = m.at ? new Date(m.at).getTime() : NaN;
    const bucket = Number.isFinite(ts) ? Math.floor(ts / 120_000) : 0;
    const key = `o|${bucket}|${t}`;
    const prev = groups.get(key);
    if (!prev) {
      groups.set(key, m);
      continue;
    }
    const prevRank = OUTBOUND_ROLE_RANK[String(prev.role || "")] || 0;
    const nextRank = OUTBOUND_ROLE_RANK[r] || 0;
    if (nextRank > prevRank) groups.set(key, m);
  }

  const dropOutboundDupes = new Set();
  for (const m of out) {
    if (m.kind !== "message") continue;
    const t = normalizeTextForDedupe(m.text);
    if (!t) continue;
    ...
    if (winner && String(winner.id) !== String(m.id)) dropOutboundDupes.add(String(m.id));
  }

  return out.filter((m) => !dropOutboundDupes.has(String(m.id)));
}
...
function mapOfferMessageRowToFeedItem(row) {
  ...
  let role = "human";
  let label = senderName || null;

  if (roleRaw === "patient") {
    role = "patient";
  } else if (actorKind === "clinic_ai") {
    role = "ai";
    label = senderName || "Care Team";
  } else if (actorKind === "doctor" || actorKind === "doctor_approved") {
    role = "doctor";
    label = senderName || "Doktor";
  } else {
    const isCoordinatorAi =
      senderNameLower === "care team" ||
      senderNameLower.includes("koordinat") ||
      senderNameLower.includes("ai koordinat") ||
      roleRaw === "assistant" ||
      roleRaw === "ai" ||
      roleRaw === "clinic" ||
      roleRaw === "system";
    if (isCoordinatorAi) {
      role = "ai";
      label = senderName || "Care Team";
    } else if (roleRaw === "doctor" || roleRaw === "dr") {
      role = "doctor";
      label = senderName || "Doktor";
    } else if (roleRaw === "clinic") {
      role = "ai";
      label = senderName || "Care Team";
    }
  }
 ...
 */
async function fetchOfferMessagesForSingleOffer(offerId, opts = {}) {
  const oid = String(offerId || "").trim();
  if (!UUID_RE.test(oid)) return [];
  const limit = Math.min(Math.max(Number(opts.limit) || 500, 20), 500);

  const selectAttempts = [
    "id, offer_id, sender_role, sender_name, text, message_text, attachment_url, created_at, message_source, actor_kind",
    "id, offer_id, sender_role, sender_name, text, attachment_url, created_at, message_source, actor_kind",
  ...
  for (const selectClause of selectAttempts) {
    ...
    if (!error) {
      ...
      return rows.map(mapOfferMessageRowToFeedItem);
    }
...
function normalizeMessageCreatedAtMs(createdRaw) {
  ...
  if (typeof createdRaw === "number" && Number.isFinite(createdRaw)) {
    return createdRaw > 0 && createdRaw < 1e11 ? Math.floor(createdRaw * 1000) : createdRaw;
  }
...
async function fetchChannelItems(profileId, limit = 150) {
  ...
  const cap = Math.min(400, Math.max(30, Number(limit) || 150));
  const { data, error } = await supabase
  ...
  const ordered = [...(data || [])].sort(
    (a, b) => normalizeMessageCreatedAtMs(a.created_at) - normalizeMessageCreatedAtMs(b.created_at),
  );
...
async function fetchOfferThreadMessages(
  ...
  const offerLimit = opts.offerMessageLimit;

  const directOfferId = String(coordinationOfferId || "").trim();
  if (UUID_RE.test(directOfferId)) {
    return await fetchOfferMessagesForSingleOffer(directOfferId, { limit: offerLimit });
  }
  ...
  if (UUID_RE.test(requestId)) {
    try {
      ...
      if (ensured.ok && ensured.offerId) {
        return await fetchOfferMessagesForSingleOffer(String(ensured.offerId), { limit: offerLimit });
      }
    } catch (e) {
  ...
  try {
    ...
    if (resolved && UUID_RE.test(resolved)) {
      return await fetchOfferMessagesForSingleOffer(resolved, { limit: offerLimit });
    }
  } catch (e) {
...
async function fetchPatientClinicMessages(patientId, clinicId) {
  ...
  const selectAttempts = [
    "id, message, sender_type, from_role, sender_name, created_at, type, attachment_url, file_url, message_source, actor_kind",
    "id, message, from_role, sender_name, created_at, attachment_url, message_source, actor_kind",
    "id, message, from_role, created_at, attachment_url",
    "id, message, from_role, created_at",
    "id, message_text, sender_type, from_role, sender_name, created_at, type, attachment_url, file_url, message_source, actor_kind",
    "id, message_text, from_role, sender_name, created_at, message_source, actor_kind",
  ...
  if (!data) {
    const loose = await supabase
      .from("patient_messages")
      .select("id, message, from_role, created_at")
      .eq("patient_id", patientId)
      .order("created_at", { ascending: true })
      .limit(200);
    if (loose.error) {
...
function injectProfileMessageFallback(merged, seen, params) {
  ...
  const lastAiNorm = normalizeTextForDedupe(lastAi);
  const hasPatient = merged.some((m) => m.role === "patient");
  const hasAi = merged.some((m) => m.role === "ai" || m.role === "human");
  const doctorAlreadySentSameText =
    lastAiNorm.length > 0 &&
    merged.some(
      (m) =>
        m.kind === "message" &&
        m.role === "doctor" &&
        normalizeTextForDedupe(m.text) === lastAiNorm,
    );
  ...
  if (lastAi && !hasAi && !doctorAlreadySentSameText) {
    pushItem(merged, seen, {
      id: "profile-last-ai",
      kind: "message",
      role: "ai",
...
/**
 * Resolve offer thread id for doctor workspace (flags may lag behind projection).
 * @param {string} patientId
 * @param {string} clinicId
 * @param {string|null|undefined} coordinationOfferId
 * @param {string|null|undefined} treatmentRequestId
 */
async function resolveWorkspaceCoordinationOfferId(
  patientId,
  clinicId,
  coordinationOfferId,
  treatmentRequestId,
) {
  const direct = String(coordinationOfferId || "").trim();
  ...
  if (UUID_RE.test(requestId)) {
    try {
      ...
      console.warn("[doctorUnifiedTimeline] workspace offer from request:", e?.message || e);
    }
  }
  ...
  try {
    ...
    if (resolved && UUID_RE.test(String(resolved))) return String(resolved);
  } catch (e) {
    console.warn("[doctorUnifiedTimeline] workspace offer resolve:", e?.message || e);
  }
...
async function buildUnifiedSupervisionFeed(params) {
  ...
  const workspaceMode = feedMode === "doctor_workspace";
  let effectiveOfferId = String(coordinationOfferId || "").trim();
  if (workspaceMode && !UUID_RE.test(effectiveOfferId)) {
    effectiveOfferId =
      (await resolveWorkspaceCoordinationOfferId(
        patientId,
        clinicId,
        coordinationOfferId,
        treatmentRequestId,
  ...
  const offerMessageLimit = workspaceMode ? 120 : 500;

  const [offerThreadMsgs, channelItems, clinicalItems, patientMsgs] = await Promise.all([
    fetchOfferThreadMessages(patientId, clinicId, effectiveOfferId || coordinationOfferId, treatmentRequestId, {
      offerMessageLimit,
    }),
  ...
  const chatEventItems = eventItems.filter((item) => {
    if (item.kind === "doctor_intent") return true;
    return (
      item.kind === "message" ||
      item.kind === "ai_draft" ||
      item.role === "patient" ||
  ...
  const pruned = pruneDuplicateSupervisionMessages(merged);
