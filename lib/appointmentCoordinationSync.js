...
function buildCalendarAppointmentPayloads(row, ctx = {}) {
  ...
  const procedure = String(row.procedure || row.treatment || "Consultation").trim();
...
/**
 * Best-effort insert into appointments / appointment_requests (admin schedule reads both).
 * @param {Record<string, unknown>} row
...
async function persistAppointmentRow(row, options = {}) {
  ...
  const built = buildCalendarAppointmentPayloads(row, {
  ...
  for (const table of tables) {
    for (const payload of built.variants) {
      ...
      try {
        const { data, error } = await supabase.from(table).insert(clean).select("id").single();
        if (!error && data?.id) {
  ...
  return { ok: false, reason: "insert_failed", message: errors[0] || "unknown" };
}
