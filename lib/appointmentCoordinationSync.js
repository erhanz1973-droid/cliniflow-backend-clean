...
const { clinicDayBoundsUtc, clinicLocalSlotToIso } = require("./clinicWorkingHours");
...
function formatAppointmentDisplay(iso, locale = "en", timeZone) {
  ...
  try {
    ...
    const connector = loc.startsWith("tr") ? " saat " : loc.startsWith("ru") ? ", " : " at ";
...
function normalizeAppointmentRecord(appointment) {
  const tz = String(appointment.timezone || "Europe/Istanbul").trim() || "Europe/Istanbul";
  const dateTimeLocal =
    appointment.date && appointment.time
      ? clinicLocalSlotToIso(
          String(appointment.date).slice(0, 10),
          String(appointment.time).trim().slice(0, 5),
          tz,
        )
      : null;
  const startAt =
 ...
 * @param {string} [locale]
 */
function buildTimelineCopy(eventType, appt, locale = "en", timeZone = "Europe/Istanbul") {
  const when = appt.startAt
    ? formatAppointmentDisplay(appt.startAt, locale, timeZone)
    : "the scheduled time";
 ...
 */
function buildAppointmentAwarenessPromptBlock(flags, timeZone = "Europe/Istanbul") {
  const f = flags && typeof flags === "object" ? flags : {};
  const appt = f.activeAppointment && typeof f.activeAppointment === "object" ? f.activeAppointment : null;
  if (!appt?.startAt) return "";

  const tz = String(appt.timezone || timeZone || "Europe/Istanbul").trim() || "Europe/Istanbul";
  const when = formatAppointmentDisplay(String(appt.startAt), "en", tz);
  const isPending =
 ...
 * @param {Record<string, unknown>|null|undefined} flags
 */
function hasActiveScheduledAppointment(flags) {
  const f = flags && typeof flags === "object" ? flags : {};
  const appt =
    f.activeAppointment && typeof f.activeAppointment === "object" ? f.activeAppointment : null;
 ...
 * @param {string} [lang]
 */
function formatActiveAppointmentWhenLabel(flags, lang = "tr") {
  const f = flags && typeof flags === "object" ? flags : {};
  const appt =
    f.activeAppointment && typeof f.activeAppointment === "object" ? f.activeAppointment : null;
  if (!appt?.startAt) return "";
  const key = String(lang || "tr").slice(0, 2).toLowerCase();
  const locale = key === "tr" ? "tr-TR" : key === "ka" ? "ka-GE" : key === "ru" ? "ru-RU" : "en-US";
  const tz = String(appt.timezone || "Europe/Istanbul").trim() || "Europe/Istanbul";
  return formatAppointmentDisplay(String(appt.startAt), locale, tz);
}

/**
 * Brief thanks reply when patient already has a booked appointment — no new booking pitch.
 * @param {string} [lang]
 * @param {Record<string, unknown>|null|undefined} flags
 */
function buildPostBookingPatientAckReply(lang = "tr", flags = {}) {
  const key = String(lang || "tr").slice(0, 2).toLowerCase();
  const when = formatActiveAppointmentWhenLabel(flags, key);

  if (key === "tr") {
    return when
      ? `Rica ederiz! Randevunuz ${when} için kayıtlı. Görüşme öncesi bir sorunuz olursa yazmanız yeterli.`
      : "Rica ederiz! Randevunuz kayıtlı. Başka bir sorunuz olursa yazabilirsiniz.";
  }
  if (key === "ka") {
    return when
      ? `მადლობა! ჩაწერა უკვე დაფიქსირებულია — ${when}.`
      : "მადლობა! ჩაწერა უკვე დაფიქსირებულია.";
  }
  if (key === "ru") {
    return when
      ? `Пожалуйста! Запись на ${when} уже оформлена. Если появятся вопросы — напишите.`
      : "Пожалуйста! Ваша запись уже оформлена.";
  }
  return when
    ? `You're welcome! Your appointment is booked for ${when}. Message us if anything comes up before the visit.`
    : "You're welcome! Your appointment is already booked. Let us know if you need anything else.";
}

/** Assistant reply still pushes scheduling though patient already has activeAppointment. */
const REDUNDANT_BOOKING_PITCH_RE =
  /\b(ne\s+zaman\s+isterseniz|istediğiniz\s+zaman|istediginiz\s+zaman|randevu\s+oluşturabilir|randevu\s+olusturabilir|randevu\s+alabilir|randevu\s+planlayabilir|randevu\s+planlamak|book\s+(?:a\s+)?(?:consultation|appointment)|schedule\s+(?:a\s+)?(?:visit|appointment)|pick\s+a\s+time|saatlerden\s+birini\s+seç|choose\s+a\s+time|whenever\s+you\s+(?:want|like|wish))\b/i;

/**
 * @param {string} reply
 */
function replyOffersRedundantBooking(reply) {
  return REDUNDANT_BOOKING_PITCH_RE.test(String(reply || ""));
}
...
async function syncAppointmentToCoordination(params) {
  ...
  const clinicTz = String(params.timezone || appt.timezone || "Europe/Istanbul").trim() || "Europe/Istanbul";
  const copy = buildTimelineCopy(eventType, appt, locale, clinicTz);
...
async function listPatientCoordinationTimelineItems(patientId, clinicId) {
  ...
  if (appt && typeof appt === "object" && appt.startAt) {
    ...
    items.push({
      ...
      description: formatAppointmentDisplay(iso, "en", "Europe/Istanbul"),
      start_date: iso,
...
async function resolveAiBookingCalendarContext(params) {
  ...
  if (patientName && /^messenger user$/i.test(String(patientName).trim())) patientName = null;

  if (!patientName && UUID_RE.test(patientId) && UUID_RE.test(clinicId)) {
    try {
      const { ensureMessengerPatientNameFromGraph } = require("./omnichannel/channelIdentity");
      const meta =
        params.profileRow?.channel_metadata &&
        typeof params.profileRow.channel_metadata === "object"
          ? params.profileRow.channel_metadata
          : {};
      const pageId = String(meta.messenger_page_id || "").trim();
      const psid = String(meta.messenger_psid || "").trim();
      const synced = await ensureMessengerPatientNameFromGraph({
        clinicId,
        patientId,
        psid,
        pageId,
      ...
      console.warn("[resolveAiBookingCalendarContext] messenger name:", mErr?.message || mErr);
    }
  }
...
async function resolvePatientDisplayLabels(patientIds, clinicId) {
  ...
  const { resolvePatientDisplayLabel } = require("./patientNameSync");
  ...
  for (let i = 0; i < ids.length; i += 80) {
    ...
    for (const run of [
      () =>
        supabase
          .from("patients")
          .select("id, name, full_name, email, phone, whatsapp, external_name, display_name_override")
          .in("id", chunk),
      () =>
        supabase
          .from("patients")
      ...
      for (const p of data || []) {
        ...
        const label = resolvePatientDisplayLabel(p) || String(p?.full_name || p?.name || p?.email || "").trim();
...
/**
 * Mark an existing calendar row cancelled before reschedule replaces it.
 * @param {string} appointmentId
 * @param {{ patientId?: string, clinicId?: string, locale?: string, source?: string, previousStartAt?: string, treatmentLabel?: string }} [options]
 */
async function cancelAppointmentRow(appointmentId, options = {}) {
  if (!isSupabaseEnabled()) return { ok: false, reason: "supabase_disabled" };
  ...
  const tables = [
    ...
    },
    {
      ...
      payloads: [{ status: "cancelled", updated_at: nowIso }],
    },
  ];

  for (const { name, payloads } of tables) {
    for (const patch of payloads) {
      try {
        const { data, error } = await supabase
          ...
          .maybeSingle();
        if (!error && data?.id) {
          if (UUID_RE.test(String(options.patientId || ""))) {
            await syncAppointmentToCoordination({
              patientId: String(options.patientId),
              clinicId: options.clinicId || null,
              eventType: EVENT_TYPES.CANCELLED,
              appointment: {
                id,
                startAt: options.previousStartAt || null,
                status: "cancelled",
                treatmentLabel: options.treatmentLabel || "Consultation",
              },
              source: options.source || "ai_reschedule_cancel",
              sendPatientMessage: false,
              locale: options.locale,
            }).catch((e) =>
              console.warn("[cancelAppointmentRow] coordination:", e?.message || e),
            );
          }
          return { ok: true, id: String(data.id), table: name };
        }
      } catch (e) {
        console.warn("[cancelAppointmentRow]", name, e?.message || e);
      }
    }
  }
...
module.exports = {
  ...
  formatActiveAppointmentWhenLabel,
