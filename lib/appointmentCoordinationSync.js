...
async function ensureEncounterForAiBooking(patientUuid, clinicId) {
  ...
  if (Array.isArray(encList) && encList.length) {
    ...
    if (encounterId && UUID_RE.test(clinicId)) {
      const row = withClinic || encList[0];
      if (!String(row?.clinic_id || "").trim()) {
        await supabase
          .from("patient_encounters")
  ...
  if (UUID_RE.test(patientId)) {
    ...
    if (!patientName) {
      try {
        ...
        if (raw && !looksLikeOpaquePatientId(raw)) patientName = raw;
        else if (pRow?.whatsapp || pRow?.phone) {
