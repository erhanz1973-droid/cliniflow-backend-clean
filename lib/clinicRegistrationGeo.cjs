...
function inferCountryFromPhone(phone) {
  ...
  // Georgia local mobile: 5XX XXX XXX (9 digits, no country prefix)
  if (/^5\d{8}$/.test(normalized)) return "GE";
  // Turkey local mobile: 5XX XXX XX XX (10 digits)
  if (/^5\d{9}$/.test(normalized)) return "TR";
  return null;
}
