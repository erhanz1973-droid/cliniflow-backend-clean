...
const APP_REGISTRATION_QUESTION_RE =
  /\b(klinik\s*kod|clinic\s*code|uygulama\w*|clinifly|clinifly\.net|indir\w*|yukle\w*|yükley\w*|kaydol\w*|kayıt|kayit|üye\s*ol|uye\s*ol|register|sign\s*up|download|app\s*store|google\s*play|hesap\s*aç|membership|enroll)\b/i;
...
function buildPatientAppDownloadDirectReply(lang = "tr", clinicCode = null) {
  ...
  if (key === "tr") {
    let text =
      `Clinifly hasta uygulamasını ${url} adresinden indirebilirsiniz. Bu sayfada App Store ve Google Play bağlantıları yer alır.\n\n`;
    text +=
  ...
  if (key === "ru") {
    let text = `Скачать приложение Clinifly: ${url} (App Store и Google Play на этой странице).\n\n`;
    text += "После установки выберите регистрацию по коду клиники и введите наш код";
    if (code) text += ` ${code}`;
    text += ".";
    return text;
  }
  let text = `Download the Clinifly patient app at ${url} — App Store and Google Play links are on that page.\n\n`;
...
function buildAiClinicMembershipAfterBookingNotice(lang = "tr", clinicCode = null) {
  ...
  if (key === "tr") {
    let text =
      `Önemli (Clinifly): Tedavi süreciniz, kliniğimize üye olduktan sonra başlar. En hızlı yol: ${url} üzerinden Clinifly hasta uygulamasını indirin (App Store / Google Play), «Klinik kodu ile kaydol» ekranına klinik kodumuzu yazın`;
    if (code) text += ` (${code})`;
    text +=
  ...
  let text =
    `Important (Clinifly): Your treatment journey starts once you enroll with our clinic in the app. Fastest path: download from ${url} (App Store / Google Play), choose register with clinic code, and enter our clinic code`;
  if (code) text += ` (${code})`;
  text +=
