 ...
 * @param {string} [lang]
 */
function buildSmileAnalysisMessengerConversionReply(lang = "en") {
  const key = String(lang || "en").slice(0, 2).toLowerCase();
  const ios = getCliniflyIosAppStoreUrl();
  const android = getCliniflyAndroidPlayStoreUrl();
  ...
  if (key === "tr") {
    return (
      "🦷 Gülüşünüzü nasıl geliştirebileceğinizi keşfetmek ister misiniz?\n\n" +
      "En iyi deneyim için Clinifly uygulamasını indirin.\n\n" +
      "📸 Diş fotoğrafınızı yükleyin\n" +
      "🤖 Kişisel AI önerileri alın\n" +
      "🏥 İsterseniz kliniklerle iletişime geçin\n\n" +
      `📱 iPhone:\n${ios}\n\n` +
      `📱 Android:\n${android}\n\n` +
      "Fotoğraf yükleme ve AI gülüş analizi Clinifly uygulamasında yapılır — Messenger içinde değil."
    );
  }
  if (key === "ka") {
    return (
      "🦷 გსურთ გაიგოთ, რა შეიძლება გაუმჯობესდეს თქვენს ღიმილში?\n\n" +
      "საუკეთესო გამოცდილებისთვის ჩამოტვირთეთ Clinifly აპი.\n\n" +
      "📸 ატვირთეთ კბილების ფოტო\n" +
      "🤖 მიიღეთ AI რეკომენდაციები\n" +
      "🏥 სურვილის შემთხვევაში დაუკავშირდით კლინიკებს\n\n" +
      `📱 iPhone:\n${ios}\n\n` +
      `📱 Android:\n${android}\n\n` +
      "ფოტოს ატვირთვა და AI ანალიზი აპში ხდება — არა Messenger-ში."
    );
  }
  if (key === "ru") {
    return (
      "🦷 Хотите узнать, что можно улучшить в вашей улыбке?\n\n" +
      "Для лучшего опыта скачайте приложение Clinifly.\n\n" +
      "📸 Загрузите фото зубов\n" +
      "🤖 Получите персональные рекомендации ИИ\n" +
      "🏥 При желании свяжитесь с клиниками\n\n" +
      `📱 iPhone:\n${ios}\n\n` +
      `📱 Android:\n${android}\n\n` +
      "Загрузка фото и анализ улыбки — только в приложении Clinifly, не в Messenger."
    );
  }
  return (
    "🦷 Want to discover what could improve your smile?\n\n" +
    "For the best experience, download the Clinifly app.\n\n" +
    "📸 Upload your dental photo\n" +
    "🤖 Get personalized AI recommendations\n" +
    "🏥 Connect with clinics if desired\n\n" +
    `📱 iPhone:\n${ios}\n\n` +
    `📱 Android:\n${android}\n\n` +
    "Photo upload and AI smile analysis happen inside the Clinifly app — not in Messenger."
  );
}
