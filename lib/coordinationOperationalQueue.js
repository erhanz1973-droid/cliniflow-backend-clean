 ...
 * @param {number} limit
 */
async function queryProfilesWithSelectFallback(buildQuery) {
  let select = PROFILE_SELECT_FULL;
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const { data, error } = await buildQuery(select);
    if (!error) return { data, error: null, selectUsed: select };
    if (attempt === 0 && isMissingColumnError(error)) {
      select = PROFILE_SELECT_CORE;
      continue;
    }
    return { data: null, error, selectUsed: select };
  }
  return { data: null, error: null, selectUsed: PROFILE_SELECT_CORE };
}

async function fetchProfileRows(clinicId, limit) {
  const { data, error } = await queryProfilesWithSelectFallback((select) =>
    supabase
 ...
 */
async function fetchProfileById(clinicId, profileId) {
  const { data, error } = await queryProfilesWithSelectFallback((select) =>
    supabase
      .from("ai_coordinator_lead_profiles")
      .select(select)
 ...
 */
function treatmentRequestPreviewText(tr) {
  return String(tr.description ?? tr.message ?? tr.notes ?? "").trim();
}
...
async function fetchActiveTreatmentRequests(clinicId, limit) {
  const selectAttempts = [
    "id, patient_id, clinic_id, status, proposal_status, lead_status, preferred_treatment, description, created_at, updated_at, first_clinic_response_at",
    "id, patient_id, clinic_id, status, preferred_treatment, description, created_at, updated_at",
    "id, patient_id, clinic_id, status, preferred_treatment, message, created_at, updated_at",
    "id, patient_id, clinic_id, status, preferred_treatment, created_at, updated_at",
  ];
  ...
  for (const selectClause of selectAttempts) {
    let q = supabase
      ...
      .order("updated_at", { ascending: false })
      .limit(limit);
    if (selectClause.includes("status") && selectClause.includes("proposal_status")) {
...
async function syncProfilesFromTreatmentRequests(clinicId, requests, profileByPatient) {
  ...
  const toSync = missing.slice(0, 12);
  for (let i = 0; i < toSync.length; i += 4) {
    const chunk = toSync.slice(i, i + 4);
    await Promise.all(
      chunk.map(async (tr) => {
...
function hydrateProfileRowsFromRequests(rows, requests, latestByRequest) {
  ...
  return rows.map((row) => {
    ...
    const reqMsg = treatmentRequestPreviewText(tr);
