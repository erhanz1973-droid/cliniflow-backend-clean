...
const { executeDirectPatientMessage } = require("./clinicalGuidanceRoutes");
...
/**
 * Match GET /api/doctor/patient/:id/messages — JWT clinic may differ from canonical thread clinic.
 * @param {import('express').Request} req
 * @param {string} patientId
 */
async function resolveDoctorWorkspaceClinicId(req, patientId) {
  const jwtClinic = getClinicId(req);
  const pid = String(patientId || "").trim();
  if (!UUID_RE.test(pid)) return jwtClinic;

  try {
    const { getCanonicalThread, CANONICAL_THREAD_READ_OPTS } = require("./canonicalChatThread");
    const canonical = await getCanonicalThread(pid, UUID_RE.test(jwtClinic) ? jwtClinic : null, {
      ...CANONICAL_THREAD_READ_OPTS,
      source: "coordination_workspace_clinic",
    });
    if (canonical.clinicId && UUID_RE.test(String(canonical.clinicId))) {
  ...
  if (UUID_RE.test(jwtClinic)) return jwtClinic;
  ...
  try {
    const { data: prow } = await supabase
      ...
      .eq("id", pid)
      .maybeSingle();
    const fromPatient = String(prow?.clinic_id || "").trim();
    if (UUID_RE.test(fromPatient)) return fromPatient;
...
async function findLeadProfileForPatient(clinicId, patientId) {
  ...
  const match = UUID_RE.test(cid)
    ? rows.find((r) => String(r.clinic_id || "") === cid) || null
    : rows[0] || null;
  if (!match) return null;
  const [named] = await attachPatientNamesToProfileRows([match]);
  return named || match;
}

function profileClinicId(row, fallbackClinicId) {
  const fromRow = String(row?.clinic_id || "").trim();
  if (UUID_RE.test(fromRow)) return fromRow;
  const fb = String(fallbackClinicId || "").trim();
  return UUID_RE.test(fb) ? fb : fromRow;
}
 ...
 * @param {string} [lang]
 */
async function fetchTimeline(profileId, lang = "en", opts = {}) {
  const limit = Math.min(Math.max(Number(opts.limit) || 120, 20), 200);
  const { data, error } = await supabase
  ...
  return rows.map((ev) => mapTimelineEvent(ev, lang));
}
...
function registerAiCoordinatorDoctorRoutes(app, deps) {
  ...
  if (typeof insertClinicMessage === "function") {
    router.post("/patients/:patientId/direct-message", requireDoctorAuth, (req, res) =>
      executeDirectPatientMessage(req, res, { insertClinicMessage }),
    );
  }
  ...
  router.get("/patients/:patientId/ai-coordination", requireDoctorAuth, async (req, res) => {
    try {
      ...
      const clinicId = await resolveDoctorWorkspaceClinicId(req, patientId);
      ...
      let row = await findLeadProfileForPatient(clinicId, patientId);
      if (!row) {
        return res.json({
          ok: true,
          profile: null,
      ...
      const { expireSnoozeAndResumeProfile, triggerSnoozeCatchUpAiReply } = require("./aiSnoozeCatchUp");
      const { hadSnoozeThatExpired } = require("./aiDelegation");
      const snoozeJustExpired = hadSnoozeThatExpired(row);
      row = await expireSnoozeAndResumeProfile(row);
      if (snoozeJustExpired && row?.patient_id) {
        void triggerSnoozeCatchUpAiReply({
          profileId: String(row.id),
          clinicId,
          patientId,
          source: "snooze_expired_poll",
        }).catch((e) => console.warn("[GET ai-coordination] catch-up:", e?.message || e));
      }
  ...
  router.get("/patients/:patientId/coordination-workspace", requireDoctorAuth, async (req, res) => {
    try {
      ...
      if (!row) {
        try {
          await ensureLeadWorkspaceForClinic(patientId, clinicId, {
            source: "doctor_coordination_workspace",
          });
          row = await findLeadProfileForPatient(clinicId, patientId);
        } catch (ensureErr) {
          console.warn("[coordination-workspace] ensure profile:", ensureErr?.message || ensureErr);
        }
      }
      if (!row) {
      ...
      void projectCoordinationState(row.id).catch((e) =>
        console.warn("[coordination-workspace] projection:", e?.message || e),
      );

      const uiLang = uiLangFromRequest(req);
      const intakeFlagsEarly =
        row.operational_intake_flags && typeof row.operational_intake_flags === "object"
          ? row.operational_intake_flags
          : {};
      const coordinationOfferIdEarly =
        intakeFlagsEarly.coordinationOfferId || intakeFlagsEarly.coordination_offer_id || null;

      const [clinicPolicy, timeline] = await Promise.all([
        loadClinicPolicyForClinic(clinicId),
        fetchTimeline(row.id, uiLang, { limit: 50 }),
      ]);
      const base = enrichLeadRow(row, mapProfileRow);
      const enriched = enrichLeadForCoordinatorUI(base, uiLang);
      const lead = attachDelegationToLead(enriched, clinicPolicy, uiLang);
      const intakeFlags = intakeFlagsEarly;
      const latestAiFromTimeline =
        [...timeline].reverse().find((ev) => ev.aiReply)?.aiReply || null;
      const conversation = await buildUnifiedSupervisionFeed({
        ...
        timeline,
        uiLang,
        coordinationOfferId: coordinationOfferIdEarly,
        treatmentRequestId: intakeFlags.treatmentRequestId || intakeFlags.treatment_request_id || null,
        feedMode: "doctor_workspace",
        lastPatientMessage: row.last_patient_message || null,
        lastPatientMessageAt: row.last_patient_message_at || null,
        lastAiReply: latestAiFromTimeline,
  ...
  router.post("/patients/:patientId/retry-ai-reply", requireDoctorAuth, async (req, res) => {
    try {
      const patientId = String(req.params.patientId || "").trim();
      const clinicId = await resolveDoctorWorkspaceClinicId(req, patientId);
      if (!UUID_RE.test(clinicId) || !UUID_RE.test(patientId)) {
      ...
      const flags =
        row.operational_intake_flags && typeof row.operational_intake_flags === "object"
          ? row.operational_intake_flags
          : {};
      const patientMessage = String(
        row.last_patient_message ||
          [...(await fetchTimeline(row.id, "en", { limit: 20 }))]
            .reverse()
            .find((ev) => ev.patientMessage)?.patientMessage ||
          "",
      ).trim();
      if (!patientMessage) {
        return res.status(400).json({ ok: false, error: "no_patient_message" });
      }

      const offerId =
        flags.coordinationOfferId || flags.coordination_offer_id || req.body?.offerId || null;
      ...
      const aiResult = await runAiReplyForClinicInbound({
        ...
        treatmentRequestId: flags.treatmentRequestId || flags.treatment_request_id || null,
      });

      return res.json({
        ok: aiResult.sent === true,
        sent: aiResult.sent === true,
        reason: aiResult.reason || null,
      });
    } catch (e) {
      console.error("[POST doctor retry-ai-reply]", e?.message || e);
      return res.status(500).json({ ok: false, error: "internal_error" });
    }
  });
  ...
  router.patch("/patients/:patientId/ai-coordination", requireDoctorAuth, async (req, res) => {
    try {
      ...
      if (!row) {
        await ensureLeadWorkspaceForClinic(patientId, clinicId, {
          source: "doctor_intervention_patch",
        });
        row = await findLeadProfileForPatient(clinicId, patientId);
      }
      if (!row) {
        return res.status(404).json({
          ...
          message: "Could not open AI coordination for this patient. Try again in a moment.",
        });
      }

      const profileClinic = profileClinicId(row, clinicId);
      const result = await patchInquiryDelegation({
        clinicId: profileClinic,
        profileId: row.id,
        body: req.body || {},
      ...
      const actionNorm = String(req.body?.action || "")
        .trim()
        .replace(/([A-Z])/g, "_$1")
        .toLowerCase()
        .replace(/^_/, "");
      const isResume =
        actionNorm === "resume_ai" ||
        req.body?.clearEscalation === true ||
        req.body?.resumeAi === true;
      if (isResume) {
        const { triggerSnoozeCatchUpAiReply } = require("./aiSnoozeCatchUp");
        void triggerSnoozeCatchUpAiReply({
          profileId: row.id,
          clinicId,
          patientId,
          source: "doctor_resume_ai",
        }).catch((e) =>
          console.warn("[PATCH doctor ai-coordination] snooze catch-up:", e?.message || e),
        );
      }
