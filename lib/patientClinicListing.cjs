...
const { sortClinicRowsByUserCountry } = require("./clinicCountryPriority.cjs");
  ...
  if (cityCanonical && String(cityCanonical).trim()) {
    const want =
      typeof resolveCityCode === "function"
        ...
        : String(cityCanonical).trim().toLowerCase();
    rows = rows.filter((c) => {
      const ex = c.city_code != null ? String(c.city_code).trim().toLowerCase() : "";
      if (ex) return ex === want;
      return resolveCityCode(c.city) === want;
    });
  }
