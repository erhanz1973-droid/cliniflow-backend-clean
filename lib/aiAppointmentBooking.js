...
async function tryCompletePendingAiBooking(params) {
  ...
  if (prevStage === "awaiting_slot_confirm") {
    if (!confirmYes) {
  ...
  let slot =
    (state.offeredSlots?.length
      ...
      : null) ||
    state.selectedSlot ||
    slotFromBookingFlags(flags, scheduling, booking, treatmentLabel, locale);
  ...
  const offeredCount = state.offeredSlots?.length || 0;
  const skipTimeFromListIndex =
    offeredCount > 0 && isBareSlotListIndexMessage(message, offeredCount);
...
async function completeBookingAfterSlotConfirmation(params) {
  ...
  if (String(state.stage || "") !== "awaiting_slot_confirm") return null;
  if (!isBookingConfirmationYes(message)) return null;
  ...
  const slotToBook = state.selectedSlot;
  const whenPending =
    slotToBook?.label ||
    (slotToBook?.startAt
...
async function prepareAiAppointmentBookingTurn(params) {
  ...
  const listIndexOnly =
    offeredSlots.length > 0 && isBareSlotListIndexMessage(message, offeredSlots.length);
  ...
  if (state.stage === "awaiting_slot_confirm" && isBookingConfirmationYes(message)) {
    const confirmed = await completeBookingAfterSlotConfirmation({
      message,
      state,
      contact,
