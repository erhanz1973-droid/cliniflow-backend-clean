...
function shouldEngageAppointmentBooking(message, leadData, flags, profileRow) {
  ...
  const timeIntent =
    messageExpressesTimeIntent(msg) && parseConversationalTimeToMinutes(msg) != null;
  const state = readAiBookingState(flags);
...
function parseSlotSelectionFromMessage(message, offeredSlots) {
  ...
  const norm = t
    ...
    .replace(/ı/g, "i")
    .trim();
  if (trOrdinals[norm] != null) {
    const idx = trOrdinals[norm] - 1;
    if (idx >= 0 && idx < slots.length) return slots[idx];
  }
  const optWord = norm.match(/^(?:secenek|secenegim|numara|option)\s*(\d{1,2})$/);
  if (optWord) {
    const idx = Number(optWord[1]) - 1;
    if (idx >= 0 && idx < slots.length) return slots[idx];
  }
...
async function prepareAiAppointmentBookingTurn(params) {
      ...
      if (reconciled.ok) {
        ...
        if (restatesOnly) {
          return {
            engaged: true,
            promptBlock: "",
            directReply: buildDirectBookingReply(
              locale.slice(0, 2),
              booking.mode,
              whenLabel,
              treatmentLabel,
  ...
  if (!selected && (timeOnly || (parsedTimeMin != null && timeHm))) {
    const timeIntro =
      timeHm && slots.length
        ? lang === "tr"
