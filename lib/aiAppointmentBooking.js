...
function buildDirectBookingReply(lang, mode, whenLabel, treatmentLabel, opts = {}) {
  ...
  if (opts.includeClinicMembershipNotice) {
    base = `${base}\n\n${buildAiClinicMembershipAfterBookingNotice(key)}`;
  }
  return base;
}
 ...
 */
async function buildDirectBookingReplyForPatient(lang, mode, whenLabel, treatmentLabel, patientId) {
...
async function prepareAiAppointmentBookingTurn(params) {
  ...
  const timeOnly = isTimeOnlyPatientMessage(message) && !isSlotIndexOnlyReply;
