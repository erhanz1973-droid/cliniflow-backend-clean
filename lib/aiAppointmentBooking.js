...
async function tryCompletePendingAiBooking(params) {
  ...
  if (prevStage === "awaiting_slot_confirm" && !confirmYes) {
    if (!shouldRepeatSlotConfirmationNudge(message, state, scheduling)) {
      return null;
    }
 ...
 * @param {{ timezone?: string }} scheduling
 */
function shouldRepeatSlotConfirmationNudge(message, state, scheduling) {
  const msg = String(message || "").trim();
  if (!msg) return false;
...
async function prepareAiAppointmentBookingTurn(params) {
  ...
  if (shouldFinalizeSlotOnPatientYes(message, state, flags, recentTurns)) {
    ...
    const whenPending =
      ...
      (state.selectedSlot?.startAt
        ? formatBookingWhenLabel(state.selectedSlot.startAt, locale, scheduling.timezone)
        : "");
    return {
      engaged: true,
      promptBlock: "",
