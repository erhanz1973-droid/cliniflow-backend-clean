...
} = require("./bookingDateParse");
const { detectMessageLanguage, normalizeLangCode } = require("./conversationLanguage");
const {
...
/**
 * Keep weekday/date intent across contact gate (e.g. "Friday" then phone-only reply).
 * @param {{
 *   message: string,
 *   timezone: string,
 ...
 *   negativeScheduling?: boolean,
 *   refDate?: Date,
 * }} params
 */
function resolveEffectivePreferredDateYmd(params) {
  if (params.negativeScheduling) return null;
  const tz = params.timezone || "Europe/Istanbul";
  const refDate = params.refDate instanceof Date ? params.refDate : new Date();
  const fromMsg = parsePreferredDateFromMessage(String(params.message || ""), tz, refDate);
  if (fromMsg) return fromMsg;
  const turns = Array.isArray(params.recentTurns) ? params.recentTurns : [];
  ...
  if (state.preferredDateYmd) return String(state.preferredDateYmd);
  const durable = readDurableBookingState(params.flags || {});
  if (durable.selectedDate) return String(durable.selectedDate);
  if (durable.preferredDateYmd) return String(durable.preferredDateYmd);
  const ab =
    params.flags?.aiBooking && typeof params.flags.aiBooking === "object"
      ? params.flags.aiBooking
      : {};
  if (ab.preferredDateYmd) return String(ab.preferredDateYmd);
  if (ab.selectedDate) return String(ab.selectedDate);
  return null;
}
...
function patientMessageAdvancesSlotBooking(message, state, scheduling) {
  ...
  if (isClinicServicesCatalogQuestion(msg) || isClinicFacilityOrInfoQuestion(msg)) return false;
  if (patientRequestsSlotListResend(msg) || patientRequestsNewSchedulingWindow(msg)) return true;
  const tz = scheduling?.timezone || "Europe/Istanbul";
  if (parseSlotSelectionFromMessage(msg, state.offeredSlots)) return true;
  if (isBookingConfirmationYes(msg, { pendingConfirmation: true }) || isBookingConfirmationNo(msg)) return true;
...
/**
 * Prefer the patient's latest message language for booking direct replies (Messenger/WhatsApp).
 * @param {string} locale
 * @param {string} [patientMessage]
 */
function resolveBookingReplyLang(locale, patientMessage) {
  const fallback = normalizeLangCode(String(locale || "").slice(0, 2)) || "en";
  const det = detectMessageLanguage(String(patientMessage || "").trim());
  if (!det.code) return fallback;
  if (det.cyrillic || det.georgianScript || det.confidence >= 0.38) {
    return det.code;
  }
  return fallback;
}
...
function buildAffirmativeContactDirectReply(lang, opts = {}) {
  ...
  if (key === "ru") {
    return "Отлично! Чтобы оформить запись, поделитесь, пожалуйста, номером телефона или WhatsApp, по которому с вами можно связаться.";
  }
  if (key === "ka") {
    return "შესანიშნავია! ჩაწერისთვის გთხოვთ, გაგვიზიაროთ ტელეფონის ან WhatsApp ნომერი, სადაც შეგვიძლია დაგიკავშირდეთ.";
  }
  ...
  if (patientHasActiveBooking(flags, state)) {
    ...
    if (state.stage === "awaiting_slot_confirm") {
      ...
      return (
        ...
        isBookingConfirmationYes(msg, { recentTurns: turns, pendingConfirmation: true }) ||
    ...
    if (state.stage === "slots_offered" && state.offeredSlots?.length) {
      ...
      if (isBookingConfirmationYes(msg, { recentTurns: turns, pendingConfirmation: true }) || isBookingConfirmationNo(msg)) return true;
      if (patientMessageAdvancesSlotBooking(msg, state, scheduling)) return true;
      return false;
    }
    if (isBookingConfirmationYes(msg, { recentTurns: turns, pendingConfirmation: true }) || isBookingConfirmationNo(msg)) {
...
function patientMessageIsAlternativeSlotRequest(message, scheduling) {
  ...
  if (/\b(\d+\.\s*hafta|\d+\s*hafta|hafta\s*\d+)\b/i.test(msg)) return true;
  if (/\brandevu\s*(ver|al|istiyorum|istiyoruz|yap)/i.test(msg) && msg.length >= 10) return true;
  return false;
}
...
/**
 * Patient asks for options again, a new scheduling window, or a vague month/week — not a list index.
 * @param {string} message
 */
function patientRequestsNewSchedulingWindow(message) {
  if (patientRequestsSlotListResend(message)) return true;
  const msg = String(message || "").trim();
  ...
  if (/\brandevu\s*(ver|al|istiyorum|istiyoruz|yap)/i.test(msg) && msg.length >= 10) return true;
  if (
    /\b(seçenek|secenek|müsait|musait|uygun)\b/i.test(msg) &&
    !/^[1-9]$|^1[0-2]$/.test(msg)
  ) {
    return true;
  }
...
/**
 * @param {string} lang
 * @param {ReturnType<typeof assessSchedulingRequestTiming>} timing
 * @param {{ timezone?: string }} [scheduling]
 */
function buildSchedulingContextIntro(lang, timing, scheduling) {
  const key = String(lang || "tr").slice(0, 2).toLowerCase();
  if (!timing?.inPast) return null;
  const tz = scheduling?.timezone || "Europe/Istanbul";
  const todayLabel = formatInTimeZone(new Date(), tz, "d MMMM yyyy");

  if (key === "tr") {
    if (timing.reason === "month_week_in_past" && timing.monthWeek?.month) {
      const monthLabel = TR_MONTH_LABELS[timing.monthWeek.month] || "ay";
      return `${monthLabel} ayının ${timing.monthWeek.week}. haftası geçmiş sayılır (bugün ${todayLabel}). Size yakın gelecekteki müsait saatleri paylaşıyorum:`;
    }
    if (timing.reason === "month_week_in_past") {
      return `Belirttiğiniz hafta geçmişte kaldı (bugün ${todayLabel}). Size yakın gelecekteki müsait saatleri paylaşıyorum:`;
    }
    if (timing.reason === "date_in_past") {
      return `Belirttiğiniz tarih geçmişte kaldı (bugün ${todayLabel}). Size güncel müsait saatleri paylaşıyorum:`;
    }
  }
  if (key === "ru") {
    return `Указанный период уже прошёл. Ниже актуальное свободное время:`;
  }
  return `That time period is in the past. Here are upcoming available times:`;
}
 ...
 */
function buildPreferredDaySlotIntro(lang, dayLabel) {
  const key = String(lang || "tr").slice(0, 2).toLowerCase();
  const day = String(dayLabel || "").trim();
  if (!day) return undefined;
  if (key === "tr") return `${day} için müsait saatler:`;
  if (key === "ru") return `Свободное время на ${day}:`;
  if (key === "ka") return `${day}-ის თავისუფალი დროები:`;
  return `Available times for ${day}:`;
}
...
/**
 * Resend stored slots or fetch fresh ones when the patient asks again / names a past or vague window.
 * @param {object} params
 */
async function prepareOrganizedSlotRefreshTurn(params) {
  const {
    message,
    state,
    scheduling,
  ...
  const msg = String(message || "").trim();
  if (!patientRequestsSlotListResend(msg) && !patientRequestsNewSchedulingWindow(msg)) return null;

  const resendTurn = buildSelectSlotListResendTurn(params);
  if (resendTurn?.directReply) return resendTurn;

  const tz = scheduling?.timezone || "Europe/Istanbul";
  const timing = assessSchedulingRequestTiming(msg, tz);
  const lang = String(locale || "tr").slice(0, 2);
  let slotSearchDate = preferredDateYmd;
  if (timing.inPast) slotSearchDate = null;
  else if (timing.requestedYmd && !timing.inPast) slotSearchDate = timing.requestedYmd;
  ...
  const slots = await findAvailableSlots({
    ...
    treatmentLabel,
    locale,
    preferredDateYmd: slotSearchDate,
    preferredTimeMin: null,
  });
  ...
  if (!slots.length) {
    return {
      ...
      directReply:
        lang === "tr"
          ? timing.inPast
            ? "Belirttiğiniz dönem geçmişte kaldı. Şu an yakın tarihlerde müsait saat bulamadım — farklı bir gün veya saat yazar mısınız?"
            : "Şu an müsait saat bulamadım — farklı bir gün veya saat yazar mısınız?"
          : timing.inPast
            ? "That period is in the past and I could not find nearby times — please suggest another day or time."
            : "I could not find available times right now — please suggest another day or time.",
      slotsOffered: 0,
    };
  }

  const intro =
    buildSchedulingContextIntro(lang, timing, scheduling) ||
    (patientRequestsSlotListResend(msg)
      ? lang === "tr"
        ? "Tabii — güncel müsait tarih ve saatler:"
        : "Sure — here are the current available times:"
      : lang === "tr"
        ? "Size uygun olabilecek güncel müsait saatler:"
        : "Here are current times that may work for you:");
  ...
  if (profileId) {
    await persistAiBookingFlags(profileId, {
      ...
      awaitingAction: BOOKING_PENDING_ACTIONS.SELECT_SLOT,
      bookingActive: true,
      preferredDateYmd: slotSearchDate || null,
    });
    logBookingAuditEvent(BOOKING_AUDIT_EVENTS.BOOKING_STARTED, {
      ...
      refreshReason: timing.inPast ? "past_window" : "slot_list_refresh",
    });
  }
...
async function findAvailableSlots(params) {
  ...
  const filtered = filterSlotsWithinWorkingHours(slots, scheduling);
  const preferredYmd = params.preferredDateYmd ? String(params.preferredDateYmd).slice(0, 10) : "";
  if (preferredYmd) {
    const onPreferredDay = filtered.filter((s) => String(s.dateYmd || "").slice(0, 10) === preferredYmd);
    if (onPreferredDay.length) return onPreferredDay.slice(0, maxSlots);
  }
  return filtered.slice(0, maxSlots);
}
...
async function tryEnforceBookingIdentityGate(params) {
  ...
  const lang = resolveBookingReplyLang(locale, message);
  const nameState = await resolvePatientRecordName({
    profileRow,
    patientMessage: message,
    recentTurns,
  ...
  const dateToPersist =
    preferredDateYmd ||
    state?.preferredDateYmd ||
    readDurableBookingState(flags || {}).preferredDateYmd ||
    readDurableBookingState(flags || {}).selectedDate ||
    null;
  if (dateToPersist) patch.preferredDateYmd = String(dateToPersist);
  if (preferredTimeMin != null) {
    patch.preferredTimeMin = Number(preferredTimeMin);
  } else if (state?.preferredTimeMin != null) {
    patch.preferredTimeMin = state.preferredTimeMin;
  }
...
async function prepareAiAppointmentBookingTurn(params) {
  ...
  const profileId = String(profileRow.id || "").trim();
  const message = String(params.patientMessage || "");
  const locale = resolveBookingReplyLang(
    String(params.locale || profileRow.conversation_primary_language || "en").slice(0, 5),
    message,
  );
  ...
  let preferredDateYmd = resolveEffectivePreferredDateYmd({
    message,
    timezone: scheduling.timezone,
    recentTurns,
    state,
  ...
  if (
    selectSlotPending &&
    !selected &&
    (patientRequestsSlotListResend(message) || patientRequestsNewSchedulingWindow(message))
  ) {
    const refreshTurn = await prepareOrganizedSlotRefreshTurn({
      message,
      state,
      scheduling,
      locale,
    ...
    if (refreshTurn?.directReply) return refreshTurn;
  }
  if (
    selectSlotPending &&
    !selected &&
  ...
  return {
    ...
    directReply: buildSlotOfferDirectReply(slots, lang, {
      intro:
        preferredDateYmd && slots.length
          ? buildPreferredDaySlotIntro(
              lang,
              formatBookingWhenLabel(`${preferredDateYmd}T12:00:00`, locale, scheduling.timezone).split(
                " at ",
              )[0] || preferredDateYmd,
            )
          : undefined,
...
module.exports = {
  ...
  prepareOrganizedSlotRefreshTurn,
