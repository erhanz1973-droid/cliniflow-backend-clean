...
function shouldEngageAppointmentBooking(message, leadData, flags, profileRow, recentTurns = []) {
  ...
  if (patientHasActiveBooking(flags, state)) {
    ...
    if (state.stage === "slots_offered" && state.offeredSlots?.length) return true;
...
/** Generic select-slot nudge — used for consecutive-prompt loop detection. */
const BOOKING_SELECT_SLOT_NUDGE_RE =
  /\b(paylaştığımız saatlerden|saatlerden birini seçin|saatlerden birini seçmek|1[–-]\d+\s*arası numara|pick a (time|number) from|reply with a number)\b/i;
...
async function prepareSelectSlotAlternativeRequest(params) {
  ...
  const whenLabel =
    ...
    (pendingProbe?.startAt
      ? formatAppointmentDisplay(pendingProbe.startAt, locale, tz)
      : dateYmd && timeMin != null
  ...
  const dateDisplay =
    dateYmd && pendingProbe?.startAt
      ? formatAppointmentDisplay(`${dateYmd}T12:00:00`, locale, tz).split(" at ")[0] ||
        formatAppointmentDisplay(pendingProbe.startAt, locale, tz).split(" ")[0]
      : whenLabel;
  ...
  if (dateYmd && timeMin != null) {
    ...
    if (exact?.startAt) {
      ...
      const whenConfirm =
        exact.label || formatAppointmentDisplay(exact.startAt, locale, tz);
      return {
        engaged: true,
        promptBlock: "",
...
async function prepareAiAppointmentBookingTurn(params) {
  ...
  const statusBookingRecord =
    ...
    (flags.activeAppointment?.startAt
      ? {
          label: flags.activeAppointment.label,
          startAt: flags.activeAppointment.startAt,
          status: flags.activeAppointment.status || "scheduled",
        }
      : null);
  ...
  if (state.stage === "awaiting_slot_confirm") {
    ...
    if (hasStaleBookingProposalVsActiveAppointment(flags)) {
      ...
      const whenActive =
        appt?.label ||
        (appt?.startAt
          ? formatAppointmentDisplay(appt.startAt, locale, scheduling.timezone)
          : "");
      return {
        engaged: true,
        promptBlock: "",
    ...
    if (availDateYmd && availTimeMin != null) {
      ...
      const whenLabel =
        pendingProbe?.startAt != null
          ? formatAppointmentDisplay(pendingProbe.startAt, locale, scheduling.timezone)
      ...
      if (askedDateYmd && (!dateMatches || !timeMatches)) {
        const askedLabel =
          askedDateYmd && askedTimeHm
            ? `${formatAppointmentDisplay(`${askedDateYmd}T12:00:00`, locale, scheduling.timezone).split(" at ")[0] || askedDateYmd} ${askedTimeHm}`
            : askedDateYmd;
        return {
          engaged: true,
          promptBlock: "",
  ...
  if (
    ...
    if (!isBookingConfirmationYes(message, { recentTurns, pendingConfirmation: true })) {
      const langNo = locale.slice(0, 2);
      return {
        engaged: true,
        promptBlock: "",
        directReply: buildSlotConfirmationReminder(langNo, whenConfirm),
        awaitingConfirmation: true,
      };
    }
