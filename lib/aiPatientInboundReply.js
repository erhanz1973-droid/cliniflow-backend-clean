...
async function runAiReplyForClinicInbound(params) {
  ...
  if (simpleDirectQuestion && detectPatientCommercialIntent(message, priorLeadData || {}).asksPrice) {
  ...
  if (conversationalShort && !commercialIntent.asksPrice) {
    pricingSalesContext = "";
  }
  ...
  if (UUID_RE.test(clinicId) && !bookingActiveLock) {
    try {
