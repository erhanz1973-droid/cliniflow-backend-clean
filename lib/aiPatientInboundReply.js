...
async function runAiReplyForClinicInbound(params) {
  ...
  if (conversationalShort && discussionMemory.pricingAlreadyDiscussed) {
    pricingSalesContext = "";
  }
  ...
  if (greetingOnly && !bookingActiveLock) {
    ...
    conversationalShort &&
    !bookingActiveLock &&
    discussionMemory.pricingAlreadyDiscussed &&
    !bookingPrep.engaged
  ) {
    ...
    console.log("[repeat_prevented]", {
      reason: "early_conversational_short_after_pricing",
    ...
    const lang = String(languageState.conversationLanguage || "tr").slice(0, 2);
    const max = Math.min(durableBookingAfterPrep.offeredSlots.length, 12);
    const altSlotRequest = patientMessageIsAlternativeSlotRequest(message, {
      timezone: "Europe/Istanbul",
    });
    replyText =
      bookingPrep.directReply ||
    ...
    turnLeadData = priorLeadData;
    workflowSelected = altSlotRequest ? "booking_alternative_slot" : "booking_lock";
    routerReason = altSlotRequest ? "select_slot_datetime_request" : bookingRouter.reason || "booking_select_slot";
  ...
  if (
    replyText &&
    bookingActiveLock &&
    detectConsecutiveBookingPromptLoop(recentTurns, replyText, message, {
      timezone: "Europe/Istanbul",
    })
  ) {
    logBookingLoopDetected({
    ...
    if (
      bookingPrep.directReply &&
      !detectConsecutiveBookingPromptLoop(recentTurns, bookingPrep.directReply, message, {
        timezone: "Europe/Istanbul",
      })
    ) {
      replyText = bookingPrep.directReply;
    } else if (patientMessageIsAlternativeSlotRequest(message, { timezone: "Europe/Istanbul" })) {
      const lang = String(languageState.conversationLanguage || "tr").slice(0, 2);
