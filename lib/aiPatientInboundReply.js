...
const { runCliniflySalesInboundReply } = require("./cliniflySalesAi");
...
} = require("./clinicSalesPromptForAi");
const {
...
} = require("./internationalPatientCoordinator");
...
async function hasAiOutboundSincePatient(profileId, lastPatientAt) {
  const { data, error } = await supabase
    ...
    .in("event_type", ["ai_reply", "clinifly_sales_reply", "continuity_fallback"])
    .limit(3);
...
async function runAiReplyForClinicInbound(params) {
  ...
  const pageAiMode = normalizePageAiMode(params.pageAiMode);
  const inboundSourceEarly = String(params.source || "").trim();
  const salesMessengerPage =
    pageAiMode === PAGE_AI_MODE.CLINIFLY_SALES &&
    (inboundSourceEarly === "messenger" ||
      resolveInboundCoordinatorChannel(params.source, params.channel) === "messenger");
  if (salesMessengerPage) {
    return runCliniflySalesInboundReply({
      patientId,
      clinicId,
      patientMessage: message,
      latencyTraceId: params.latencyTraceId,
      inboundPatientMessageAt: params.inboundPatientMessageAt,
  ...
  const clinicalUrgencyAck = detectClinicalUrgency(message);
  const allowUrgencyPatientAck = clinicalUrgencyAck && delegation.aiEscalationRequired;

  if (delegation.aiEscalationRequired && !allowUrgencyPatientAck) {
  ...
  if (!delegation.autoReplyAllowed && !allowUrgencyPatientAck) {
  ...
  const internationalCoordinator = analyzeInternationalCoordinatorTurn({
    message,
    leadData: priorLeadData,
    profileRow,
    travelContextDetected: patientContextStrategy.travel_context_detected,
  });
  const internationalCoordinatorContext = internationalCoordinator.promptBlock;
  ...
  const earlyCommercialIntent = detectPatientCommercialIntent(message, priorLeadData || {});
  if (
    simpleDirectQuestion &&
    (earlyCommercialIntent.asksDirectPrice || earlyCommercialIntent.asksVariantComparison)
  ) {
  ...
  if (catchUpNeeded && catchUpSince) {
    ...
    if (process.env.NODE_ENV !== "production" && snoozeCatchUpPrompt) {
  ...
  const coordinatorAskedName = coordinatorRecentlyAskedForName(recentTurns);
  const extractedName =
    extractPatientNameFromMessage(message, {
      coordinatorAskedName,
    }) || (coordinatorAskedName && looksLikeStandaloneNameLine(message)
      ? normalizePatientDisplayName(message)
      : null);
  if (extractedName) {
    await syncPatientNameColumn(patientId, extractedName, { source: params.source || "inbound" });
    if (profileRow?.id) {
      void markNameCollectionCollected(profileRow.id, extractedName).catch(() => {});
    }
  }
  ...
  const hasBookedAppointment = hasActiveScheduledAppointment(persistedFlags);
  const repetitionSuppressionPrompt = buildRepetitionSuppressionPromptBlock(
    discussionMemory,
    { patientMessage: message, recentTurns, hasActiveAppointment: hasBookedAppointment },
  );
  ...
  if (
    ...
    !commercialIntent.asksVariantComparison
  ) {
    pricingSalesContext = "";
  }
  ...
  const isSalesMessengerPage =
    pageAiMode === PAGE_AI_MODE.CLINIFLY_SALES && inboundChannel === "messenger";
  ...
  let knownPatientName =
    (await loadPatientKnownDisplayName(patientId)) ||
    normalizePatientDisplayName(extractedName) ||
    null;
  const nameEval = evaluateNameCollectionCandidate(profileRow, persistedFlags, inboundChannel, {
    knownName: knownPatientName,
    patientMessage: message,
    recentTurns,
  });
  let nameCollectionContext = "";
  if (knownPatientName && coordinatorAskedName && looksLikeStandaloneNameLine(message)) {
    nameCollectionContext = buildNameAcknowledgmentPromptBlock(
      knownPatientName,
      languageState.conversationLanguage,
    );
  } else if (nameEval.candidate) {
    nameCollectionContext = buildNameCollectionPromptBlock(
      nameEval,
      contextMode,
      languageState.conversationLanguage,
    );
  ...
  const appDownloadQuestion =
    !isTreatmentGuide && !isSalesMessengerPage && patientAskedAboutAppRegistration(message);
  const cliniflyPricingQuestion =
    !isTreatmentGuide && !isSalesMessengerPage && patientAskedAboutCliniflyPricing(message);
  const hasAppDownloadIntent =
    !isSalesMessengerPage &&
  ...
  if (greetingOnly) {
    replyText = buildGreetingDirectReply(message, languageState.conversationLanguage, {
      bookingContinuity: bookingActiveLock || bookingPrep.engaged,
    });
    if (nameEval.candidate) {
      replyText = appendNameAskSuffixToReply(replyText, languageState.conversationLanguage);
    }
    turnLeadData = priorLeadData;
    workflowSelected = "greeting";
    routerReason =
      bookingActiveLock || bookingPrep.engaged ? "greeting_only_booking" : "greeting_only";
    ...
    patientAskedDurationOnly(message, priorLeadData) &&
    !bookingActiveLock &&
    !bookingPrep.engaged &&
    UUID_RE.test(clinicId)
  ) {
    try {
      const durationReply = await buildDurationEstimateDirectReply(clinicId, {
        message,
        leadData: priorLeadData,
        lang: languageState.conversationLanguage,
      });
      if (durationReply) {
        replyText = durationReply;
        turnLeadData = priorLeadData;
        workflowSelected = "duration_estimate_direct";
        routerReason = "kac_dakika_settings";
      }
    } catch (durErr) {
      console.warn("[aiPatientInboundReply] duration direct:", durErr?.message || durErr);
    }
  }
  const patientTravelIntent = classifyPatientTravelIntent(message);

  if (!replyText && allowUrgencyPatientAck) {
    replyText = buildClinicalUrgencyDirectReply(languageState.conversationLanguage);
    turnLeadData = priorLeadData;
    workflowSelected = "clinical_urgency_direct";
    routerReason = "clinical_urgency_ack";
  }
  ...
  if (
    ...
    if (UUID_RE.test(clinicId)) {
      try {
        travelHotels = await getTopHotelsForAi(clinicId, 3);
      } catch (_) {
        travelHotels = [];
      }
    }
    replyText = buildTravelCoordinationDirectReply(
      patientTravelIntent,
      languageState.conversationLanguage,
      { hotels: travelHotels, message },
    );
    turnLeadData = priorLeadData
      ? {
          ...priorLeadData,
          travelTimeline: priorLeadData.travelTimeline || "international_inquiry",
        }
      : { travelTimeline: "international_inquiry" };
    workflowSelected = "travel_coordination_direct";
    routerReason = `travel_intent_${patientTravelIntent}`;
  }
  if (
    !replyText &&
    internationalCoordinator.stayDurationQuestion &&
    !bookingActiveLock &&
    !bookingPrep.engaged &&
    !isTreatmentGuide
  ) {
    replyText = buildStayDurationDirectReply(
      languageState.conversationLanguage,
      priorLeadData,
      message,
    );
    turnLeadData = priorLeadData;
    workflowSelected = "stay_duration_direct";
    routerReason = "stay_duration_question";
  }
  if (
    !replyText &&
    internationalCoordinator.postOpQuestion &&
    !internationalCoordinator.clinicalUrgency &&
    !bookingActiveLock &&
    !bookingPrep.engaged &&
    !isTreatmentGuide
  ) {
    replyText = buildPostOpCoordinationDirectReply(languageState.conversationLanguage);
    turnLeadData = priorLeadData;
    workflowSelected = "post_op_coordination_direct";
    routerReason = "post_op_question";
  }
  if (
    !replyText &&
    isOrthodonticTreatmentIntent(message, priorLeadData) &&
    !patientMessageIsTravelCoordination(message) &&
    !bookingActiveLock &&
    !bookingPrep.engaged
  ) {
    const clinicCodeOrtho = UUID_RE.test(clinicId)
      ? await fetchClinicCodeByClinicId(clinicId)
      : null;
    replyText = buildOrthodonticPhotoIntakeDirectReply(
      languageState.conversationLanguage,
      clinicCodeOrtho,
    );
    turnLeadData = priorLeadData
      ? { ...priorLeadData, treatmentInterest: "orthodontics" }
      : { treatmentInterest: "orthodontics" };
    workflowSelected = "orthodontic_photo_intake_direct";
    routerReason = "orthodontic_treatment_intent";
  }
  if (
    !replyText &&
    patientAskedDoctorAssignment(message) &&
    !bookingActiveLock &&
    !bookingPrep.engaged &&
    UUID_RE.test(clinicId)
  ) {
    try {
      ...
      replyText = buildDoctorAssignmentReply(profiles, {
        lang: languageState.conversationLanguage,
        clinicName: clinicContext || clinicName,
        message,
      });
      replyText = sanitizeDoctorNamesInReply(replyText, profiles);
    } catch (doctorAssignErr) {
      console.warn("[doctorAssignment] direct reply:", doctorAssignErr?.message || doctorAssignErr);
      replyText = buildDoctorAssignmentReply([], {
        lang: languageState.conversationLanguage,
        clinicName: clinicContext,
        message,
      });
    ...
    turnLeadData = priorLeadData;
    workflowSelected = "doctor_assignment_direct";
    routerReason = "doktorum_kim";
  } else if (
    hasBookedAppointment &&
    (conversationalShort || isSocialAcknowledgmentMessage(message)) &&
    !patientAsksScheduling &&
    !bookingActiveLock &&
    !bookingPrep.engaged
  ) {
    replyText = buildPostBookingPatientAckReply(
      languageState.conversationLanguage,
      persistedFlags,
    );
    turnLeadData = priorLeadData;
    workflowSelected = "post_booking_ack_direct";
    routerReason = "thanks_after_confirmed_appointment";
  } else if (
    conversationalShort &&
    ...
    replyText = buildConversationForwardReply({
      ...
      discussionMemory,
      recentTurns,
      hasActiveAppointment: hasBookedAppointment,
      operationalFlags: persistedFlags,
    });
  ...
  if (!isTreatmentGuide && !isSalesMessengerPage) {
  ...
  let orthodonticIntakePrompt = "";
  if (
    !replyText &&
    !patientTravelIntent &&
    !patientMessageIsTravelCoordination(message) &&
    (treatmentTopic?.slug === "orthodontics" ||
      isOrthodonticTreatmentIntent(message, priorLeadData))
  ) {
    const clinicCodeOrthoPrompt = UUID_RE.test(clinicId)
      ? await fetchClinicCodeByClinicId(clinicId)
      : null;
    orthodonticIntakePrompt = buildOrthodonticPhotoIntakePromptBlock({
      lang: languageState.conversationLanguage,
      clinicCode: clinicCodeOrthoPrompt,
    });
  }
  ...
  try {
    ...
    const result = await coordinatorChatReply({
      ...
      whatsappCollectionContext,
      nameCollectionContext,
      conversionStrategyContext: bookingActiveLock ? "" : conversionStrategy.promptBlock,
      patientContextStrategyPrompt,
      internationalCoordinatorContext: bookingActiveLock ? "" : internationalCoordinatorContext,
    ...
    if (UUID_RE.test(clinicId) && patientAskedDoctorAssignment(message)) {
      try {
        const { profiles } = await loadClinicDoctorProfilesForAi(clinicId, {
          lang: languageState.conversationLanguage,
        });
        replyText = sanitizeDoctorNamesInReply(replyText, profiles);
      } catch {
    ...
    replyText = enforcePatientReplyLanguage(replyText, {
      ...
      lockedConversationLanguage:
        languageState.conversationPrimaryLanguage ||
        readConversationLanguageFromProfile(profileRow),
      patientMessage: message,
      logLabel: "aiPatientInboundReply",
    });
    }
  ...
  replyText = enforcePatientReplyLanguage(replyText, {
    ...
    lockedConversationLanguage:
      ...
      readConversationLanguageFromProfile(profileRow),
    patientMessage: message,
    logLabel: "aiPatientInboundReply_booking_direct",
  });
  replyText = coercePatientFacingReply(replyText, {
    lang: languageState.conversationLanguage,
    patientMessage: message,
    logLabel: "aiPatientInboundReply",
  });
  ...
  if (nameEval.candidate) {
    void markNamePromptOffered(profileRow.id).catch(() => {});
  }
