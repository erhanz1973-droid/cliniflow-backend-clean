...
async function runAiReplyForClinicInbound(params) {
  ...
  const bookingPrep =
    !isTreatmentGuide
      ? await prepareAiAppointmentBookingTurn({
          clinicId,
          patientId,
          profileRow,
          patientMessage: message,
      ...
      : { engaged: false, promptBlock: "", directReply: null };
  ...
  if (knownWhatsapp && (phoneOnlyTurn || sharesWhatsappNumber)) {
    replyText = formatPhoneAcknowledgmentReply(knownWhatsapp, phoneAckLang);
  ...
  const documentIntakeWithBooking = bookingPromptBlock
    ? `${bookingPromptBlock}\n\n${documentIntakeContext}`
    : documentIntakeContext;
  ...
  try {
    ...
    replyText = repairPhoneNumberTurnReply(replyText, message, {
      lang: phoneAckLang,
      languageState,
      profileRow,
      recentTurns,
      whatsappNumber: knownWhatsapp,
    });
