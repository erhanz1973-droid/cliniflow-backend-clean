...
async function runAiReplyForClinicInbound(params) {
  ...
  if (leadPipeline.profileId) {
    ...
    await supabase
      .from("ai_coordinator_lead_profiles")
      .update({
        operational_intake_flags: repliedFlags,
  ...
  logBookingAudit({
    ...
    pendingAction: durableBooking.awaitingAction || durableBooking.pending_action || null,
    ...
    bookingStage: durableBooking.stage || null,
    detectedIntent: bookingActiveLock ? "booking_lock" : greetingOnly ? "greeting" : "coordinator",
    bookingPrep: {
