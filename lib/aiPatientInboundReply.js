...
async function runAiReplyForClinicInbound(params) {
  ...
  const bookingActiveLock = isBookingFlowInProgress(persistedFlags);
  ...
  const abFlags =
    persistedFlags?.aiBooking && typeof persistedFlags.aiBooking === "object"
      ...
      : {};
  const offeredSlotCount = Array.isArray(durableBooking.offeredSlots)
    ? durableBooking.offeredSlots.length
  ...
  const bookingOwnsConfirmYes =
    ...
    (resolvesPendingConfirmation(message, durableBooking) ||
  ...
  const conversionStrategy = bookingActiveLock
  ...
  if (greetingOnly && !bookingActiveLock) {
    ...
    replyText = bookingPrep.directReply;
    turnLeadData = priorLeadData;
  } else if (bookingActiveLock && bookingPrep.awaitingConfirmation) {
    ...
    bookingActiveLock &&
    durableBooking.awaitingAction === BOOKING_PENDING_ACTIONS.SELECT_SLOT &&
    durableBooking.offeredSlots.length
  ) {
    const lang = String(languageState.conversationLanguage || "tr").slice(0, 2);
    const max = Math.min(durableBooking.offeredSlots.length, 12);
    replyText =
      bookingPrep.directReply ||
      (lang === "tr"
  ...
  if (!isTreatmentGuide && !bookingActiveLock && UUID_RE.test(clinicId)) {
    try {
  ...
  const documentIntakeWithBooking = bookingActiveLock
  ...
  try {
    if (replyText) {
      ...
      replyText =
        bookingPrep.directReply ||
        (String(languageState.conversationLanguage || "tr").slice(0, 2) === "tr"
  ...
  logBookingAudit({
    ...
    bookingState: {
      bookingActive: durableBooking.bookingActive,
      stage: durableBooking.stage || null,
      bookingId: durableBooking.bookingId ? String(durableBooking.bookingId).slice(0, 8) : null,
    },
    pendingAction:
      durableBooking.pendingAction ||
      durableBooking.awaitingAction ||
      durableBooking.pending_action ||
      null,
    selectedDate: durableBooking.selectedDate || null,
    selectedSlot: durableBooking.selectedSlot?.startAt || null,
    slotListId: durableBooking.slotListId || null,
    intent: bookingActiveLock ? "booking_lock" : greetingOnly ? "greeting" : "coordinator",
    toolCalls: [],
