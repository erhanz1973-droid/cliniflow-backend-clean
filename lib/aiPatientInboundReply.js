...
async function runAiReplyForClinicInbound(params) {
  ...
  const bookingPrep = await prepareAiAppointmentBookingTurn({
    clinicId,
    patientId,
    profileRow,
    patientMessage: message,
  ...
  const bookingOwnsShortTime =
    ...
    (bookingPrep.directReply || bookingPrep.promptBlock || bookingPrep.needContact);
  const workflowDirectReply =
    workflowEval.directReply && !(bookingOwnsShortTime && workflowEval.parsedIntent === "appointment_time_answer")
  ...
  if (phoneForAck && (phoneOnlyTurn || sharesWhatsappNumber)) {
  ...
  const documentIntakeWithBooking = [workflowPromptBlock, bookingPromptBlock, documentIntakeContext]
    .filter(Boolean)
    .join("\n\n");
