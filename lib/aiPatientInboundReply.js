...
async function runAiReplyForClinicInbound(params) {
  ...
  let bookingActiveLock = isBookingFlowInProgress(persistedFlags);
  ...
  bookingActiveLock = isBookingFlowInProgress(persistedFlags);
  const bookingStatusLock =
  ...
  if (!isTreatmentGuide && !bookingActiveLock && !bookingStatusLock && UUID_RE.test(clinicId)) {
    try {
