 ...
 */
function leadDataToDbColumns(lead, opts = {}) {
  const l = lead && typeof lead === "object" ? lead : emptyLeadData();
  const conv =
    opts.conversationLanguage && String(opts.conversationLanguage).trim().slice(0, 2)
      ? String(opts.conversationLanguage).trim().toLowerCase().slice(0, 2)
      : null;
  ...
  if (conv) {
    cols.preferred_language = conv;
    cols.conversation_primary_language = conv;
  }
  return cols;
}
...
async function persistAiCoordinatorLead(params) {
  ...
  try {
    const { data: existing, error: loadErr } = await supabase
      ...
      .select(
        "id, message_count, patient_id, clinic_id, coordination_mode, ai_mode, ai_paused, ai_escalation_required, escalation_flags, operational_intake_flags, conversation_primary_language, preferred_language",
      )
    ...
    const escalation = softenEscalationForCommercialIntent(
      detectEscalationSignals(patientMessage),
      patientMessage || "",
    );
    ...
    const convLang = params.conversationLanguage
      ? String(params.conversationLanguage).trim().toLowerCase().slice(0, 2)
      : null;
    const prevIntake =
      existing?.operational_intake_flags && typeof existing.operational_intake_flags === "object"
        ? existing.operational_intake_flags
        : {};
    const intakeFlags =
      convLang
        ? {
            ...prevIntake,
            conversationPrimaryLanguage: convLang,
          }
        : prevIntake;

    const aiReply = String(params.aiReply || "").trim().slice(0, 8000) || null;
    /** Patient timestamps are set on inbound touch — do not overwrite with AI completion time. */
    const recordPatientTimestampsInProfile = Boolean(patientMessage) && !aiReply;

    const row = normalizeProfileRowChannels({
      session_id: sessionId,
      patient_id: patientId || existing?.patient_id || null,
      clinic_id: clinicId || existing?.clinic_id || null,
      ...cols,
      ...(Object.keys(intakeFlags).length ? { operational_intake_flags: intakeFlags } : {}),
      conversation_summary: summary || null,
      ...(recordPatientTimestampsInProfile
        ? {
      ...
      primary_channel: params.channel || "in_app",
    ...
    if (profileId) {
      let { error: updErr } = await supabase
        ...
        .update(row)
        .eq("id", profileId);
      if (updErr && isChannelCheckConstraintError(updErr)) {
        const op = normalizeCoordinatorChannel(params.channel, "in_app");
        updErr = (
          await supabase
            .from("ai_coordinator_lead_profiles")
            .update({
      ...
      if (updErr && String(updErr.message || "").includes("conversation_primary_language")) {
        const { conversation_primary_language: _drop, ...rowNoConv } = row;
        updErr = (await supabase.from("ai_coordinator_lead_profiles").update(rowNoConv).eq("id", profileId))
          .error;
      }
      if (updErr) {
      ...
      let { data: inserted, error: insErr } = await supabase
        ...
        .select("id")
        .single();
      if (insErr && isChannelCheckConstraintError(insErr)) {
        const op = normalizeCoordinatorChannel(params.channel, "in_app");
      ...
      if (insErr && String(insErr.message || "").includes("conversation_primary_language")) {
        const { conversation_primary_language: _drop, ...rowNoConv } = row;
        const retry = await supabase
          .from("ai_coordinator_lead_profiles")
          .insert(rowNoConv)
    ...
    if (profileId) {
      ...
      const { error: evErr } = await insertLeadEventWithChannel({
      ...
      if (escalation.any && !wasEscalated) {
        const { error: escErr } = await insertLeadEventWithChannel({
          profile_id: profileId,
        ...
        if (escErr) {
          console.warn("[aiLeadPipeline] escalation event:", escErr.message);
        }
      }
      ...
      if (patientMessage || aiReply) {
        ...
        const { error: chErr } = await insertChannelMessagesWithChannel(rows);
        if (chErr) {
