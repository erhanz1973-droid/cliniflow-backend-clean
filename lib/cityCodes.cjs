...
/**
 * Prefer city_code column; fallback legacy city text; expose pending_city_raw.
 */
function clinicCityPayloadFromRow(row) {
  ...
  const pend = row.pending_city_raw != null ? String(row.pending_city_raw).trim() : "";
  if (pend) {
    return { city: null, city_code: null, pending_city_raw: pend };
  }
