...
const FALLBACK_DELAY_DEFAULT_SEC = 30;
...
function normalizeAiRepliesConfig(communicationPolicy) {
  ...
  const envInstant = parseInt(process.env.AI_INBOUND_REPLY_DELAY_MS || "400", 10);
  const envOmniInstant = parseInt(process.env.AI_OMNICHANNEL_INSTANT_DELAY_MS || "", 10);
  const envFirstInstant = parseInt(process.env.AI_FIRST_INBOUND_REPLY_DELAY_MS || "200", 10);
  ...
  return {
    ...
    omnichannelInstantDelayMs: Math.max(
      ...
      Number(ai.omnichannelInstantDelayMs) > 0
        ...
        : Number.isFinite(envOmniInstant) && envOmniInstant >= 0
          ? envOmniInstant
          : DEFAULT_MESSAGE_BUFFER_MS,
    ),
    messageBufferPreset:
      ai.messageBufferPreset != null
        ? String(ai.messageBufferPreset)
        : ai.messageBufferDelay != null
          ? String(ai.messageBufferDelay)
          : "5s",
    messageBufferDelayMs: resolveMessageBufferDelayMs(ai),
...
async function resolveInboundAiOrchestration(params) {
  ...
  const messageBufferDelayMs = resolveMessageBufferDelayMs(cfg);
  if (omnichannel) {
    instantDelayMs = messageBufferDelayMs;
