...
const { isBookingFlowInProgress } = require("./aiBookingState");
const { normalizePageAiMode, PAGE_AI_MODE, isCliniflySalesLeadProfile } = require("./pageAiMode");
const { runCliniflySalesInboundReply } = require("./cliniflySalesAi");
const {
  mergeBufferedInboundPayload,
  DEFAULT_MESSAGE_BUFFER_MS,
} = require("./aiMessageBuffer");
...
function scheduleCoalescedInstantAiReply(key, payload, delayMs, runFn) {
  const prev = pendingInstantAiReplies.get(key);
  const nextMsg = String(payload.patientMessage || "").trim();
  const prevMsg = String(prev?.payload?.patientMessage || "").trim();
  if (prev?.payload && patientMessagesNearDuplicate(prevMsg, nextMsg)) {
    return;
  }
  if (prev?.timer) clearTimeout(prev.timer);

  const merged = mergeBufferedInboundPayload(prev?.payload || null, payload);
...
async function runDoctorSilenceAiFallback(params) {
  ...
  if (
    ...
    (params.inboundChannel === "messenger" || params.source === "messenger")
  ) {
    return runCliniflySalesInboundReply({
      patientId,
      clinicId,
      patientMessage,
      inboundPatientMessageAt: scheduledForPatientAt,
    });
  }
  ...
  const { data: row, error } = await supabase
    ...
    .select(
      "id, patient_id, clinic_id, conversation_type, source, channel_metadata, coordination_mode, ai_mode, ai_paused, ai_escalation_required, escalation_flags, operational_intake_flags, last_patient_message, last_patient_message_at, last_channel_message_at, last_human_reply_at, last_ai_reply_at",
    )
  ...
  if (isCliniflySalesLeadProfile(row)) {
    const msg =
      String(row.last_patient_message || "").trim() || patientMessage;
    const inboundChannel = params.inboundChannel || "messenger";
    if (inboundChannel === "messenger" || params.source === "messenger") {
      const lastAt = String(
      ...
      return runCliniflySalesInboundReply({
        ...
        clinicId,
        patientMessage: msg,
        inboundPatientMessageAt: scheduledForPatientAt || lastAt,
      });
    }
    console.log("[aiSlaContinuity] doctor_silence_fallback skipped: clinifly_sales_non_messenger", {
      profileId: String(row.id).slice(0, 8),
    });
    return { sent: false, reason: "clinifly_sales" };
  }
  ...
  const silenceFlags =
    row.operational_intake_flags && typeof row.operational_intake_flags === "object"
      ...
      : {};
  if (isBookingFlowInProgress(silenceFlags)) {
    console.log("[aiSlaContinuity] doctor_silence_fallback skipped: active_booking", {
      patientId: patientId.slice(0, 8),
      clinicId: clinicId.slice(0, 8),
    });
    return { sent: false, reason: "active_booking" };
  }
...
const CONTINUITY_TEMPLATES = {
  en: `Thank you for your message 😊\nWe're reviewing your details now and will share treatment information and estimated pricing shortly.`,
  tr: `Mesajınız için teşekkürler 😊\nBilgilerinizi inceliyoruz; kısa süre içinde tedavi detayları ve tahmini fiyat bilgisini paylaşacağız.`,
  ru: `Спасибо за сообщение 😊\nМы изучаем вашу информацию и скоро отправим детали лечения и ориентировочную стоимость.`,
  ka: `გმადლობთ შეტყობინებისთვის 😊\nთქვენს ინფორმაციას ვიხილავთ და მალე გაგიზიარებთ მკურნალობის დეტალებსა და სავარაუდო ფასებს.`,
};
...
async function trySendContinuityForProfile(profileRow, opts = {}) {
  ...
  if (isCliniflySalesLeadProfile(profileRow)) {
    console.log("[aiSlaContinuity] continuity skipped: clinifly_sales", {
  ...
  const continuityFlags =
    profileRow.operational_intake_flags && typeof profileRow.operational_intake_flags === "object"
      ...
      : {};
  if (isBookingFlowInProgress(continuityFlags)) {
    console.log("[aiSlaContinuity] continuity skipped: active_booking", {
      profileId: profileId.slice(0, 8),
    });
    return { sent: false, reason: "active_booking" };
  }
  ...
  const { data: recentAi } = await supabase
    ...
    .select("id")
    .eq("profile_id", profileId)
    .in("event_type", ["ai_reply", "clinifly_sales_reply"])
    .gte("created_at", lastPatientAt)
    .limit(1);
  if (recentAi?.length) {
    return { sent: false, reason: "ai_already_replied" };
...
async function afterPatientInboundMessage(params) {
  ...
  if (
    pageAiMode === PAGE_AI_MODE.HUMAN &&
    (inboundSource === "messenger" || params.channel === "messenger")
  ) {
    console.log("[aiSlaContinuity] messenger page human_only — skipping AI", {
      patientId: patientId.slice(0, 8),
      pageId: params.pageId ? String(params.pageId).slice(0, 12) : null,
    });
    return;
  }
  ...
  console.log("[aiSlaContinuity] inbound orchestration", {
    ...
    messageBufferDelayMs: orch.messageBufferDelayMs,
  ...
  const runInstantAi = async (turnPayload = {}) => {
    ...
    try {
      ...
      const useCliniflySales =
        pageAiMode === PAGE_AI_MODE.CLINIFLY_SALES &&
        (source === "messenger" || inboundChannel === "messenger");
      const aiResult = useCliniflySales
        ? await runCliniflySalesInboundReply({
            patientId,
            clinicId,
        ...
        : await runAiReplyForClinicInbound({
            patientId,
            clinicId,
            patientMessage: turnMessage,
            channel: inboundChannel,
      ...
      if (aiResult.reason === "superseded_by_newer_patient_message") {
        const pending = pendingInstantAiReplies.get(instantKey);
        const { data: latestProf } = await supabase
          ...
          .eq("clinic_id", clinicId)
          .maybeSingle();
        const latestMsg = String(
          pending?.payload?.patientMessage ||
            latestProf?.last_patient_message ||
            turnMessage,
        ).trim();
        if (latestMsg) {
          const retry = useCliniflySales
            ? await runCliniflySalesInboundReply({
                patientId,
                clinicId,
      ...
      if (aiResult.deferRetry === true || aiResult.reason === "parallel_ai_generation_deferred") {
        const deferKey = `${patientId}:${clinicId}`;
        const deferMs = Math.max(
          2800,
          Number(orch.messageBufferDelayMs) || DEFAULT_MESSAGE_BUFFER_MS,
          Number(orch.instantDelayMs) || 0,
        ) + 300;
        scheduleCoalescedInstantAiReply(
          deferKey,
          {
            patientMessage: turnMessage,
            scheduledForPatientAt: turnScheduledAt,
            externalMessageId: turnExternalId,
          },
          deferMs,
          runInstantAi,
        );
        logAiReplyLatency(latency.traceId, "instant_deferred_retry", {
          aiReason: aiResult.reason,
          patientPreview: turnMessage.slice(0, 80),
        });
        latency.finish({ mode: "instant", sent: false, aiReason: aiResult.reason, deferred: true });
        return;
      }
  ...
  const bufferMs = Math.max(
    0,
    Number(orch.messageBufferDelayMs) ||
      (isOmnichannelBurst ? DEFAULT_MESSAGE_BUFFER_MS : Number(orch.instantDelayMs) || 400),
  );
  const delayMs = isOmnichannelBurst ? bufferMs : Math.max(bufferMs, 180);
...
async function runSlaContinuitySweep() {
  ...
  const { data: rows, error } = await supabase
    .from("ai_coordinator_lead_profiles")
    .select(
      "id, patient_id, clinic_id, conversation_type, source, channel_metadata, coordination_mode, primary_channel, ai_mode, ai_paused, ai_escalation_required, escalation_flags, conversation_primary_language, preferred_language, operational_intake_flags, last_patient_message, last_patient_message_at, last_channel_message_at, last_human_reply_at, last_ai_reply_at",
    )
  ...
  for (const row of rows || []) {
    if (isCliniflySalesLeadProfile(row)) continue;
    if (!isWaitingOnClinic(row)) continue;
    const sweepFlags =
      row.operational_intake_flags && typeof row.operational_intake_flags === "object"
        ? row.operational_intake_flags
        : {};
    if (isBookingFlowInProgress(sweepFlags)) continue;
    const lastPatientAt = String(
      row.last_patient_message_at || row.last_channel_message_at || "",
    );
