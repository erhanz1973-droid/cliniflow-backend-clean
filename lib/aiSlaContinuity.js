...
function scheduleCoalescedInstantAiReply(key, payload, delayMs, runFn) {
  ...
  const mergeBurst =
    prev?.payload &&
    (shouldMergeWithLastPatientMessage(nextMsg, prevMsg) ||
      (!patientMessagesNearDuplicate(prevMsg, nextMsg) && prevMsg && nextMsg));

  const merged = mergeBurst
    ? {
        ...payload,
        patientMessage: [prevMsg, nextMsg].filter(Boolean).join("\n"),
...
async function afterPatientInboundMessage(params) {
  ...
  const isOmnichannelBurst =
    ...
    source === "messenger" ||
    source === "instagram";
  const burstCoalesceMs = isOmnichannelBurst ? 1500 : 180;
