...
function doctorSilenceFallbackMinutes() {
  const n = parseFloat(process.env.AI_DOCTOR_SILENCE_FALLBACK_MINUTES || "2");
...
function scheduleDoctorSilenceAiFallback(scheduleKey, params) {
  ...
  const delayMs = Math.max(30_000, Math.round(doctorSilenceFallbackMinutes() * 60 * 1000));
...
async function touchLeadProfileFromInbound(patientId, clinicId, patientMessage) {
  ...
  if (existing?.id) {
    ...
    if (!doctorExplicitTakeover && existing.ai_escalation_required !== true) {
...
async function afterPatientInboundMessage(params) {
  ...
  const profileId = await touchLeadProfileFromInbound(patientId, clinicId, patientMessage);
  ...
  const delayMs = await resolveInboundReplyDelayMs(profileId, source, isQuoteRequest);
  ...
  setTimeout(async () => {
    try {
      ...
      if (
        ...
        console.log("[aiSlaContinuity] immediate AI skipped; doctor-silence timer still active", {
          patientId: patientId.slice(0, 8),
          clinicId: clinicId.slice(0, 8),
          aiReason: aiResult.reason,
          fallbackMinutes: doctorSilenceFallbackMinutes(),
        });
        return;
      }

      if (!aiResult.sent) {
        console.log("[aiSlaContinuity] immediate AI failed; waiting for doctor-silence fallback", {
          patientId: patientId.slice(0, 8),
          reason: aiResult.reason,
          fallbackMinutes: doctorSilenceFallbackMinutes(),
        });
