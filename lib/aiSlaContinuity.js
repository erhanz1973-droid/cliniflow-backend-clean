...
function doctorSilenceFallbackMinutes(overrideMinutes) {
  ...
  const n = parseFloat(process.env.AI_DOCTOR_SILENCE_FALLBACK_MINUTES || "5");
...
async function afterPatientInboundMessage(params) {
  ...
  if (profileId) {
    const { data: profRow } = await supabase
      .from("ai_coordinator_lead_profiles")
      .select(
        "id, ai_mode, ai_paused, ai_escalation_required, coordination_mode, escalation_flags, assigned_doctor_id",
      )
      .eq("id", profileId)
      .maybeSingle();
    profileRowForDelegation = profRow;
  }
  ...
  const delayMs = orch.instantDelayMs;
  ...
  const timer = setTimeout(() => {
    pendingInstantAiReplies.delete(instantKey);
    void runInstantAi();
  }, delayMs);
  timer.unref?.();
  pendingInstantAiReplies.set(instantKey, timer);
}
