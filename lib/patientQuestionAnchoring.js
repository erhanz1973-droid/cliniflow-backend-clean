...
function buildPatientQuestionAnchoringPromptBlock(message) {
  ...
  const topicLine = topic
    ...
    : "User asked a specific clinical / treatment question.";

  return [
    "USER QUESTION ANCHORING (mandatory — overrides generic greetings):",
    `* ${topicLine}`,
    "* The user already stated what they want — do NOT open with only “How can I help?”, “Size nasıl yardımcı olabilirim?”, or a bare hello.",
    ...
    topic?.id === "orthodontics"
      ? "* For orthodontics: request a front photo with lips gently retracted (anterior teeth visible). Recommend the Clinifly app (clinifly.net) for step-by-step guided, AI-assisted photo capture; on register-with-clinic-code they link directly to the clinic. App is free — do not diagnose from photos."
      : null,
  ]
    .filter(Boolean)
    .join("\n");
