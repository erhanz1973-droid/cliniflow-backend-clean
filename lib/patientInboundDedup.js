...
/**
 * One active AI generation per patient/clinic.
 * Continuation/near-duplicate bursts are dropped; substantive messages are deferred (not lost).
 * @param {string} patientId
 * @param {string} clinicId
 * @param {string} [message]
 * @param {{ recentTurns?: Array<{ role?: string, text?: string, body?: string }> }} [opts]
 * @returns {{ allowed: boolean, deferRetry?: boolean, reason?: string }}
 */
function beginAiReplyGeneration(patientId, clinicId, message = "", opts = {}) {
  const key = inflightKey(patientId, clinicId);
  const norm = normalizePatientMessageKey(message);
  const now = Date.now();
  ...
  const prev = aiGenerationInProgress.get(key);
  if (prev) {
    let suppressWithoutRetry = false;
    try {
      const { isSchedulingContinuationFragment } = require("./aiInboundRouter");
      if (isSchedulingContinuationFragment(message, opts.recentTurns || [])) {
        suppressWithoutRetry = true;
      }
    } catch (_) {
      /* optional */
    }
    if (!suppressWithoutRetry && norm && prev.norm && patientMessagesNearDuplicate(prev.norm, norm)) {
      suppressWithoutRetry = true;
    }
    if (suppressWithoutRetry) {
      return { allowed: false, deferRetry: false, reason: "parallel_burst_suppressed" };
    }
    return { allowed: false, deferRetry: true, reason: "parallel_ai_generation_deferred" };
  }
  aiGenerationInProgress.set(key, { at: now, norm: norm || null });
  return { allowed: true };
}
