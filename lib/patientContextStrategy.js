...
const TRAVEL_SIGNAL_PATTERNS = [
  ...
  /\b(from|i live in|den|dan)\s+(germany|uk|england|france|usa|u\.s\.|netherlands|belgium|saudi|dubai|russia|georgia|gürcistan|gurcistan|turkey|türkiye|iran|iraq|ukraine|poland|italy|spain|tiflis|tbilisi)\b/i,
  /\b(tiflis|tbilisi|antalya|gürcistan|gurcistan)\b/i,
  /\b(geleceğim|gelecegim|geleceğiz|gelecegiz)\b/i,
  /\b(uçak|ucak|uçak\s*bilet|otel|konaklama)\b/i,
...
USER CONTEXT — DEFAULT LOCAL (strategy layer):
* Assume this user is local or same-city unless travel intent is explicitly established in the conversation.
...
* If the user asks only about price, treatment, or clinical steps, answer directly without travel add-ons.
* Only discuss stay/transfer/logistics when the user clearly indicates they are traveling for treatment or asks about accommodation/transfer.
...
function buildPatientContextStrategyPromptBlock(strategy) {
  const lines = [
    `User context class: ${strategy.patient_context_class}`,
