...
/** Patient says the stored WhatsApp/mobile number is wrong and gives the correct one. */
const PHONE_CORRECTION_RE =
  /\b(yanlış|yanlis|hatalı|hatali|wrong|incorrect|not\s+correct|that'?s\s+wrong|düzelt|duzelt|düzeltin|duzeltin|correct|fix|update|değiştir|degistir|aslında|aslinda|benim\s+numaram|my\s+(?:correct\s+)?number\s+is|doğru\s+numara|dogru\s+numara|yanlış\s+yaz|yanlis\s+yaz|yanlış\s+kaydet|yanlis\s+kaydet|numaramı\s+yanlış|numarami\s+yanlis|numaram\s+değil|numaram\s+degil|kayıtlıyım|kayitliyim|zaten\s+kayıtlı|zaten\s+kayitli)\b/i;
 ...
 * @param {string} text
 */
function patientClaimsPhoneCorrection(text) {
  return PHONE_CORRECTION_RE.test(String(text || ""));
}

/**
 * Extract a corrected phone when the patient disputes the stored number.
 * @param {string} message
 * @param {string|null|undefined} existingNumber
 */
function extractPhoneCorrectionFromMessage(message, existingNumber) {
  const text = String(message || "").trim();
  if (!text) return null;

  const extracted = extractWhatsappFromPatientMessage(text, { awaiting: true });
  if (!extracted?.number) return null;

  const normalized = normalizeWhatsappNumber(extracted.number);
  if (!normalized) return null;

  const existing = normalizeWhatsappNumber(existingNumber);
  const claimsCorrection = patientClaimsPhoneCorrection(text);
  const explicitIntl = /\+\d{2,4}/.test(text);

  if (claimsCorrection && (!existing || normalized !== existing)) {
    return { number: normalized, previousNumber: existing || null, explicit: true };
  }

  if (existing && normalized !== existing && explicitIntl) {
    return { number: normalized, previousNumber: existing, explicit: true };
  }
...
/**
 * Conversation arrived via WhatsApp Cloud API (sender wa_id is the contact).
 * @param {Record<string, unknown>|null|undefined} profileRow
 * @param {string} [source]
 */
function isWhatsappInboundConversation(profileRow, source) {
  const src = String(source || profileRow?.primary_channel || profileRow?.source || "")
    .trim()
    .toLowerCase();
  return src === "whatsapp";
}
...
/**
 * Meta WhatsApp inbound already includes sender wa_id — use it instead of asking again.
 * @param {Record<string, unknown>} profileRow
 */
function resolveWhatsappFromInboundChannel(profileRow) {
  const row = profileRow && typeof profileRow === "object" ? profileRow : {};
  const existing = normalizeWhatsappNumber(row.whatsapp_number);
  if (existing) return existing;

  const meta =
    row.channel_metadata && typeof row.channel_metadata === "object"
      ? /** @type {Record<string, unknown>} */ (row.channel_metadata)
      : {};
  return normalizeWhatsappNumber(meta.whatsapp_wa_id);
}

/**
 * Resolve best-known patient contact phone (WhatsApp/mobile) without asking again on WhatsApp.
 * @param {Record<string, unknown>} profileRow
 * @param {{
 *   leadData?: Record<string, unknown>|null,
 *   patientMessage?: string,
 *   bookingState?: Record<string, unknown>|null,
 *   loadPatientRow?: boolean,
 * }} [opts]
 */
async function resolvePatientContactPhone(profileRow, opts = {}) {
  const profile = profileRow && typeof profileRow === "object" ? profileRow : {};
  const lead = opts.leadData && typeof opts.leadData === "object" ? opts.leadData : {};
  const flags =
    profile.operational_intake_flags && typeof profile.operational_intake_flags === "object"
      ? profile.operational_intake_flags
      : {};
  const ab =
    opts.bookingState && typeof opts.bookingState === "object"
      ? opts.bookingState
      : flags.aiBooking && typeof flags.aiBooking === "object"
        ? flags.aiBooking
        : {};

  let phone =
    normalizeWhatsappNumber(profile.whatsapp_number) ||
    resolveWhatsappFromInboundChannel(profile) ||
    normalizeWhatsappNumber(lead.whatsappNumber ?? lead.whatsapp_number) ||
    normalizeWhatsappNumber(ab.contactPhone);

  if (!phone && opts.patientMessage) {
    const extracted = extractWhatsappFromPatientMessage(String(opts.patientMessage), {
      awaiting: true,
    });
    if (extracted?.number) phone = normalizeWhatsappNumber(extracted.number);
  }

  if (!phone && opts.loadPatientRow !== false && UUID_RE.test(String(profile.patient_id || ""))) {
    try {
 ...
 * @param {string} [lang]
 */
function buildWhatsappContactConfirmationPhrase(number, lang = "tr") {
  const code = String(lang || "tr").slice(0, 2).toLowerCase();
  const display = number ? String(number) : "";
  if (code === "en") {
    return display
      ? `Should we continue using this WhatsApp number (${display}) for communication? You can share a different number if you prefer.`
      : "Should we continue using this WhatsApp number for communication? You can share a different number if you prefer.";
  }
  if (code === "ru") {
    return display
      ? `Продолжим использовать этот номер WhatsApp (${display}) для связи? Можете указать другой номер.`
      : "Продолжим использовать этот номер WhatsApp для связи? Можете указать другой номер.";
  }
  return display
    ? `İletişim için bu WhatsApp numarasını (${display}) kullanmaya devam edelim mi? Farklı bir numara isterseniz yazabilirsiniz.`
    : "İletişim için bu WhatsApp numaranızı kullanmaya devam edelim mi? Farklı bir numara isterseniz yazabilirsiniz.";
}
...
function evaluateWhatsappCollectionCandidate(profileRow, flags, leadData = {}, patientMessage = "") {
  ...
  const primaryChannel = String(profileRow.primary_channel || profileRow.source || "").toLowerCase();
  if (primaryChannel === "whatsapp" || isWhatsappInboundConversation(profileRow)) {
    return {
      candidate: false,
      reason: "whatsapp_inbound_channel",
      settings,
      whatsappNumber: existing || resolveWhatsappFromInboundChannel(profileRow) || null,
    };
  }
...
* If the patient is already messaging on WhatsApp, NEVER ask for their WhatsApp number — use the sender number and only confirm: "Should we continue using this WhatsApp number for communication?"
 ...
 * @param {string} [lang]
 */
function formatPhoneCorrectionAcknowledgmentReply(number, lang = "tr") {
  const code = String(lang || "tr").slice(0, 2).toLowerCase();
  const display = number ? String(number) : "";
  if (code === "en") {
    return display
      ? `Sorry about that — we've corrected your WhatsApp number to ${display}. We'll use it only for appointment and travel coordination.`
      : "Sorry about that — we've corrected your WhatsApp number. We'll use it only for appointment and travel coordination.";
  }
  if (code === "ru") {
    return display
      ? `Извините — мы исправили ваш номер WhatsApp на ${display}. Используем его только для координации визита.`
      : "Извините — мы исправили ваш номер WhatsApp. Используем его только для координации визита.";
  }
  return display
    ? `Özür dileriz — numaranızı düzelttik (${display}). Randevu ve seyahat koordinasyonu için bu numarayı kullanacağız.`
    : "Özür dileriz — numaranızı düzelttik. Randevu ve seyahat koordinasyonu için bu numarayı kullanacağız.";
}
...
function repairPhoneNumberTurnReply(reply, patientMessage, opts = {}) {
  ...
  const sharesNumber = patientMessageSharesWhatsappNumber(patient);
  const claimsCorrection = patientClaimsPhoneCorrection(patient);
  if (!patient || (!sharesNumber && !claimsCorrection)) return out;
  ...
  const existingStored = normalizeWhatsappNumber(
    opts.profileRow?.whatsapp_number || opts.previousWhatsappNumber,
  );
  const correction = extractPhoneCorrectionFromMessage(patient, existingStored);
  const num =
    correction?.number ||
    extractWhatsappFromPatientMessage(patient, { awaiting: true })?.number ||
    normalizeWhatsappNumber(opts.whatsappNumber) ||
    null;

  if (correction?.number) {
    console.warn("[whatsappCollection] repaired phone correction reply", {
      from: correction.previousNumber ? String(correction.previousNumber).slice(0, 8) + "…" : null,
      to: correction.number.slice(0, 8) + "…",
    });
    return formatPhoneCorrectionAcknowledgmentReply(num, ackLang);
  }
  const thanksRe =
...
function repairCoordinatorCapabilityMisreply(reply, patientMessage, opts = {}) {
  ...
  const channel = String(opts.channel || opts.inboundChannel || "").toLowerCase();
  const profileRow = opts.profileRow && typeof opts.profileRow === "object" ? opts.profileRow : null;
  const onWhatsapp =
    channel === "whatsapp" ||
    String(profileRow?.primary_channel || "").toLowerCase() === "whatsapp";
  if (onWhatsapp) {
    const { repairWhatsappNumberAskOnChannel } = require("./aiInboundRouter");
    out = repairWhatsappNumberAskOnChannel(
      out,
      "whatsapp",
      opts.whatsappNumber || opts.knownWhatsapp || null,
    );
  }
  return out;
}
...
/**
 * @param {string|null} number E.164
 * @param {string|null|undefined} previousNumber
 * @param {string} [lang]
 */
function buildPhoneCorrectionTurnBlock(number, previousNumber, lang = "tr") {
  const display = number ? String(number) : "";
  const prev = previousNumber ? String(previousNumber) : "";
  const code = String(lang || "tr").slice(0, 2).toLowerCase();
  const example = formatPhoneCorrectionAcknowledgmentReply(number, code);
  return (
    "PHONE / WHATSAPP CORRECTION (mandatory — overrides all other replies this turn):\n" +
    `* The patient says we saved the wrong number${prev ? ` (was ${prev})` : ""} and gave the correct one${display ? ` (${display})` : ""}.\n` +
    "* Apologize briefly, confirm the corrected number, and say it is for appointment/travel coordination only.\n" +
    "* Do NOT repeat the old wrong number.\n" +
    "* Do NOT ask for the number again.\n" +
    `* Example tone (adapt, do not copy verbatim if unnatural): ${example}`
  );
}
...
function extractWhatsappFromPatientMessage(message, opts = {}) {
  ...
  const leadingNum = text.match(/\b(\+?\d{10,15})\b\s*(?:numaram|telefonum|numara\b|whatsapp)/i);
  if (leadingNum) {
    const normalizedLeading = normalizeWhatsappNumber(leadingNum[1]);
    if (normalizedLeading) return { number: normalizedLeading };
  }
...
function resolveWhatsappFromPatientTurn(message, ctx = {}) {
  ...
  const correction = extractPhoneCorrectionFromMessage(message, existing);
  if (correction?.number) {
    return {
      number: correction.number,
      previousNumber: correction.previousNumber || null,
      source: "correction",
    };
  }
  ...
  if (result?.number) {
    if (!existing || result.number !== existing) {
      return { ...result, source: "current_message" };
    }
  }
...
async function persistWhatsappCollection(profileId, input) {
  ...
  if (input.declined) {
    ...
    if (input.previousNumber) {
      flags.whatsappPreviousNumber = normalizeWhatsappNumber(input.previousNumber);
      flags.whatsappCorrectedAt = now;
    }
  }
  ...
  if (input.number && UUID_RE.test(String(row?.patient_id || ""))) {
    const n = normalizeWhatsappNumber(input.number);
    if (n) {
      const patientPatch = { updated_at: now, phone: n };
      const { error: patientErr } = await supabase
        .from("patients")
        .update(patientPatch)
        .eq("id", String(row.patient_id));
      if (patientErr && !String(patientErr.message || "").toLowerCase().includes("column")) {
        console.warn("[whatsappCollection] patient phone sync:", patientErr.message);
      }
    }
  }

  return { ok: true, corrected: !!input.previousNumber };
}
