...
const LANGUAGE_CAPABILITY_DEFLECTION_RE =
  /\b(türkçe\s+biliyorum|turkce\s+biliyorum|turkish|ingilizce\s+biliyorum|know\s+turkish|speak\s+turkish|size\s+türkçe\s+yardımcı|size\s+turkce\s+yardimci|can\s+help\s+you\s+in\s+turkish|reply\s+in\s+turkish)\b/i;
...
function repairPhoneNumberTurnReply(reply, patientMessage, opts = {}) {
  ...
  if (!patient || !isPhoneOnlyPatientMessage(patient)) return out;
  ...
  if (LANGUAGE_CAPABILITY_DEFLECTION_RE.test(out) || isGenericPhoneTurnMisreply(out)) {
    ...
    return formatPhoneAcknowledgmentReply(num, opts.lang);
  }
  return out;
}
