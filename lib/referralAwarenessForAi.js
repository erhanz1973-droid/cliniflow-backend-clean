 ...
 * @param {{ message?: string, topicContext?: Record<string, unknown>|null, recentTurns?: Array<{ role?: string, text?: string }> }} [opts]
 */
function shouldSurfaceReferralThisTurn(signals, opts = {}) {
  const gate = evaluateReferralMentionGate({
    message: opts.message || "",
    asksReferral: signals.asksReferral,
    priceSensitive: signals.priceSensitive,
    topicContext: opts.topicContext,
    recentTurns: opts.recentTurns,
  });
  return gate.shouldSurface;
}
...
async function buildReferralAwarenessForAi(params) {
  ...
  const mentionGate = evaluateReferralMentionGate({
    message,
    asksReferral: signals.asksReferral,
    priceSensitive,
    topicContext,
  ...
  const shouldSurface = mentionGate.shouldSurface;
  const shortReferralContinuation =
    shouldSurface &&
    topicContext?.isShortReferralContinuation === true &&
    (signals.asksReferral || recentPatientTurnsDiscussReferral(recentTurns));
  ...
  const promptBlock = shouldSurface
    ? [
        buildReferralGatingPromptSnippet(mentionGate),
        buildReferralAwarenessPromptBlock(state, {
          asksReferral: signals.asksReferral || shortReferralContinuation,
          priceSensitive: priceSensitive && mentionGate.user_current_intent === "price_cost",
...
function buildReferralAwarenessPromptBlock(state, signals) {
  ...
  const lines = [
    ...
    "* Do NOT append referral/promotional information unless directly relevant to the user's current intent.",
    "* NEVER add a referral paragraph after answering a medical, operational, or address/travel question.",
    signals.shortContinuation || signals.pendingAction === "provide_referral_code"
      ...
      : signals.asksReferral
        ? "* Patient asked about referral/discount/campaign — explain clearly; keep reply focused on referral only."
        : signals.priceSensitive
          ? "* Patient asked about price/cost reduction — you MAY mention referral in ONE short optional sentence after answering their price question. No promotional tail."
          : "",
