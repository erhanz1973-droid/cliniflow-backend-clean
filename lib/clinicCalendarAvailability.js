...
const {
  parsePreferredDateFromMessage: parsePreferredDateFromMessageCore,
...
function parsePreferredDateFromMessage(message, timezone = "Europe/Istanbul", refDate = new Date()) {
  return parsePreferredDateFromMessageCore(message, timezone, refDate);
}
...
const { patientHasNegativeSchedulingIntent } = require("./aiBookingState");
...
/**
 * Patient lost the slot list (WhatsApp scroll) or explicitly asks to see options/times again.
 * @param {string} message
 */
function patientRequestsSlotListResend(message) {
  const raw = String(message || "").trim();
  const t = raw.toLowerCase();
  if (!t || t.length > 220) return false;
  if (/^[1-9]$|^1[0-2]$/.test(t)) return false;
  if (patientHasNegativeSchedulingIntent(message)) return false;

  if (
    /\b(tekrar|yeniden|bir\s*daha|yine|again)\b/.test(t) &&
    /\b(saat|saatler|seçenek|secenek|tarih|müsait|musait|uygun|liste|list|option|time|slot|randevu|at|gönder|gonder|yolla|paylaş|paylas|göster|goster)\b/.test(
      t,
    )
  ) {
    return true;
  ...
  if (
    /\b(hangi\s+(saat|saatler|seçenek|secenek|tarih|seçenekler)|seçenekleri?\s+(yolla|gönder|gonder|at|paylaş|paylas)|müsait\s+(tarih|saat)|musait\s+(tarih|saat)|uygun\s+(saat|saatler|tarih)|saatleri?\s+(yolla|gönder|gonder|at|paylaş|paylas|göster|goster)|bana\s+(seçenek|secenek|saat|müsait|musait)|seçenekler(inizi)?\s)/i.test(
      t,
    )
  ) {
    return true;
  ...
  return /^(hangi saat(ler)?|saatler neler|müsait saatler|musait saatler|uygun saatler)\??$/i.test(raw);
}
