 ...
 */
async function fetchClinicBusyIntervalsForDay(clinicId, dateYmd, timezone = "UTC") {
...
function parsePreferredTimeMinutesFromMessage(message) {
  const t = String(message || "").toLowerCase();
  const m = t.match(/\b(?:saat\s*)?(\d{1,2})\s*[:.]?\s*(\d{2})?\s*(?:'|de|da)?\b/);
  if (m) {
    const h = Number(m[1]);
    const min = m[2] != null ? Number(m[2]) : 0;
    if (h >= 0 && h <= 23) return h * 60 + min;
  }
  if (/\bsabah\b/i.test(t)) return 9 * 60;
  if (/\böğleden sonra\b|\bogleden sonra\b|\bafternoon\b/i.test(t)) return 14 * 60;
  if (/\bakşam\b|\baksam\b|\bevening\b/i.test(t)) return 17 * 60;
  return null;
}
...
function patientRequestsAlternateSlots(message) {
  const t = String(message || "").toLowerCase();
  return (
    /\b(başka|baska|farklı|farkli|alternatif|olmaz|olmasın|olmasin|uygun değil|uygun degil|musait degil|müsait değil|yer yok|dolu|meşgul|mesgul|başka saat|baska saat|farklı saat|farkli saat|erken|geç|gec|yarın|yarin)\b/i.test(
      t,
    ) || /\b\d{1,2}\s*[:.]\s*\d{2}\b/.test(t)
  );
}
