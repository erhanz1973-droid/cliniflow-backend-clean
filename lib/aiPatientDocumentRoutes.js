      ...
      if (sessionId) {
        ...
        if (profile?.id) {
          const { data: row } = await supabase
            .from("ai_coordinator_lead_profiles")
            .select(
              "operational_intake_flags, treatment_interest, country, preferred_language, travel_timeline, urgency, booking_intent, budget_signal",
            )
            .eq("id", profile.id)
            .maybeSingle();
          ...
          documents = await listDocumentsForProfile(profile.id, { clinicId });
