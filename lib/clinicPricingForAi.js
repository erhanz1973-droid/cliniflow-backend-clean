...
const { normalizeTreatmentLabelI18n } = require("./clinicMultilingual");
...
function mapPriceRow(row) {
  ...
  const labelI18n = normalizeTreatmentLabelI18n(row.label_i18n);
...
async function listTreatmentPricesForClinic(clinicId, opts = {}) {
  ...
  const variantMap = await listVariantsByPriceIds(items.map((i) => i.id), { activeOnly: true });
...
async function getPricingKnowledgeForAi(clinicId) {
  ...
  return {
    ...
    treatments: prices.map((p) => ({
      ...
      labelI18n: p.labelI18n && Object.keys(p.labelI18n).length ? p.labelI18n : undefined,
