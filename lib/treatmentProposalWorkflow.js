...
const { countFormalOffersForRequest } = require("./treatmentRequestLifecycle");
...
const PATIENT_STATUS_LABELS = {
  ...
  coordinator_responded: {
    en: "The clinic team has replied — open coordination chat for details.",
    tr: "Klinik ekibi yanıt verdi — detaylar için koordinasyon sohbetini açın.",
    ru: "Команда клиники ответила — откройте чат координации.",
  },
  proposal_pending: {
    en: "Clinic is preparing your treatment estimate.",
    tr: "Klinik tedavi tahmininizi hazırlıyor.",
 ...
 */
function countFormalOffersInList(offers) {
  return (offers || []).filter((o) => !isCoordinationPlaceholderOffer(o)).length;
}

/**
 * @param {string|null|undefined} raw
 * @param {boolean} hasFormalOffers — real treatment_offers only (not coordination chat placeholder)
 */
function normalizeProposalStatus(raw, hasFormalOffers) {
  if (hasFormalOffers) return PROPOSAL_STATUS.QUOTE_SENT;
  const s = String(raw || "")
    .trim()
    .toLowerCase();
  /** DB may say quote_sent after SLA sweep counted a coordination placeholder — repair on read. */
  if (s === PROPOSAL_STATUS.QUOTE_SENT) {
    return PROPOSAL_STATUS.COORDINATOR_RESPONDED;
  }
  if (s === PROPOSAL_STATUS.PROPOSAL_PENDING) return PROPOSAL_STATUS.WAITING_FOR_QUOTE;
  if (s === PROPOSAL_STATUS.COORDINATOR_RESPONDED) return s;
  if (ACTIVE_QUOTE_STATUSES.has(s)) return s;
 ...
 * @param {{ formalOfferCount?: number, offerCount?: number, offers?: Array<Record<string, unknown>> }} [opts]
 */
function enrichRequestProposalFields(row, opts = {}) {
  const formalOfferCount =
    opts.formalOfferCount != null
      ? Number(opts.formalOfferCount) || 0
      : Array.isArray(opts.offers)
        ? countFormalOffersInList(opts.offers)
        : 0;
  const hasFormalOffers = formalOfferCount > 0;
  const proposalStatus = normalizeProposalStatus(row.proposal_status, hasFormalOffers);
  const since =
    row.proposal_waiting_since || row.proposal_status_at || row.created_at || null;
  const waitingMinutes = hasFormalOffers ? null : waitingMinutesSince(since);
  const draft =
  ...
  return {
    ...
    coordinator_queue_title:
      !hasFormalOffers && ACTIVE_QUOTE_STATUSES.has(proposalStatus)
        ? formatCoordinatorQueueTitle(waitingMinutes)
        : null,
    needs_quote_action: !hasFormalOffers && ACTIVE_QUOTE_STATUSES.has(proposalStatus),
  };
}
...
async function markProposalQuoteSent(requestId) {
  const { markTreatmentRequestQuoted } = require("./treatmentRequestLifecycle");
  await markTreatmentRequestQuoted(requestId);
...
async function listProposalQueueForClinic(clinicId, opts = {}) {
  ...
  const formalOfferCountByReq = {};
  if (ids.length) {
    const { data: offers } = await supabase
      .from("treatment_offers")
      .select("request_id, note, price_text, price_range, is_coordination_placeholder")
      .in("request_id", ids);
    for (const o of offers || []) {
      if (isCoordinationPlaceholderOffer(o)) continue;
      const rid = o.request_id;
      formalOfferCountByReq[rid] = (formalOfferCountByReq[rid] || 0) + 1;
    }
  }
  ...
  return (rows || [])
    .filter((r) => !(formalOfferCountByReq[r.id] > 0))
    .map((r) => {
...
async function runProposalSlaSweep() {
  ...
  const withFormalOffer = new Set();
  for (const o of offers || []) {
    if (!isCoordinationPlaceholderOffer(o) && o.request_id) {
      withFormalOffer.add(o.request_id);
    }
  }
  ...
  const { data: staleQuoted } = await supabase
    ...
    .eq("proposal_status", PROPOSAL_STATUS.QUOTE_SENT)
    .limit(80);
  for (const row of staleQuoted || []) {
    if (withFormalOffer.has(row.id)) continue;
    const formal = await countFormalOffersForRequest(String(row.id));
    if (formal === 0) {
      void setProposalStatus(String(row.id), PROPOSAL_STATUS.COORDINATOR_RESPONDED);
    }
  }

  for (const row of rows) {
    if (withFormalOffer.has(row.id)) {
