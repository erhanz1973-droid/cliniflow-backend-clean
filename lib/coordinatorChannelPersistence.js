...
async function insertChannelMessagesWithChannel(rows) {
  ...
  let result = await supabase.from("ai_coordinator_channel_messages").insert(prepared);
  if (!result.error) return result;

  if (isChannelCheckConstraintError(result.error)) {
    const fallbackRows = prepared.map((row) => {
    ...
    result = await supabase.from("ai_coordinator_channel_messages").insert(fallbackRows);
  }
  return result;
}
