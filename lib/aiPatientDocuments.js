...
async function savePatientDocumentUpload(params) {
  ...
  const contentHash =
    normalizeContentHash(params.contentHash) || computeBufferSha256(file.buffer);
  const documentTypeEarly = normalizeDocumentType(params.documentType);
  const sessionIdEarly = params.sessionId ? String(params.sessionId).trim() : null;

  if (contentHash) {
    const dup = await findExistingAiPatientDocument({
      patientId: params.patientId,
      clinicId: params.clinicId,
      sessionId: sessionIdEarly,
      documentType: documentTypeEarly,
      contentHash,
    });
    if (dup?.id) {
      return { ok: true, document: mapDocumentRow(dup), reused: true };
    }
  }
  ...
  const row = {
    ...
    storage_metadata: contentHash ? { content_hash: contentHash } : {},
    uploaded_at: nowIso,
    updated_at: nowIso,
  };
