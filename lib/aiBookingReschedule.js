...
const { formatAppointmentDisplayWithTimezone: formatBookingWhenLabel } = require("./bookingTimezoneIntegrity");
const {
...
const RESCHEDULE_EXPLICIT_RE =
  /\b(reschedule|re-?schedule|move\s+it\s+to|move\s+(my\s+)?appointment|change\s+(the\s+)?(time|date|day|hour)|another\s+(hour|time|day)|different\s+(day|time|date|hour)|ertele\w*|degistir\w*|değiştir\w*|baska\s+(saat|gun|gün|tarih)|başka\s+(saat|gün|gun|tarih)|yeniden\s+planla|saati\s+degistir|saati\s+değiştir|farkli\s+(saat|gun|gün|tarih)|farklı\s+(saat|gün|tarih)|randevuyu\s+tas\w*|randevu\s+degistir|randevu\s+değiştir)\b/i;

const RESCHEDULE_DATE_ALALIM_RE =
  /\b(\d{1,2}\s*\w+\s*(?:alalim|alalım|olsun|olur|yapalim|yapalım)|\w+\s*\d{1,2}\s*(?:alalim|alalım|olsun|olur))\b/i;

const RESCHEDULE_TIME_CORRECTION_RE =
  /\b(\d{1,2}[:.]\d{2})\s*(istemiştim|istemistim|istiyordum|demek\s+istedim|olmalı|olmali|yazmıştım|yazmistim|seçmiştim|secmistim)\b/i;

/**
 * Explicit calendar date in the current message only — never infer from older turns.
 * @param {string} message
 * @param {string} timezone
 * @param {string|null|undefined} existingDateYmd
 */
function parseExplicitRescheduleDateFromMessage(message, timezone, existingDateYmd = null) {
  const msg = String(message || "").trim();
  if (!msg) return null;
  const refDate = new Date();
  return (
    parseRescheduleTargetDateFromMessage(msg, timezone, refDate, existingDateYmd || null) ||
    parsePreferredDateFromMessage(msg, timezone, refDate) ||
    null
  );
}

/**
 * Patient wants a different clock time on the same appointment day.
 * @param {string} message
 */
function patientRescheduleIsTimeOnlyChange(message) {
  const msg = String(message || "").trim();
  if (!msg) return false;
  if (parseExplicitRescheduleDateFromMessage(msg, "Europe/Istanbul")) return false;
  if (isTimeOnlyPatientMessage(msg)) return true;
  if (RESCHEDULE_TIME_CORRECTION_RE.test(msg)) return true;
  if (parseConversationalTimeToMinutes(msg) != null && messageExpressesTimeIntent(msg)) {
    if (/\b(saati|saat|time|hour|istemiştim|istemistim|istiyordum|olmalı|olmali|yanlış|yanlis|hatalı|hatali)\b/i.test(msg)) {
      return true;
    }
  }
  return false;
...
function resolveExistingAppointmentSlot(flags, scheduling, locale) {
  ...
  return {
    ...
    startAt,
    label: appt?.label || canonical?.label || formatBookingWhenLabel(startAt, locale, tz),
...
function patientRescheduleIntent(message, flags, ctx = {}) {
  ...
  if (RESCHEDULE_DATE_ALALIM_RE.test(msg)) return true;
 ...
 */
function buildRescheduleConfirmDirectReply(lang, fromLabel, toLabel, treatmentLabel, opts = {}) {
  const key = String(lang || "tr").slice(0, 2).toLowerCase();
  const from = String(fromLabel || "").trim();
  const to = String(toLabel || "").trim();
  if (opts.timeOnlySameDate && opts.dateLabel && opts.fromTime && opts.toTime) {
    const dateLabel = String(opts.dateLabel).trim();
    const fromTime = String(opts.fromTime).trim();
    const toTime = String(opts.toTime).trim();
    const tzSuffix =
      key === "tr" ? "Türkiye saati" : key === "ru" ? "время Турции" : "Turkey Time";
    if (key === "tr") {
      return `${dateLabel} randevunuzu ${fromTime} yerine ${toTime} (${tzSuffix}) olarak güncellemek ister misiniz? Onaylamak için «Evet» yazmanız yeterli.`;
    }
    if (key === "ru") {
      return `Обновить запись ${dateLabel} с ${fromTime} на ${toTime} (${tzSuffix})? Напишите «Да» для подтверждения.`;
    }
    if (key === "ka") {
      return `${dateLabel} ჩაწერა ${fromTime}-დან ${toTime}-მდე (${tzSuffix})? დაწერეთ «კი» დასადასტურებლად.`;
    }
    return `Update your ${dateLabel} appointment from ${fromTime} to ${toTime} (${tzSuffix})? Reply «Yes» to confirm.`;
  }
  if (key === "tr") {
    return `Mevcut randevunuzu ${from} yerine ${to} olarak taşımak ister misiniz? Onaylamak için «Evet» yazmanız yeterli.`;
  }
  if (key === "ru") {
...
function resolveRescheduleTargetSlot(message, existing, scheduling, recentTurns, buildSlotFn) {
  const tz = scheduling?.timezone || "Europe/Istanbul";
  const msg = String(message || "").trim();
  const explicitDateInMessage = parseExplicitRescheduleDateFromMessage(
    msg,
    tz,
    existing.dateYmd || null,
  );
  ...
  if (preferredTimeMin != null) {
    const dateYmd = explicitDateInMessage || existing.dateYmd;
    if (dateYmd) {
      return buildSlotFn(dateYmd, preferredTimeMin);
    }
  }

  if (explicitDateInMessage && preferredTimeMin == null) {
    const existingMin = parseHm(existing.time);
    if (existingMin != null) {
      return buildSlotFn(explicitDateInMessage, existingMin);
    }
  }
...
async function completeRescheduleAfterConfirmation(params) {
  ...
  const fromLabel = existing.label || formatBookingWhenLabel(existing.startAt, locale, scheduling.timezone);
  const toLabel =
    targetSlot.label || formatBookingWhenLabel(targetSlot.startAt, locale, scheduling.timezone);
...
async function prepareAiAppointmentRescheduleTurn(params) {
  ...
  logBookingWorkflowState("reschedule_turn_start", flags, { messagePreview: message.slice(0, 80) });
  ...
  if (durable.rescheduleMode === true && String(durable.stage || "") === "awaiting_slot_confirm") {
    if (isBookingConfirmationNo(message)) {
      ...
      const whenLabel =
        existing.label || formatBookingWhenLabel(existing.startAt, locale, scheduling.timezone);
      return {
        engaged: true,
        promptBlock: "",
    ...
    if (durable.selectedSlot?.startAt) {
      const whenPending =
        durable.selectedSlot?.label ||
        formatBookingWhenLabel(durable.selectedSlot?.startAt, locale, scheduling.timezone);
      return {
        engaged: true,
        promptBlock: "",
        directReply: buildRescheduleConfirmDirectReply(
          ...
          treatmentLabel,
          existing.dateYmd &&
            durable.selectedSlot?.dateYmd &&
            String(existing.dateYmd) === String(durable.selectedSlot.dateYmd)
            ? {
                timeOnlySameDate: true,
                dateLabel: formatBookingWhenLabel(existing.startAt, locale, scheduling.timezone).split(
                  " at ",
                )[0],
                fromTime: existing.time,
                toTime:
                  durable.selectedSlot?.time ||
                  formatInTimeZone(
                    new Date(durable.selectedSlot.startAt),
    ...
    // selectedSlot missing (e.g. prior persist wiped it) — re-resolve from this message below.
  }
  ...
  const targetSlot = resolveRescheduleTargetSlot(
    message,
    existing,
    scheduling,
    recentTurns,
  ...
  if (!targetSlot) {
    if (profileId) {
      ...
      logBookingWorkflowState("reschedule_need_details", flags);
    }
    return {
      engaged: true,
      promptBlock: "",
  ...
  const toLabel =
    targetSlot.label || formatBookingWhenLabel(targetSlot.startAt, locale, scheduling.timezone);
  const timeOnlySameDate =
    existing.dateYmd &&
    targetSlot.dateYmd &&
    String(existing.dateYmd) === String(targetSlot.dateYmd);
  const confirmOpts = timeOnlySameDate
    ? {
        timeOnlySameDate: true,
        dateLabel: formatBookingWhenLabel(existing.startAt, locale, scheduling.timezone).split(
          " at ",
  ...
  if (profileId) {
    ...
    logBookingWorkflowState("reschedule_started", flags, {
      from: existing.startAt,
      to: targetSlot.startAt,
    });
  }
...
module.exports = {
  ...
  patientRescheduleIsTimeOnlyChange,
  parseExplicitRescheduleDateFromMessage,
