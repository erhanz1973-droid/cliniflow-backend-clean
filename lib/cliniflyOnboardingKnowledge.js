...
function looksLikeOnboardingSupportQuery(message) {
  ...
  if (!m) return false;
  return (
    /\b(register|login|log in|settings|whatsapp|messenger|facebook|instagram|clinic code|referral|price list|treatment price|ai training|directory profile|marketplace profile|lead inbox|doctor app|approve doctor|google reviews|forgot password|admin register|how do i set up|how to connect|onboarding|customer support|kurulum|kayit|giris|doktor|dizin profili|ai egitim|fiyat listesi|video|youtube|tutorial|how to use|kullanım video|kullanim video|egitim video|eğitim video|nasıl kullan|nasil kullan|videolar|შესვლა|რეგისტრაცია|ექიმი|ლიდ|კატალოგი)\b/.test(
      m,
    ) ||
