...
function looksLikeOnboardingSupportQuery(message) {
  ...
  if (!m) return false;
  return (
    /\b(register|login|log in|settings|whatsapp|messenger|facebook|clinic code|referral|price list|treatment price|ai training|forgot password|admin register|how do i set up|how to connect|onboarding|kurulum|kayit|giris|შესვლა|რეგისტრაცია|whatsapp|messenger)\b/.test(
      m,
    ) || /(clinic code|klinik kod|კლინიკის კოდი)/.test(m)
  );
}
