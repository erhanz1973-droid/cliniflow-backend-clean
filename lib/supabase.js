...
async function getPatientsByClinicPaginated(
  ...
  const selectCols = `id, patient_id, name, phone, status, created_at, updated_at, primary_doctor_id`;
  ...
  let { data, error, count } = await run(true, true);
  if (error && patientListUpdatedAtOrderError(error)) {
    ({ data, error, count } = await run(true, false));
  }
  if (error && !includeLeads && patientListIsLeadFilterError(error)) {
    ({ data, error, count } = await run(false, true));
    if (error && patientListUpdatedAtOrderError(error)) {
      ({ data, error, count } = await run(false, false));
    }
  }
