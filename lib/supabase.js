...
async function getPatientById(patientId, opts = {}) {
  ...
  const looksLikeUuid =
    /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(rawPid);

  // UUID path: always try primary key `id` first. Querying `patient_id` with a UUID can yield
  // non–PGRST116 errors in some schemas/RLS and previously aborted before the `id` attempt.
  const attempts = looksLikeUuid
    ? [
        { column: "id", label: "id" },
  ...
  for (const a of attempts) {
    ...
    if (error) {
      ...
      console.warn("[SUPABASE] getPatientById non-fatal:", a.label, rawPid.slice(0, 12), lastErrorMsg);
      continue;
    }
  }
