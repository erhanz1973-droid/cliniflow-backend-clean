...
async function getPatientsByClinic(clinicId, { includeLeads = false } = {}) {
  ...
  const selectCols = `
      ...
      primary_doctor_id
    `;
  const run = async (applyLeadExclusion, orderByUpdatedAt) => {
...
async function getPatientsByClinicPaginated(
  ...
  const selectColsBase = `id, patient_id, name, phone, status, created_at, updated_at, primary_doctor_id`;
  const selectColsWithAssigned = `${selectColsBase}, assigned_doctor_id`;
  const searchTerm = String(search || "").trim().replace(/[%_,]/g, " ").slice(0, 80);

  const run = async (applyLeadExclusion, orderByUpdatedAt, withAssignedCol) => {
    const selectCols = withAssignedCol ? selectColsWithAssigned : selectColsBase;
    let q = supabase
      .from("patients")
  ...
  let { data, error, count } = await run(true, true, withAssignedCol);
  if (error && patientListAssignedDoctorColumnError(error)) {
    withAssignedCol = false;
    ({ data, error, count } = await run(true, true, false));
  }
  if (error && patientListUpdatedAtOrderError(error)) {
    ({ data, error, count } = await run(true, false, withAssignedCol));
  }
  if (error && !includeLeads && patientListIsLeadFilterError(error)) {
    ({ data, error, count } = await run(false, true, withAssignedCol));
    if (error && patientListUpdatedAtOrderError(error)) {
      ({ data, error, count } = await run(false, false, withAssignedCol));
    }
  }
