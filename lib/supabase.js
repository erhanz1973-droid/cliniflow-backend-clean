...
async function getPatientsByClinicPaginated(
  ...
  const selectColsBase = `id, patient_id, name, phone, status, is_lead, created_at, updated_at, primary_doctor_id`;
  const selectColsWithAssigned = `${selectColsBase}, assigned_doctor_id`;
  const searchTerm = String(search || "").trim().replace(/[%_,]/g, " ").slice(0, 80);
  void includeLeads;

  const run = async (orderByUpdatedAt, withAssignedCol, withLeadCol) => {
  ...
  let { data, error, count } = await run(true, withAssignedCol, withLeadCol);
  if (error && patientListAssignedDoctorColumnError(error)) {
    withAssignedCol = false;
    ({ data, error, count } = await run(true, withAssignedCol, withLeadCol));
  }
  if (error && patientListUpdatedAtOrderError(error)) {
    ({ data, error, count } = await run(false, withAssignedCol, withLeadCol));
  }
  if (error && patientListIsLeadFilterError(error)) {
    withLeadCol = false;
    ({ data, error, count } = await run(true, withAssignedCol, withLeadCol));
