...
const { enrichClinicRegistrationGeo } = require('./clinicRegistrationGeo.cjs');
...
async function getClinicByCode(clinicCode) {
  ...
  const exact = await supabase.from('clinics').select('*').eq('clinic_code', searchCode).maybeSingle();
  data = exact.data;
  error = exact.error;

  // Case-insensitive / whitespace-tolerant (e.g. DB stored "TRABZON " or "trabzon")
  if (!data) {
    const { data: ilikeRows, error: ilikeErr } = await supabase
      .from('clinics')
      .select('*')
      .ilike('clinic_code', `${searchCode}%`)
      .limit(8);
    if (ilikeErr && !['PGRST116', '42703', 'PGRST204', 'PGRST205'].includes(String(ilikeErr.code || ''))) {
      console.error('[SUPABASE] getClinicByCode ilike fallback:', ilikeErr.message);
    } else {
      data =
        (ilikeRows || []).find(
          (c) => String(c?.clinic_code || '').trim().toUpperCase() === searchCode,
        ) || null;
    }
  }

  // Legacy `code` column (some rows / migrations)
  if (!data) {
    const legacy = await supabase.from('clinics').select('*').eq('code', searchCode).maybeSingle();
    if (legacy.error && !['PGRST116', '42703', 'PGRST204', 'PGRST205'].includes(String(legacy.error.code || ''))) {
      console.error('[SUPABASE] getClinicByCode legacy code column:', legacy.error.message);
    } else if (legacy.data) {
      data = legacy.data;
    }
  }
...
async function createClinic(clinicData) {
  ...
  if (row.is_listed == null) row.is_listed = false;
  row = enrichClinicRegistrationGeo(row);
...
/** Insert with automatic removal of unknown columns (heterogeneous clinic schemas). */
async function insertIntoTableWithColumnPruning(table, payload, selectClause = "*") {
  if (!supabaseConfigured) {
    return { data: null, error: new Error("supabase_not_configured") };
  }
  let current = { ...(payload || {}) };
  let lastError = null;
  const hadThreadId = Object.prototype.hasOwnProperty.call(current, "thread_id") && current.thread_id != null;
  for (let attempt = 0; attempt < 16; attempt += 1) {
      ...
      if (
        ...
        try {
          ...
          logMessageThreadMissing({
            ...
            caller: "patient_messages",
            patient_id: current.patient_id || null,
            clinic_id: current.clinic_id || null,
            thread_id: null,
            resolved_thread_id: current.thread_id || null,
            reason: "thread_id_not_persisted_after_prune",
          });
        } catch (_) {
          /* optional */
        }
    ...
    if (!missingColumn || !(missingColumn in current)) return { data: null, error };
    if (table === "patient_messages" && missingColumn === "thread_id" && hadThreadId) {
      try {
        const { logMessageThreadMissing } = require("./patientMessageThreadGuard");
        logMessageThreadMissing({
          insert_fn: "insertIntoTableWithColumnPruning",
          ...
          thread_id: null,
          resolved_thread_id: current.thread_id || null,
          reason: "thread_id_column_missing",
        });
      } catch (_) {
        /* optional */
      }
...
/** Bare UUID ↔ `p_<uuid>` so admin/mobile URLs match DB rows either way. */
function expandPatientIdLookupKeys(patientId) {
  const raw = String(patientId ?? "").trim();
  if (!raw) return [];
  const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  const P_PREFIX_RE = /^p_([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/i;
  const out = [];
  const seen = new Set();
  const add = (v) => {
  ...
  add(raw);
  const m = P_PREFIX_RE.exec(raw);
  if (m) add(m[1].toLowerCase());
  if (UUID_RE.test(raw)) add(`p_${raw.toLowerCase()}`);
  return out;
}

const UUID_RE_FK = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const P_PREFIX_UUID_RE_FK =
  /^p_([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/i;

/** Bare lowercase UUID when id is UUID or `p_<uuid>`; otherwise unchanged (legacy slugs). */
function canonicalPatientIdForTimeline(patientId) {
  const raw = String(patientId ?? "").trim();
  if (!raw) return "";
  const m = P_PREFIX_UUID_RE_FK.exec(raw);
  if (m) return m[1].toLowerCase();
  if (UUID_RE_FK.test(raw)) return raw.toLowerCase();
  return raw;
}

/** All `patient_id` string variants to use in `.in('patient_id', keys)` bulk queries (UUID ∪ p_<uuid>). */
function patientFkInClauseKeysFromPatientRow(idField, legacyPatientIdField) {
  const out = new Set();
  for (const k of [
    ...expandPatientIdLookupKeys(String(idField ?? "")),
    ...expandPatientIdLookupKeys(String(legacyPatientIdField ?? "")),
  ]) {
    const v = String(k || "").trim();
    if (!v) continue;
    if (UUID_RE_FK.test(v) || P_PREFIX_UUID_RE_FK.test(v)) out.add(v);
  }
  return [...out];
}

function allPatientFkInClauseKeysForClinicTimeline(patientRows) {
  const acc = new Set();
  for (const p of patientRows || []) {
    for (const k of patientFkInClauseKeysFromPatientRow(p?.id, p?.patient_id)) acc.add(k);
  }
  return [...acc];
}

async function getPatientById(patientId, opts = {}) {
  if (!supabaseConfigured) return null;
  const clinicId = opts.clinicId != null ? String(opts.clinicId).trim() : "";
  const rawPid = String(patientId ?? "").trim();
  if (!rawPid) return null;

  const keys = expandPatientIdLookupKeys(rawPid);
  // Prefer `id` before `patient_id` (avoids odd RLS / schema noise on patient_id in some envs).
  const columnOrder = ["id", "patient_id"];

  let lastErrorMsg = "";
  for (const key of keys) {
    for (const column of columnOrder) {
      // patients.id is UUID typed; never send p_<uuid> or legacy text keys to this predicate.
      if (column === "id" && !UUID_RE_FK.test(String(key || "").trim())) {
        continue;
      }
      let q = supabase
        .from("patients")
      ...
      if (column === "patient_id" && isMissingColumnError(error, "patient_id")) continue;

      if (error) {
        lastErrorMsg = error?.message || String(error?.code || "");
        console.warn("[SUPABASE] getPatientById non-fatal:", column, key.slice(0, 16), lastErrorMsg);
      }
    }
  }

  if (lastErrorMsg) {
    console.error("[SUPABASE] getPatientById: no row after attempts", rawPid.slice(0, 24), lastErrorMsg);
  }
  return null;
}

async function getPatientByPhone(phone, clinicId = null) {
  if (!supabaseConfigured) return null;
  let q = supabase
    .from("patients")
    .select("*, clinics(id, name, clinic_code)")
  ...
  if (clinicId != null && String(clinicId).trim() !== "") {
    q = q.eq("clinic_id", String(clinicId).trim());
  }
  const { data, error } = await q.maybeSingle();
...
async function getPatientByEmail(email, clinicId = null) {
  if (!supabaseConfigured) return null;
  let q = supabase
    .from("patients")
    .select("*, clinics(id, name, clinic_code)")
...
function patientListUpdatedAtOrderError(error) {
  if (!error) return false;
  const raw = `${error.message || ""} ${error.details || ""} ${error.hint || ""}`;
  const low = raw.toLowerCase();
  if (!low.includes("updated_at")) return false;
  const code = String(error.code || "");
  if (code === "42703" || code === "PGRST204" || code === "PGRST205") return true;
  if (low.includes("does not exist")) return true;
  return false;
...
function patientListAssignedDoctorColumnError(error) {
  if (!error) return false;
  const raw = `${error.message || ""} ${error.details || ""} ${error.hint || ""}`;
  const low = raw.toLowerCase();
  if (!low.includes("assigned_doctor")) return false;
  const code = String(error.code || "");
  if (code === "42703" || code === "PGRST204" || code === "PGRST205") return true;
  if (low.includes("does not exist")) return true;
  return false;
...
function patientListDisplayNameColumnError(error) {
  if (!error) return false;
  const raw = `${error.message || ""} ${error.details || ""} ${error.hint || ""}`.toLowerCase();
  if (!raw.includes("external_name") && !raw.includes("display_name_override")) return false;
  const code = String(error.code || "");
  return code === "42703" || code === "PGRST204" || code === "PGRST205" || raw.includes("does not exist");
}
...
async function getPatientsByClinic(clinicId, { includeLeads = false } = {}) {
  ...
  const selectCols = `
      ...
      primary_doctor_id
    `;
  const run = async (orderByUpdatedAt) => {
    let q = supabase.from("patients").select(selectCols).eq("clinic_id", clinicId);
    // Clinic-scoped rows (Messenger/WhatsApp leads) have clinic_id set — do not hide via is_lead.
    void includeLeads;
    if (orderByUpdatedAt) {
      return q.order("updated_at", { ascending: false }).order("created_at", { ascending: false });
    }
    return q.order("created_at", { ascending: false });
  };
  let { data, error } = await run(true);
  if (error && patientListUpdatedAtOrderError(error)) {
    ({ data, error } = await run(false));
  }
  if (error && patientListIsLeadFilterError(error)) {
    const selectColsNoLead = selectCols.replace(/\s*is_lead,?\s*/i, "");
    let q = supabase.from("patients").select(selectColsNoLead).eq("clinic_id", clinicId);
...
async function getPatientsByClinicPaginated(
  clinicId,
  { page = 1, limit = 20, includeLeads = false, search = "" } = {}
) {
  if (!supabaseConfigured) return { data: [], total: 0 };
  const from = (page - 1) * limit;
  const to   = from + limit - 1;
  const selectColsLegacy = `id, patient_id, name, phone, status, is_lead, created_at, updated_at, primary_doctor_id`;
  const selectColsBase = `${selectColsLegacy}, external_name, display_name_override`;
  const selectColsWithAssigned = `${selectColsBase}, assigned_doctor_id`;
  const searchTerm = String(search || "").trim().replace(/[%_,]/g, " ").slice(0, 80);
  void includeLeads;

  const run = async (orderByUpdatedAt, withAssignedCol, withLeadCol, withDisplayNameCols) => {
    let selectCols = withAssignedCol ? selectColsWithAssigned : selectColsBase;
    if (!withDisplayNameCols) {
      selectCols = withAssignedCol ? `${selectColsLegacy}, assigned_doctor_id` : selectColsLegacy;
    }
    if (!withLeadCol) {
      selectCols = selectCols.replace(/\s*is_lead,?\s*/i, "");
    }
    let q = supabase
      .from("patients")
    ...
    if (searchTerm) {
      ...
      if (withDisplayNameCols) {
        q = q.or(
          `name.ilike.%${esc}%,phone.ilike.%${esc}%,display_name_override.ilike.%${esc}%,external_name.ilike.%${esc}%`,
        );
      } else {
        q = q.or(`name.ilike.%${esc}%,phone.ilike.%${esc}%`);
      }
    }
    if (orderByUpdatedAt) {
  ...
  let withAssignedCol = true;
  let withLeadCol = true;
  let withDisplayNameCols = true;
  let { data, error, count } = await run(true, withAssignedCol, withLeadCol, withDisplayNameCols);
  if (error && patientListDisplayNameColumnError(error)) {
    withDisplayNameCols = false;
    ({ data, error, count } = await run(true, withAssignedCol, withLeadCol, withDisplayNameCols));
  }
  if (error && patientListAssignedDoctorColumnError(error)) {
    withAssignedCol = false;
    ({ data, error, count } = await run(true, withAssignedCol, withLeadCol, withDisplayNameCols));
  }
  if (error && patientListUpdatedAtOrderError(error)) {
    ({ data, error, count } = await run(false, withAssignedCol, withLeadCol, withDisplayNameCols));
  }
  if (error && patientListIsLeadFilterError(error)) {
    withLeadCol = false;
    ({ data, error, count } = await run(true, withAssignedCol, withLeadCol, withDisplayNameCols));
...
/**
 * Raw `referrals` rows scoped like GET /api/admin/referrals (Supabase path), before legacy mapping.
 * Order: getReferralsByClinic → patient id OR → broad patient-clinic fallback.
 */
async function fetchReferralsRawForAdminClinic(clinicId, clinicCode) {
  if (!supabaseConfigured || !clinicId) return [];

  let items = (await getReferralsByClinic(clinicId, clinicCode)) || [];
  ...
  if (items.length === 0) {
    const { data: allReferrals, error: allErr } = await supabase
      .from("referrals")
      .select("*")
      .order("created_at", { ascending: false });
...
module.exports = { 
  ...
  // Patients
  expandPatientIdLookupKeys,
  canonicalPatientIdForTimeline,
  patientFkInClauseKeysFromPatientRow,
  allPatientFkInClauseKeysForClinicTimeline,
  getPatientById,
  getPatientByPhone,
  getPatientByEmail,
  getPatientsByClinic,
