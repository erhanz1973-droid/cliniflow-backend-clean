...
• ONLY when the visitor explicitly asks to register their clinic, add their clinic, or clinic sign-up — NOT when they ask to download the mobile app.
 ...
 * @param {{ preserveAppStoreLinks?: boolean }} [options]
 */
function sanitizeSalesRegistrationUrls(replyText, options = {}) {
  const correct = getCliniflyClinicRegisterUrl();
  let out = String(replyText || "");
  const hasAppStoreLink =
    /apps\.apple\.com/i.test(out) || /play\.google\.com/i.test(out);
  if (options.preserveAppStoreLinks && hasAppStoreLink) {
    return out.trim();
  }
  for (const pattern of BAD_REGISTRATION_URL_PATTERNS) {
    out = out.replace(pattern, correct);
  }
  if (/\bsign-up\b/i.test(out) && !out.includes(correct) && !hasAppStoreLink) {
    out = `${out.trim()}\n\n${correct}`;
  }
  return out.trim();
}
