...
const { sendDirectDoctorPatientMessage } = require("./clinicalGuidanceDirectSend");
  ...
  return resolveInquiryDelegation(
    {
      ...
      primary_responder_type: profileRow.primary_responder_type,
      assigned_doctor_id: profileRow.assigned_doctor_id,
    },
    { clinicPolicy },
  );
}
...
/**
 * Doctor may direct-send when Devral (profile) or assigned/enrolled at clinic without AI profile yet.
 */
async function doctorCanDirectSendToPatient(actor, clinicId, patientId, profile, clinicPolicy) {
  if (profile) {
    const delegation = delegationForProfile(profile, clinicPolicy);
    return {
      allowed: delegation.canSendPatientMessageAsDoctor,
      reason: delegation.canSendPatientMessageAsDoctor ? null : "ai_owns_conversation",
    };
  }

  const doctorId = String(actor.id || "").trim();
  const { data: prow } = await supabase
    ...
    .maybeSingle();
  const atClinic =
    prow?.clinic_id != null && String(prow.clinic_id).trim() === String(clinicId).trim();
  if (!atClinic) {
    return { allowed: false, reason: "patient_not_at_clinic" };
  }
  ...
  const assigned = String(thread?.assigned_doctor_id || "").trim();
  if (assigned && doctorId && assigned === doctorId) {
    return { allowed: true, reason: null };
  }
  if (prow.is_lead === false) {
    return { allowed: true, reason: null };
  }
  return { allowed: false, reason: "devral_required" };
}

/**
 * Shared handler — POST /api/ai/direct-patient-message and /api/doctor/patients/:id/direct-message
 */
async function executeDirectPatientMessage(req, res, deps) {
  const { insertClinicMessage } = deps;
  try {
    ...
    const patientId = String(
      req.params?.patientId || body.patientId || body.patient_id || "",
    ).trim();
    const clinicId = String(body.clinicId || actor.clinicId || "").trim();
    const doctorInput = String(
      body.message || body.text || body.finalText || body.doctorInput || "",
    ).trim();
    ...
    let profile = await findProfile(clinicId, patientId);
    if (!profile) {
      await ensureLeadWorkspaceForClinic(patientId, clinicId, { source: "doctor_direct_send" });
      profile = await findProfile(clinicId, patientId);
    }

    const clinicPolicy = await loadClinicPolicyForClinic(clinicId);
    const sendGate = await doctorCanDirectSendToPatient(actor, clinicId, patientId, profile, clinicPolicy);
    if (!sendGate.allowed && actor.role === "doctor") {
      const msg =
        sendGate.reason === "ai_owns_conversation" || sendGate.reason === "devral_required"
          ? "Önce Devral ile konuşmayı devralın."
          : "Bu hastaya mesaj gönderme yetkiniz yok.";
      return res.status(403).json({
        ok: false,
        error: sendGate.reason || "send_not_allowed",
        message: msg,
      });
    }
    ...
    if (profile) {
      ...
      if (delegation.aiMode === "ESCALATION_REQUIRED" && delegation.aiEscalationRequired) {
        return res.status(403).json({
          ...
          message: "Resolve escalation before sending.",
        });
      }
    }

    const doctorLabel = String(
      req.doctor?.name || req.doctor?.full_name || req.doctor?.display_name || "Doctor",
    ).trim();

    const result = await sendDirectDoctorPatientMessage({
      patientId,
      clinicId,
      doctorInput,
      approvedBy: actor.id,
      profileId: profile?.id || null,
      senderDisplayName: doctorLabel,
      insertClinicMessage,
    });
    ...
    console.error("[direct-patient-message]", e?.message || e);
...
function registerClinicalGuidanceRoutes(app, deps) {
  ...
  /** Static catalog — no PHI; safe without doctor token (mobile may load before Bearer is set). */
  router.get("/intent-tags", (_req, res) => {
  ...
  router.post("/expand-clinical-guidance", auth, async (req, res) => {
    try {
      ...
      if (profile) {
        const delegation = delegationForProfile(profile, clinicPolicy);
        const explicitAiAssist =
          body.explicitAiAssist === true ||
          body.explicit_ai_assist === true ||
          body.allowAiAssist === true;
        if (delegation.doctorOwnsConversation && !explicitAiAssist) {
          return res.status(403).json({
            ok: false,
            error: "direct_send_required",
            message:
              "Doctor takeover: use Direct send for verbatim messages, or enable AI assist explicitly.",
          });
        }
      ...
      const expanded = await expandClinicalGuidance({
        ...
        profileRow: profile,
        conversationSummary: profile?.conversation_summary || null,
        leadData: profile
          ? {
  ...
  router.post("/direct-patient-message", auth, (req, res) => executeDirectPatientMessage(req, res, deps));
        ...
        if (profile) {
          ...
          if (delegation.doctorOwnsConversation && !explicitAiAssist) {
            return res.status(403).json({
              ok: false,
              error: "direct_send_required",
              message: "Doctor takeover: AI rewrite is disabled. Use Direct send.",
            });
          }
        }
      }
      ...
      if (draftId && UUID_RE.test(draftId)) {
        ...
        if (existing.status === "sent") {
          return res.status(409).json({
            ok: false,
            error: "draft_already_sent",
            message: "This draft was already sent. Create a new patient draft.",
          });
        }
  ...
  router.post("/clinical-guidance/:guidanceId/send", auth, async (req, res) => {
    try {
      ...
      if (profile) {
        ...
        if (
          actor.role === "doctor" &&
          delegation.conversationOwner === "ai" &&
          !delegation.canSendPatientMessageAsDoctor
        ) {
          return res.status(403).json({
            ok: false,
            error: "ai_owns_conversation",
            message: "AI is handling this conversation. Use Devral before sending a patient message.",
          });
        }
      }
      ...
      const sendMode = String(body.sendMode || body.send_mode || "ai_assisted")
        .trim()
        .toLowerCase();
...
module.exports = { registerClinicalGuidanceRoutes, executeDirectPatientMessage };
