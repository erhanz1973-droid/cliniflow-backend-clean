...
function normalizeKnowledgeBaseConfig(raw) {
  ...
  const lp = kb.learnedPatterns ?? kb.learned_patterns;
  const learned =
    lp && typeof lp === "object" && !Array.isArray(lp)
      ? {
      ...
      : { version: 1, greetings: [], faqs: [], phrases: [], insights: [] };
  return {
    ...kb,
  ...
  return {
    ...
    communicationPolicy: {
      ...
      aiReplies: {
        ...
        instantDelayMs: 400,
        omnichannelInstantDelayMs: 5000,
        messageBufferPreset: "5s",
        messageBufferDelayMs: 5000,
        firstContactInstantDelayMs: 200,
        fallbackDelaySeconds: 30,
        officeHoursOnlyInstant: false,
      },
      whatsappCollection: {
        requestWhatsappEnabled: true,
        askWhatsappAfterStage: "responded",
        whatsappRequiredForBooking: true,
