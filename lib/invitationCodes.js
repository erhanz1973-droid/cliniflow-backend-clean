...
function isInvitationSchemaMissingError(message) {
  const m = String(message || "").toLowerCase();
  return (
    m.includes("redeemed_invitation_code") ||
    m.includes("invitation_code_redemptions") ||
    m.includes("invitation_redeemed_at")
  );
}

/** Admin-facing copy for redeem failures (TR + EN fallback). */
function invitationRedeemUserMessage(reason, technicalError) {
  const tech = String(technicalError || "").trim();
  if (reason === "clinic_load_failed" && isInvitationSchemaMissingError(tech)) {
    return "Kod sistemi henüz tam kurulmamış. Lütfen Clinifly destek ile iletişime geçin (migration gerekli).";
  }
  const map = {
    code_not_found:
      "Geçersiz kod. CLINIFLY2026, CLINIFLY2016 veya WELCOME60 deneyin (60 gün Premium).",
    code_inactive: "Bu kod devre dışı bırakılmış.",
    code_expired: "Bu kodun süresi dolmuş.",
    code_max_uses_reached: "Bu kod kullanım limitine ulaşmış.",
    clinic_already_redeemed: "Bu klinik zaten bir davet kodu kullandı.",
    clinic_not_found: "Klinik bulunamadı.",
    invalid_params: "Geçersiz istek.",
    supabase_required: "Veritabanı bağlantısı yapılandırılmamış.",
    redemption_insert_failed: "Kod kaydedilemedi. Lütfen tekrar deneyin.",
    clinic_update_failed: "Plan güncellenemedi. Lütfen tekrar deneyin.",
  };
  return map[reason] || tech || reason || "Kod etkinleştirilemedi.";
}
  ...
  app.post("/api/admin/invitation-codes/redeem", requireAdminAuth, async (req, res) => {
    try {
      ...
      if (!result.ok) {
        ...
        const message = invitationRedeemUserMessage(result.reason, result.error);
