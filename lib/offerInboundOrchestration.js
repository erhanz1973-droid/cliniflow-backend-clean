...
let notifyClinicOfferReplyFn = null;

/**
 * @param {{ insertIntoTableWithColumnPruning: typeof insertWithPruningFn, emitOfferNewMessage?: typeof emitOfferNewMessageFn, notifyClinicOfferReply?: typeof notifyClinicOfferReplyFn }} deps
 */
function setupOfferInboundOrchestration(deps) {
  ...
  notifyClinicOfferReplyFn = deps.notifyClinicOfferReply || null;
}
  ...
  return async function offerAwareInsertClinicMessage(params) {
    ...
    if (UUID_RE.test(offerId)) {
      ...
      if (!offerResult?.error) {
        ...
        if (notifyClinicOfferReplyFn && offerResult.data?.id) {
          void notifyClinicOfferReplyFn({
            offerId,
            patientId,
            clinicId,
            messageRow: offerResult.data,
            authorKind,
          }).catch((e) =>
            console.warn("[offerInboundOrchestration] offer push:", e?.message || e),
          );
        }
        return offerResult;
      }
