 ...
 * @param {{ offerId: string, message: string, senderName?: string, clinicId?: string|null, doctorId?: string|null }} params
 */
async function insertClinicReplyToOfferThread(params) {
  ...
  const built = await buildClinicAiOfferMessageInsert({
    offerId,
    message,
  ...
  const senderName = String(built.row.sender_name || "Care Team");
  const roleAttempts = ["doctor", "clinic", "assistant", "system"];
  ...
  return async function offerAwareInsertClinicMessage(params) {
    ...
    if (UUID_RE.test(offerId)) {
      ...
      return insertClinicReplyToOfferThread({
        ...
        clinicId: params.contextClinicId || params.clinicId || null,
