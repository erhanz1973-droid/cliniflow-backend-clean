...
async function resolveOutboundChannelContext(patientId, clinicId) {
  ...
  let channel = normalizeCoordinatorChannel(profile?.primary_channel, "in_app");
  ...
  if (profile?.id && (!deliverExternal || !EXTERNAL_CHANNELS.has(channel))) {
    const { data: recentInbound } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    const recentCh = normalizeCoordinatorChannel(recentInbound?.channel, "");
    if (EXTERNAL_CHANNELS.has(recentCh)) {
      channel = recentCh;
      const { data: idForChannel } = await supabase
        .from("channel_identities")
        .select("id, external_user_id, metadata")
        .eq("clinic_id", clinicId)
        .eq("channel", channel)
        .eq("patient_id", patientId)
        .maybeSingle();
      if (channel === "whatsapp" && idForChannel) {
        const waFromId = String(idForChannel.external_user_id || "").trim();
        const pnid =
          phoneNumberId ||
          String(idForChannel.metadata?.phone_number_id || meta.whatsapp_phone_number_id || "").trim() ||
          null;
        if (waFromId) {
          return {
            channel,
            deliverExternal: Boolean(pnid && waFromId),
            ...
            phoneNumberId: pnid,
            waId: waFromId,
            identityId: idForChannel.id || identity?.id || null,
          };
        }
      }
      if (channel === "messenger") {
...
function createChannelAwareClinicMessageInsert(baseInsert) {
  return async function channelAwareInsert(params) {
    ...
    const runExternalDelivery = async (deliveryChannel) => {
      try {
    ...
    const opChannelRaw =
      params.messageProvenance?.operational_channel ||
      params.messageProvenance?.operationalChannel ||
      null;
    const opChannel = opChannelRaw
      ? normalizeCoordinatorChannel(String(opChannelRaw), "")
      : null;

    let deliverExternal = false;
    let ctx = { channel: "in_app", deliverExternal: false };
    try {
      ctx = await resolveOutboundChannelContext(patientId, clinicId);
      const preferredChannel =
        opChannel && EXTERNAL_CHANNELS.has(opChannel) ? opChannel : ctx.channel;
      ctx = { ...ctx, channel: preferredChannel };
      deliverExternal =
        EXTERNAL_CHANNELS.has(preferredChannel) &&
        (preferredChannel === "whatsapp"
          ? Boolean(ctx.phoneNumberId && ctx.waId)
          : preferredChannel === "messenger"
            ? Boolean(ctx.pageId && ctx.psid)
            : ctx.deliverExternal);
    } catch (_) {
      /* optional */
    }
    ...
    const isAiAuto =
      params.messageProvenance?.message_source === "ai_auto_reply" ||
      params.senderName === "Care Team";

    if (deliverExternal || (isAiAuto && EXTERNAL_CHANNELS.has(ctx.channel))) {
      await runExternalDelivery(ctx.channel);
      if (isAiAuto && lastExternalDelivery && lastExternalDelivery.delivered !== true) {
        await new Promise((r) => setTimeout(r, 400));
        await runExternalDelivery(ctx.channel);
      }
    } else if (isAiAuto) {
      try {
        if (ctx.channel === "whatsapp" && !ctx.deliverExternal) {
          console.warn("[outboundDelivery] whatsapp AI reply not sent externally", {
            patientId: patientId.slice(0, 8),
            clinicId: clinicId.slice(0, 8),
            phoneNumberId: ctx.phoneNumberId || null,
            waId: ctx.waId ? `${String(ctx.waId).slice(0, 8)}…` : null,
            hint: "Ensure channel_identities + lead profile channel_metadata have phone_number_id and wa_id",
          });
        }
      } catch (_) {
        /* optional */
