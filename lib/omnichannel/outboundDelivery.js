...
async function resolveOutboundChannelContext(patientId, clinicId) {
  ...
  const metaRaw = String(meta.messenger_psid_raw || "").trim();
  const fromIdentity = String(identity?.external_user_id || meta.messenger_psid || "").trim();
  const rawPsid =
    [metaRaw, fromIdentity].filter(Boolean).sort((a, b) => b.length - a.length)[0] || null;
