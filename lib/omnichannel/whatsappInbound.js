...
const { AI_MODE, isAiSnoozeActive, hadSnoozeThatExpired } = require("../aiDelegation");
...
async function handleInboundWhatsAppMessage(parsed, connectionRowIn) {
  ...
  const omniPublicId = `omni_whatsapp_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
  const metadata = {
    ...
    mirror_message_id: omniPublicId,
    omni_public_id: omniPublicId,
  };
  ...
  if (identity.profileId) {
    ...
    chResult = await insertChannelMessagesWithChannel({
      ...channelRow,
      direction: "inbound",
    });
    if (chResult?.error) {
      const msg = String(chResult.error?.message || "").toLowerCase();
      if (msg.includes("direction") && (msg.includes("column") || msg.includes("schema"))) {
        chResult = await insertChannelMessagesWithChannel(channelRow);
      }
    }
    if (chResult?.error) {
      console.warn("[whatsappInbound] channel_messages:", chResult.error.message);
    }
  }
  ...
  const mirror = await mirrorOmnichannelInboundToPatientMessages({
    ...
    publicMessageId: omniPublicId,
  });
  if (!mirror.ok && !mirror.skipped) {
    console.warn("[whatsappInbound] patient_messages mirror:", mirror.error || mirror.reason, {
      omniPublicId: omniPublicId.slice(0, 48),
    });
  } else if (mirror.ok && chResult?.data?.id) {
    try {
      const chId = String(chResult.data.id);
      const mirrorPublicId = String(mirror.row?.message_id || omniPublicId).trim();
      const meta = {
        ...metadata,
        mirror_message_id: mirrorPublicId,
        omni_public_id: mirrorPublicId,
        patient_messages_row_id: mirror.row?.id || null,
      };
      await supabase
        .from("ai_coordinator_channel_messages")
      ...
      console.warn("[whatsappInbound] mirror_message_id metadata:", metaErr?.message || metaErr);
    }
  }

  try {
    const { assignInboundLeadFromClinicRouting } = require("../autoAssignRespondingDoctor");
    await assignInboundLeadFromClinicRouting(identity.patientId, clinicId);
  } catch (e) {
    console.warn("[whatsappInbound] lead routing assign:", e?.message || e);
  }

  if (mirror.ok && mirror.row) {
    const { notifyDoctorAfterOmnichannelMirror } = require("../mirrorOmnichannelPatientMessage");
    void notifyDoctorAfterOmnichannelMirror(
      {
        patientId: identity.patientId,
        clinicId,
        sender: "patient",
        ...
        threadId: mirror.threadId || mirror.row?.thread_id || null,
      },
      mirror.row,
    ).catch((e) => {
      console.warn("[whatsappInbound] doctor notify after mirror:", e?.message || e);
    });
  }
...
async function touchLeadProfileFromWhatsApp(patientId, clinicId, message, profileId, channelHints = {}) {
  const { syncPatientNameFromWhatsAppTurn } = require("../patientNameSync");
  const { touchLeadProfileFromInbound } = require("../aiSlaContinuity");
  const { isRoutineDentalChiefComplaint, detectMedicalEmergency } = require("../aiDelegation");
  const { resumeExpiredSnoozeBeforeInboundAi } = require("../aiSnoozeCatchUp");
  ...
  void syncPatientNameFromWhatsAppTurn(patientId, {
    profileName: channelHints.profileName || null,
    message,
  });
  ...
  let { data: leadRow } = await supabase
    .from("ai_coordinator_lead_profiles")
    .select(
      "id, ai_mode, ai_paused, ai_escalation_required, human_takeover_at, operational_intake_flags",
    )
    .eq("id", pid)
    .maybeSingle();

  const snoozeResume = await resumeExpiredSnoozeBeforeInboundAi({
    profileId: pid,
    profileRow: leadRow,
  });
  if (snoozeResume.resumed && snoozeResume.profileRow) {
    leadRow = snoozeResume.profileRow;
    whatsappLog("whatsapp.pipeline.snooze_expired_resumed", { profileId: pid.slice(0, 8) });
  }

  const takeoverMs = leadRow?.human_takeover_at
    ? new Date(String(leadRow.human_takeover_at)).getTime()
    : 0;
  const snoozeActive = isAiSnoozeActive(leadRow || {});
  const snoozeExpired = hadSnoozeThatExpired(leadRow || {});
