...
async function handleInboundWhatsAppMessage(parsed, connectionRowIn) {
  ...
  if (
    afterPatientInboundMessageFn &&
    !connectionAllowsWhatsAppAutoAi(connectionRow) &&
    connectionAiMode(connectionRow) === AI_MODE.AI_DRAFT &&
    connectionRow.id
  ) {
    try {
      ...
      if (cfg.replyMode === REPLY_MODE.INSTANT && cfg.instantEnabled) {
        const healed = await setWhatsAppConnectionAiMode(
          String(connectionRow.id),
          "AI_ACTIVE",
          "inbound_instant_ai_heal",
        );
        if (healed.ok) {
          connectionRow =
            (await getWhatsAppConnectionByPhoneNumberId(phoneNumberId)) || connectionRow;
      ...
      console.warn("[whatsappInbound] instant AI heal:", e?.message || e);
    }
  }

  if (afterPatientInboundMessageFn && connectionAllowsWhatsAppAutoAi(connectionRow)) {
  ...
  } else if (!connectionAllowsWhatsAppAutoAi(connectionRow)) {
    whatsappLog("whatsapp.pipeline.ai_skipped", {
      reason: "connection_ai_mode",
      aiMode: connectionRow.ai_mode || null,
      hint:
        "WhatsApp → Communication Channels: set AI mode to «AI active» (AI_DRAFT only saves drafts, no auto-reply to patient)",
    });
  } else {
