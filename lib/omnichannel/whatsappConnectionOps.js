...
async function enrichConnectionForAdminList(row) {
  ...
  if (String(row.status) === "pending" || isPendingPhoneNumberId(phoneNumberId)) {
    return {
      ...row,
      ...
      health: {
        webhookActive: false,
        webhookStatusLabel: "Awaiting Clinifly setup",
        aiEnabled: false,
        aiStatusLabel: "Not live yet",
        tokenValid: null,
      },
      stats: {},
      clinicPolicy,
...
async function previewWhatsAppConnectionForAdmin(phoneNumberId, opts = {}) {
  ...
  const probe = await probeWhatsAppPhoneNumberId(pid, token, {
    tokenSource,
    matchPhone: opts.matchPhone || opts.requestedPhone || null,
  });
  if (!probe.ok) {
    const discovery = probe.discovery || null;
    const hintParts = [
      probe.message || discovery?.diagnosis || null,
      discovery?.probeIdWasWaba
        ? "You pasted a WABA ID — use Phone Number ID from the list below."
        : null,
    ].filter(Boolean);
    return {
      ok: false,
