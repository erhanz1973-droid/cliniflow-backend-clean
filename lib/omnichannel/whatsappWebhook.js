 ...
 * @param {Buffer|string} [rawBody]
 */
function summarizeWhatsAppWebhookBody(body, rawBody) {
  ...
  const rawBuf = Buffer.isBuffer(rawBody) ? rawBody : Buffer.from(String(rawBody || ""), "utf8");
  let rawBodyPreview = "";
  try {
    rawBodyPreview = rawBuf.slice(0, 400).toString("utf8");
  } catch {
    rawBodyPreview = "";
  }
  return {
 ...
 * @param {Buffer|string} [rawBody]
 */
function safeParseWhatsAppWebhookBody(rawBody) {
  try {
    const body = parseWhatsAppWebhookBody(rawBody);
    return { body, parseError: null };
  } catch (e) {
    return {
      body: { object: null, entry: [], valid: false },
