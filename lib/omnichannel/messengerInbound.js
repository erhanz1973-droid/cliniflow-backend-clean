...
const { getActivePageConnectionByPageId } = require("./metaPageConnections");
 ...
 */
async function touchLeadProfileFromMessenger(patientId, clinicId, message, profileId) {
...
async function processMessagingWebhookEvent(rawEvent) {
  ...
  metaTrace("webhook.event", {
    ...
    psid: parsed.psid ? String(parsed.psid).slice(0, 8) : null,
  ...
  const pageRow = await getActivePageConnectionByPageId(parsed.pageId);
  if (!pageRow?.clinic_id) {
    console.warn("[messengerInbound] unknown page", { pageId: parsed.pageId });
    metaTrace("webhook.event.page_not_connected", {
      pageId: parsed.pageId,
      hint: "Reconnect Page in admin-messenger or check Page ID matches meta_page_connections.page_id",
    });
