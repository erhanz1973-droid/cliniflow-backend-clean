...
async function listActivePagesForDiagnostics() {
  ...
  const { data, error } = await supabase
    ...
    .select("clinic_id, page_id, page_name, webhook_subscribed, status, page_access_token_enc")
    .eq("status", "active");
  if (error) {
  ...
  /** @type {Map<string, { name?: string, clinic_code?: string }>} */
  ...
  if (clinicIds.length) {
    ...
    for (const c of clinics || []) {
      if (c?.id) clinicById.set(String(c.id), c);
    }
  }
  return rows.map((r) => {
    const clinic = clinicById.get(String(r.clinic_id || ""));
    return {
      ...r,
      clinic_name: clinic?.name || null,
      clinic_code: clinic?.clinic_code || null,
    };
  });
}
...
async function runMetaWebhookStartupDiagnostics() {
  ...
  for (const row of pages) {
    ...
    metaTrace("webhook.startup.page_subscribed_apps", {
      pageId: row.page_id,
      pageName: row.page_name,
      clinicId: row.clinic_id ? String(row.clinic_id).slice(0, 8) : null,
      clinicName: row.clinic_name,
      clinicCode: row.clinic_code,
