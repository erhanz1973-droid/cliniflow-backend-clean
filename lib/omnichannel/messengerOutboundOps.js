...
/** Map JS camelCase patch keys → PostgREST snake_case columns. */
const OUTBOUND_PATCH_KEY_MAP = {
  ...
  lastDeliveryAttemptAt: "last_delivery_attempt_at",
  httpStatus: "http_status",
  tokenSource: "token_source",
  recipientPsid: "recipient_psid",
  pageId: "page_id",
 ...
 */
function normalizeOutboundDeliveryPatch(patch) {
  /** @type {Record<string, unknown>} */
  const row = {};
  for (const [key, value] of Object.entries(patch || {})) {
    if (value === undefined) continue;
    const col = OUTBOUND_PATCH_KEY_MAP[key] || key;
    row[col] = value;
  }
...
async function patchOutboundDeliveryRow(id, patch) {
  ...
  const now = new Date().toISOString();
  const row = normalizeOutboundDeliveryPatch(patch);
  row.updated_at = now;
