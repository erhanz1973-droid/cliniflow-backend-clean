...
function isPendingPhoneNumberId(phoneNumberId) {
  return String(phoneNumberId || "").startsWith(PENDING_ID_PREFIX);
}
 ...
 */
async function getWhatsAppConnectionById(connectionId) {
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) {
    console.warn("[whatsappPhoneConnections] getById:", error.message);
    return null;
  }
  return data || null;
}
...
/**
 * Pending onboarding requests (clinic entered WhatsApp number only).
 */
async function listPendingWhatsAppConnections() {
  if (!isSupabaseEnabled()) return [];
  const { data, error } = await supabase
    .from("whatsapp_phone_connections")
    .select(SELECT_ROW)
  ...
  if (error) {
    console.warn("[whatsappPhoneConnections] listPending:", error.message);
    return [];
  }
  const rows = data || [];
  const out = [];
...
/**
 * Clinic submits WhatsApp number — pending until Super Admin completes Meta setup.
 * @param {{ clinicId: string, phoneNumber: string, connectedBy?: string|null }} params
 */
async function createPendingWhatsAppRequest(params) {
  if (!isSupabaseEnabled() || !UUID_RE.test(params.clinicId)) {
    return { ok: false, error: "supabase_or_clinic_invalid" };
  }
  const norm = normalizeWhatsAppPhoneNumber(params.phoneNumber);
  if (!norm.ok) return { ok: false, error: norm.error };

  const existingRows = await listWhatsAppConnectionsForClinic(params.clinicId);
  for (const row of existingRows) {
    if (String(row.status) === "disconnected") continue;
    const rowNorm = normalizeWhatsAppPhoneNumber(row.phone_number || "");
    if (rowNorm.ok && rowNorm.e164 === norm.e164) {
      if (String(row.status) === "pending") {
        return { ok: true, connection: row, alreadyPending: true };
      }
      return { ok: false, error: "phone_already_connected" };
    }
  }

  const nowIso = new Date().toISOString();
  const pendingId = `${PENDING_ID_PREFIX}${crypto.randomUUID().replace(/-/g, "")}`;
  const row = {
    clinic_id: params.clinicId,
    phone_number_id: pendingId,
    phone_number: norm.e164,
    status: "pending",
    is_enabled: false,
    ai_mode: "AI_ACTIVE",
    connected_by: params.connectedBy || "clinic_admin",
    created_at: nowIso,
    updated_at: nowIso,
    metadata: {
      onboarding: "simple_request",
      requestedAt: nowIso,
      requestedPhone: norm.e164,
    },
  };
  ...
  const result = await supabase
    .from("whatsapp_phone_connections")
    .insert(row)
    .select("id, clinic_id, phone_number_id, phone_number, status, created_at")
    .single();
...
/**
 * Super Admin completes Meta setup for a pending request.
 * @param {{
 *   connectionId: string,
 ...
 * }} params
 */
async function activatePendingWhatsAppConnection(params) {
  if (!isSupabaseEnabled()) return { ok: false, error: "supabase_required" };
  const id = String(params.connectionId || "").trim();
  const phoneNumberId = String(params.phoneNumberId || "").trim();
  if (!id || !phoneNumberId || isPendingPhoneNumberId(phoneNumberId)) {
    return { ok: false, error: "invalid_params" };
  }

  const { data: existing, error: fetchErr } = await supabase
    .from("whatsapp_phone_connections")
    .select("id, clinic_id, phone_number_id, phone_number, status, metadata")
    .eq("id", id)
    .maybeSingle();
  ...
  const { data: conflict } = await supabase
    ...
    .maybeSingle();
  if (conflict?.id) {
    return { ok: false, error: "phone_number_id_in_use" };
  }
  ...
  const row = {
    ...
    phone_number: params.phoneNumber || existing.phone_number || null,
    display_name: params.displayName || null,
    waba_id: params.wabaId || null,
    status: "active",
    is_enabled: false,
    updated_at: nowIso,
    connected_by: params.actor || "super_admin",
    metadata: {
      ...prevMeta,
      onboarding: "super_admin_activated",
      activatedAt: nowIso,
      previousPendingId: existing.phone_number_id,
    },
  };
  ...
  await logOmnichannelConnectionAudit({
    ...
    metadata: {
      fromPending: true,
      requestedPhone: existing.phone_number || null,
...
async function setWhatsAppConnectionEnabled(connectionId, enabled, actor) {
  ...
  if (String(existing.status) === "pending") {
    return { ok: false, error: "connection_pending" };
  }
...
async function syncWhatsAppConnectionsForInstantClinicAi(clinicId, actor = "clinic_instant_ai") {
  ...
  const cfg = normalizeAiRepliesConfig((await getClinicAiProfile(clinicId)).communicationPolicy);
