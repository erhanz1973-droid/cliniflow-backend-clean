...
function verifyWebhookChallenge(req) {
  ...
  const checks = {
    ...
    verifyTokenMatches: Boolean(expected && token && token === expected),
    challengePresent: challenge != null && String(challenge) !== "",
  };
  ...
  if (
    ...
    return { ok: true, challenge: String(challenge), checks };
  }
