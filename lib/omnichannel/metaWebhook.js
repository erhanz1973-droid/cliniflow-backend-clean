...
function repairConcatenatedPsid(psid, pageId, recipientId) {
  ...
  if (id.length >= 15) {
    for (const prefixLen of [8, 9, 10, 11, 12, 13, 14, 15]) {
      if (prefixLen >= id.length - 1) continue;
      const prefix = id.slice(0, prefixLen);
      const suffix = id.slice(prefixLen);
      if (prefix.length < 6 || suffix.length < 6) continue;
      if (suffix === page || suffix === recipient) continue;
      if (page && (page.endsWith(suffix) || suffix.endsWith(page))) continue;
      return prefix;
    }
  }
  return id.trim();
