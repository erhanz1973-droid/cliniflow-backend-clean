  ...
  const challenge =
    challengeRaw == null || challengeRaw === ""
      ...
      : typeof challengeRaw === "string"
        ? challengeRaw
        : String(challengeRaw);
  const expectedList = expectedVerifyTokens();
...
function normalizeInboundPsid(senderId, recipientId, pageId) {
  let psid = coerceMetaGraphId(senderId);
  const recipient = coerceMetaGraphId(recipientId);
  const page = String(pageId || "").trim();
  if (!psid) return "";
  if (page && psid === page) return "";
  if (recipient && recipient.length >= 8 && psid.endsWith(recipient) && psid.length > recipient.length) {
    psid = psid.slice(0, psid.length - recipient.length);
  }
  if (page && page.length >= 8 && psid.endsWith(page)) {
    psid = psid.slice(0, psid.length - page.length);
  }
  return psid.trim();
}
