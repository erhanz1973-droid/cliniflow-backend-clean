...
const { metaAppId } = require("./metaConfig");
const { exchangeEmbeddedSignupCode } = require("./metaGraph");
const {
  listWhatsAppConnectionsForClinic,
  listAllActiveWhatsAppConnections,
  createPendingWhatsAppRequest,
...
function registerWhatsAppAdminRoutes(app, deps) {
  ...
  app.get("/api/integrations/whatsapp/status", requireAdminAuth, async (req, res) => {
    try {
      ...
      const pending = connections.filter((c) => String(c.status) === "pending");
      return res.json({
        ok: true,
  ...
  app.post("/api/integrations/whatsapp/onboarding/request", requireAdminAuth, async (req, res) => {
    try {
      const clinicId = String(req.clinicId || "").trim();
      if (!UUID_RE.test(clinicId)) {
        return res.status(400).json({ ok: false, error: "clinic_required" });
      ...
      const phoneNumber = String(
        req.body?.phoneNumber || req.body?.phone_number || req.body?.whatsappNumber || "",
      ).trim();
      if (!phoneNumber) {
        return res.status(400).json({ ok: false, error: "phone_required" });
      }
      const result = await createPendingWhatsAppRequest({
        clinicId,
        phoneNumber,
        connectedBy: req.clinicCode || req.adminEmail || "clinic_admin",
      });
      if (!result.ok) {
        const messages = {
          phone_already_connected: "This WhatsApp number is already connected to your clinic.",
          invalid_phone: "Enter a valid WhatsApp number with country code, e.g. +995599220690.",
          phone_required: "Enter your WhatsApp number.",
        };
        return res.status(400).json({
          ok: false,
          error: result.error,
          message: messages[result.error] || "Could not save your WhatsApp number.",
        });
      }
      return res.json({
        ok: true,
        ...
        message: result.alreadyPending
          ? "Your WhatsApp connection is already pending Clinifly setup."
          : "WhatsApp connection pending — our team will complete Meta setup shortly.",
        status: "pending",
      });
    } catch (e) {
  ...
  app.get("/api/integrations/whatsapp/onboarding", requireAdminAuth, async (req, res) => {
    try {
      ...
      const connections = await listWhatsAppConnectionsForClinic(clinicId);
      const primary =
        connections.find((c) => String(c.status) === "active") || connections[0] || null;
      const hasPending = connections.some((c) => String(c.status) === "pending");
      const hasActive = connections.some((c) => String(c.status) === "active");
      const health = whatsappHealthSnapshot();
      const profile = await getClinicAiProfile(clinicId);
      const steps = {
        connect: hasPending || hasActive,
        pendingApproval: hasPending && !hasActive,
        verifyWebhook: Boolean(primary?.last_webhook_at && String(primary.status) === "active"),
        testMessage: Boolean(primary?.metadata?.lastTest?.at && String(primary.status) === "active"),
        configureAi: profile.isConfigured,
        goLive:
          primary &&
          String(primary.status) === "active" &&
      ...
      return res.json({
        ...
        embeddedSignupAvailable: whatsappEmbeddedSignupAvailable(),
      });
    } catch (e) {
  ...
  app.get("/api/integrations/whatsapp/embedded-signup/config", requireAdminAuth, async (_req, res) => {
    try {
      const available = whatsappEmbeddedSignupAvailable();
      return res.json({
        ok: true,
        available,
        appId: available ? metaAppId() : null,
        configId: available ? whatsappEmbeddedSignupConfigId() : null,
        graphApiVersion: metaGraphApiVersion(),
        hint: available
          ? null
          : "Set META_APP_ID, META_APP_SECRET, and WHATSAPP_EMBEDDED_SIGNUP_CONFIG_ID on Railway (Meta → WhatsApp → Embedded Signup).",
      });
    } catch (e) {
      return res.status(500).json({ ok: false, error: e?.message || "embedded_config_failed" });
    }
  });

  app.post("/api/integrations/whatsapp/embedded-signup/complete", requireAdminAuth, async (req, res) => {
    try {
      if (!whatsappEmbeddedSignupAvailable()) {
        return res.status(503).json({
          ok: false,
          error: "embedded_signup_not_configured",
          message: "WhatsApp Embedded Signup is not configured on the server.",
        });
      }
      const clinicId = String(req.body?.clinicId || req.clinicId || "").trim();
      if (!UUID_RE.test(clinicId)) {
      ...
      if (!code || !phoneNumberId) {
        return res.status(400).json({ ok: false, error: "code_and_phone_number_id_required" });
      }

      let accessToken = String(req.body?.accessToken || req.body?.access_token || "").trim();
      if (!accessToken) {
        const exchanged = await exchangeEmbeddedSignupCode(code);
        accessToken = String(exchanged?.access_token || "").trim();
        if (!accessToken) {
          return res.status(400).json({
            ok: false,
            error: "token_exchange_failed",
            message: "Meta did not return an access token for this signup code.",
          });
        }
      }
      ...
      const result = await upsertWhatsAppPhoneConnection({
        ...
        wabaId: req.body?.wabaId || req.body?.waba_id || null,
        accessToken,
        connectedBy: req.clinicCode || req.adminEmail || "embedded_signup",
        metadata: {
          embeddedSignup: true,
          signupPayload: req.body?.signupPayload || req.body?.signup_payload || null,
        },
      ...
      const graph = await enrichWhatsAppConnectionFromGraph(phoneNumberId, accessToken);
      const profile = graph.probe?.data || {};
      ...
      console.warn("[whatsapp] embedded-signup/complete:", e?.message || e);
      return res.status(500).json({ ok: false, error: e?.message || "embedded_signup_failed" });
    }
  });
