...
/** Lazy load — metaPageConnections imports this module for token validation. */
function metaPageConnections() {
  return require("./metaPageConnections");
}
...
function evaluateFacebookReconnectNeed(ctx) {
  ...
  const pageScopeOk = pageId ? pageTokenGrantedForPageId(tokenDebug, pageId) : false;
  const messengerReady =
    tokenType === "PAGE" &&
    tokenDebug?.isValid === true &&
    scopeOk &&
    (pageScopeOk || (pageId && isPagesReadEngagementProbeError(pageGet)));
...
async function runMessengerGraphDiagnostics(opts = {}) {
  ...
  const mapping = pageId ? await metaPageConnections().lookupPageClinicMapping(pageId) : null;
...
module.exports = {
  ...
  isPagesReadEngagementProbeError,
  pageTokenGrantedForPageId,
  RECONNECT_CTA,
};
