...
function buildFacebookOAuthUrl(state, redirectUri, opts = {}) {
  ...
  // pages_read_engagement: required for Page node access_token (granular Page IDs).
  // business_management: required for /me/accounts when Pages are in Business Manager.
...
function registerMetaIntegrationRoutes(app, deps) {
  ...
  app.get("/api/integrations/meta/oauth/callback", async (req, res) => {
    try {
      ...
      // Steps 4–6: compare, short-/me/accounts fallback, extended fallbacks
