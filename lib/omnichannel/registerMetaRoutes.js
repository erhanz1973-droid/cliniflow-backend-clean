...
function registerMetaMessengerWebhook(app) {
  app.get("/api/integrations/meta/health", (_req, res) => {
    const vt = metaWebhookVerifyToken();
    res.json({
      ok: true,
      messengerWebhookPath: MESSENGER_WEBHOOK_PATH,
      integrationEnabled: metaIntegrationEnabled(),
      verifyTokenConfigured: Boolean(vt),
      verifyTokenLength: vt ? vt.length : 0,
    });
  });
  ...
  app.get(MESSENGER_WEBHOOK_PATH, (req, res) => {
    ...
    metaTrace("webhook.GET.rejected", {
      ...
      hint:
        result.reason === "hub.mode_not_subscribe"
          ? "Browser visit without ?hub.mode=subscribe is expected to fail; Meta Verify and save sends full query string"
          : result.reason === "verify_token_mismatch"
            ? "META_WEBHOOK_VERIFY_TOKEN on Railway must exactly match Verify token in Meta Webhooks UI"
            : result.reason === "META_WEBHOOK_VERIFY_TOKEN_not_set"
...
function buildFacebookOAuthUrl(state, redirectUri, opts = {}) {
  ...
  // Messenger/Page only — do not add deprecated or non-Messenger scopes (e.g. pages_read_engagement).
  url.searchParams.set(
    "scope",
    ["pages_show_list", "pages_messaging", "pages_manage_metadata"].join(","),
  );
  ...
  app.get("/api/integrations/meta/oauth/callback", async (req, res) => {
    try {
      ...
      metaTrace("oauth.callback.token_flow", {
        step: "short_lived_from_code",
        token: tokenHint(userToken),
        graphVersion: metaGraphApiVersion(),
      });

      const shortAuditPreExchange = await auditTokenForPages(
        "short_lived_before_exchange",
        userToken,
      );

      metaTrace("oauth.callback.token_flow", {
        step: "exchange_for_long_lived",
      ...
      metaTrace("oauth.callback.token_flow", {
        step: "audit_long_lived_after_exchange",
        longToken: tokenHint(longToken),
      });

      const pageResult = await resolveOAuthPages(userToken, longToken, {
        shortAuditPreExchange,
      });
      ...
      metaTrace("oauth.callback.pages", {
        ...
        tokenUsed: tokenHint(pageResult.tokenUsed),
        scopeCompare: pageResult.scopeCompare,
        scopes: pageResult.shortAudit?.debug?.scopes || [],
        pages: pages.map((p) => ({
          id: p.id,
          name: p.name,
      ...
      if (!pages.length) {
        ...
        metaTrace("oauth.callback.no_pages", {
          shortPageCount: pageResult.shortAudit?.pages?.length ?? 0,
          longPageCount: pageResult.longAudit?.pages?.length ?? 0,
          shortGranularIds: pageResult.shortAudit?.granularPageIds || [],
          longGranularIds: pageResult.longAudit?.granularPageIds || [],
          scopeCompare: pageResult.scopeCompare,
          scopes: pageResult.longAudit?.debug?.scopes || pageResult.shortAudit?.debug?.scopes,
          hasPagesShowList: (pageResult.shortAudit?.debug?.scopes || []).includes("pages_show_list"),
        });
        return res.redirect(
          302,
    ...
    try {
      ...
      for (const p of pages) {
        ...
        if (result.ok) {
          ...
          failed.push({ pageId, error: result.error || "connect_failed" });
        }
      }
