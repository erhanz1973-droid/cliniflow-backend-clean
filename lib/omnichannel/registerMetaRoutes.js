...
const { normalizePageAiMode } = require("../pageAiMode");
...
function registerMetaIntegrationRoutes(app, deps) {
      ...
      const pagesWithHealth = await Promise.all(
        pages.map(async (p) => {
          ...
          return {
            ...
            status: p.status,
            aiMode: normalizePageAiMode(p.ai_mode),
  ...
  app.patch("/api/integrations/meta/pages/ai-mode", requireAdminAuth, async (req, res) => {
    try {
      const clinicId = String(req.clinicId || "").trim();
      const pageId = String(req.body?.pageId || req.body?.page_id || "").trim();
      const aiMode = normalizePageAiMode(req.body?.aiMode ?? req.body?.ai_mode);
      if (!UUID_RE.test(clinicId) || !pageId) {
        return res.status(400).json({ ok: false, error: "invalid_params" });
      }
      const result = await updatePageConnectionAiMode(clinicId, pageId, aiMode);
      if (!result.ok) {
        const status = result.error === "page_not_connected" ? 404 : 500;
        return res.status(status).json({ ok: false, error: result.error || "update_failed" });
      }
      metaTrace("page_ai_mode.updated", {
        clinicId: clinicId.slice(0, 8),
        pageId,
        aiMode,
      });
      ...
      return res.status(500).json({ ok: false, error: e?.message || "ai_mode_update_failed" });
    }
  });
