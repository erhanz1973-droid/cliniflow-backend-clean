  ...
  app.get("/api/integrations/meta/oauth/callback", async (req, res) => {
    try {
      ...
      try {
        ...
        metaTrace("oauth.callback.long_lived_ok", { userToken: tokenHint(longToken) });
      } catch (e) {
      ...
      const pageResult = await fetchManagedPages(longToken);
      ...
      metaTrace("oauth.callback.pages", {
        ...
        scopes: pageResult.debug?.scopes || [],
        pages: pages.map((p) => ({
          id: p.id,
          name: p.name,
      ...
      if (!pages.length) {
        const scopeHint = (pageResult.debug?.scopes || []).join(",") || "unknown";
        metaTrace("oauth.callback.no_pages", {
          scopes: pageResult.debug?.scopes,
          hasPagesShowList: (pageResult.debug?.scopes || []).includes("pages_show_list"),
        });
        return res.redirect(
          302,
