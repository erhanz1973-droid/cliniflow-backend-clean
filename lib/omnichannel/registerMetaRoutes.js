...
const { runMetaWebhookStartupDiagnostics } = require("./metaWebhookDiagnostics");
const {
...
function registerMetaMessengerWebhook(app) {
  ...
  setImmediate(() => {
    void runMetaWebhookStartupDiagnostics().catch((e) => {
      console.warn("[metaWebhook] startup diagnostics failed:", e?.message || e);
    });
  });
}
...
function registerMetaIntegrationRoutes(app, deps) {
  ...
  app.get("/api/integrations/meta/webhook/diagnostics", requireAdminAuth, async (_req, res) => {
    try {
      const report = await runMetaWebhookStartupDiagnostics();
