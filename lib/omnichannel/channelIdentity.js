...
const { pageAccessTokenFromRow, getActivePageConnectionByPageId } = require("./metaPageConnections");
const {
...
async function resolveMessengerIdentity(params) {
  ...
  if (existingIdentity?.patient_id) {
    ...
    void ensureMessengerPatientNameFromGraph({
      clinicId,
      patientId,
      psid: resolvedPsid || incomingPsid,
      pageId,
      pageConnectionRow: params.pageConnectionRow,
      identityId: existingIdentity.id,
    }).catch((e) =>
      console.warn("[channelIdentity] messenger name sync:", e?.message || e),
    );
    return {
      ok: true,
      patientId,
...
/**
 * Sync Messenger Graph profile name → patients.name + channel_identities.display_name.
 * @param {{
 *   clinicId: string,
 *   patientId: string,
 *   psid?: string|null,
...
async function ensureMessengerPatientNameFromGraph(params) {
  ...
  if (storedDisplay) {
    ...
    if (synced.name) return { name: synced.name, synced: true };
  }

  if (!psid) return { name: null, synced: false };
  ...
  if (!token) return { name: null, synced: false };

  const profile = await fetchMessengerUserProfile(psid, token, pageId);
  const displayName = normalizePatientDisplayName(
    [profile?.first_name, profile?.last_name].filter(Boolean).join(" "),
  );
  if (!displayName) return { name: null, synced: false };
