...
async function findPatientByWhatsAppPhone(waId, clinicId) {
  ...
  const { data: byPhone, error: phoneErr } = await supabase
    .from("patients")
    .select("id, clinic_id, phone, name, first_name, last_name, whatsapp_number")
    .or(orPhone)
    .limit(10);
  ...
  let rows = byPhone || [];
  if (!rows.length) {
    const orWa = variants.map((p) => `whatsapp_number.eq.${p}`).join(",");
    const { data: byWa, error: waErr } = await supabase
      .from("patients")
      .select("id, clinic_id, phone, name, first_name, last_name, whatsapp_number")
      .or(orWa)
      .limit(10);
    if (waErr) {
      if (!isMissingColumnError(waErr)) {
        console.warn("[channelIdentity] find by whatsapp_number:", waErr.message);
      }
    } else {
      rows = byWa || [];
    }
...
async function resolveWhatsAppIdentity(params) {
  ...
  if (existingPatient?.id) {
    ...
    if (!existingPatient.whatsapp_number) patch.whatsapp_number = phoneE164;
