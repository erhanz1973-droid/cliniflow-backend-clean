...
function isOrthodonticTreatmentIntent(message, leadData) {
  ...
  const clinicalContinuation =
    TREATMENT_WANT_RE.test(t) ||
    /\b(tedavi|tedavisi|bilgi|information|foto|photo|görüntü|goruntu)\b/i.test(t) ||
    (hasOrthoWord && t.length >= 14);

  // Stale lead orthodontics must not hijack unrelated turns (travel, hotel, booking, etc.).
  if (leadOrtho && !hasOrthoWord) {
    return clinicalContinuation;
  }
