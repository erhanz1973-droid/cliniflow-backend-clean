...
const { AI_MODE, isAiSnoozeActive } = require("./aiDelegation");
const {
...
function deriveResponderMode(row) {
  ...
  const paused = row.ai_paused === true || isAiSnoozeActive(row);
  const escalated =
 ...
 * @param {string} [lang]
 */
function attachResponderToLead(lead, lang = "en") {
  ...
  const responderLocalized = opsT(L, `responder.${responderMode}`);
  const handlingLocalized = opsT(L, `handling.${handlingState}`);

  return {
    ...lead,
    responderMode,
    responderModeLabel: responderLocalized.startsWith("ops.responder.")
      ? RESPONDER_LABELS[responderMode] || responderMode
      : responderLocalized,
    primaryResponder: primary,
    primaryResponderLabel: localizePrimaryResponderLabel(primary, L),
    handlingState,
    handlingStateLabel: handlingLocalized.startsWith("ops.handling.")
      ? HANDLING_LABELS[handlingState] || handlingState
      : handlingLocalized,
    isAiHandling:
