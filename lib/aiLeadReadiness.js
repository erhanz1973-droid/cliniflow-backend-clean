...
function computeLeadReadiness(input) {
  ...
  const tier = String(flags.treatmentComplexity || "medium");

  const hasDocs = docs.length > 0;
  if (tier === "low") {
    checks.push({
      key: "documents",
      label: "Documents (optional for routine visits)",
      met: true,
      weight: 18,
    });
  } else {
  ...
  if (tier === "low" || !flags.missingXray) {
    checks.push({
      key: "imaging",
      label:
        tier === "low"
          ? "Imaging (not required for routine visits)"
          : "Imaging present when commonly needed",
      met: true,
      weight: 15,
    });
  } else {
  ...
  if (tier === "low" || !cosmeticTags || !flags.missingSmilePhotos) {
