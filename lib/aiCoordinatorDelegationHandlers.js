 ...
 */
function attachDelegationToLead(lead, clinicPolicy, lang = "en") {
  ...
  return attachResponderToLead(merged, lang);
}
...
async function patchInquiryDelegation(params) {
  ...
  const explicitAssignedDoctorId = String(
    body.assignedDoctorId || body.assigned_doctor_id || "",
  ).trim();
  ...
  } else if (action === "snooze_ai" || body.snoozeAi === true || body.snooze_ai === true) {
    /** Temporary mute — flags only; do not set ai_paused / HUMAN_ONLY (resume works when timer ends). */
    if (params.actorRole === "doctor") {
      primaryResponderType = PRIMARY_RESPONDER.DOCTOR;
    }
    snoozeMinutes = Math.min(
      60,
      Math.max(1, parseInt(String(body.snoozeAiMinutes ?? body.snooze_ai_minutes ?? 5), 10) || 5),
    );
  } else if (action === "resume_ai") {
    targetResponderMode = RESPONDER_MODE.AI_ACTIVE;
    primaryResponderType = PRIMARY_RESPONDER.AI_COORDINATOR;
  } else if (action === "set_hybrid") {
    if (params.actorRole === "doctor") {
      return {
        ok: false,
        status: 400,
        error: "hybrid_disabled",
        message: "Single-owner mode: use takeOver (Devral) or resumeAi (AI devam etsin).",
      };
    }
  ...
  const hasDelegationIntent =
    ...
    body.snoozeAi === true ||
    body.snooze_ai === true ||
    body.snoozeAiMinutes != null ||
    body.snooze_ai_minutes != null ||
    body.requireDoctorReview === true ||
    body.ai_escalation_required === true;

  if (primaryResponderType && !hasDelegationIntent) {
    const allowedPrimary = new Set(Object.values(PRIMARY_RESPONDER));
    if (!allowedPrimary.has(primaryResponderType)) {
      return {
        ok: false,
        status: 400,
        error: "invalid_primary_responder",
        message: "primaryResponderType must be ai_coordinator, doctor, or shared_queue",
      };
    }
    const { data: current, error: curErr } = await supabase
      .from("ai_coordinator_lead_profiles")
      .select(
        "id, ai_mode, ai_paused, ai_escalation_required, responder_mode, coordination_mode, primary_responder_type",
      )
      .eq("id", profileId)
      .eq("clinic_id", clinicId)
      .maybeSingle();
    if (curErr) {
      return { ok: false, status: 500, error: "query_failed", message: curErr.message };
    }
    if (!current) {
      return { ok: false, status: 404, error: "not_found" };
    }
    targetResponderMode =
      String(current.responder_mode || "").toUpperCase() || deriveResponderMode(current);
  }
  ...
  if (targetResponderMode) {
    ...
    if (action === "take_over" || action === "takeover") {
      timelineEvents.push({
        ...
        meta: {
          ...
          conversationOwner: params.actorRole === "doctor" ? "doctor" : "human",
          label:
            params.actorRole === "doctor"
              ? "Doctor took over conversation"
              : "Human took over conversation",
        },
      });
    } else if (body.pauseAi === true || body.ai_paused === true || action === "pause_ai") {
      timelineEvents.push({
        eventType: "ai_paused",
        meta: { actorLabel, conversationOwner: "doctor" },
      });
    ...
    /** Snooze-only patch — built below after aiMode branch (no responderMode / aiMode required). */
  } else {
    let aiMode = null;
    ...
    if (!aiMode) {
      return {
        ...
        error: "invalid_mode",
        message:
          "Provide responderMode, action (takeOver|pauseAi|resumeAi|snoozeAi), aiMode, uiPreset, pauseAi, or requireDoctorReview",
      };
    }
  ...
  if (snoozeMinutes != null && !patch) {
    patch = { updated_at: nowIso };
    if (params.actorRole === "doctor" && params.actorId) {
      patch.assigned_doctor_id = params.actorId;
    }
    if (!timelineEvents.some((ev) => ev.eventType === "ai_snoozed")) {
      timelineEvents.push({
        eventType: "ai_snoozed",
        meta: {
          actorLabel,
          minutes: snoozeMinutes,
          conversationOwner: "doctor",
          temporary: true,
        },
      });
    }
  ...
  /** Snooze assigns responding doctor — thread + inbox must stay visible (not only after full Devral). */
  if (snoozeMinutes != null && patch?.assigned_doctor_id && UUID_RE.test(String(patch.assigned_doctor_id))) {
    const { data: profRow } = await supabase
      ...
      .eq("clinic_id", clinicId)
      .maybeSingle();
    if (profRow?.patient_id) {
      const { syncPatientLeadThreadAssignedDoctor } = require("./doctorLeadThreadSync");
      void syncPatientLeadThreadAssignedDoctor(
        String(profRow.patient_id),
        clinicId,
        String(patch.assigned_doctor_id),
      ).catch((e) =>
        console.warn("[patchInquiryDelegation] snooze thread sync:", e?.message || e),
      );
    }
  }

  if (primaryResponderType && patch) {
    patch.primary_responder_type = primaryResponderType;
  }

  if (!patch && UUID_RE.test(explicitAssignedDoctorId)) {
    patch = { assigned_doctor_id: explicitAssignedDoctorId, updated_at: nowIso };
  }
  ...
  if (!patch) {
    return {
      ...
      error: "invalid_mode",
      message:
        "Provide responderMode, action, aiMode, uiPreset, assignedDoctorId, pauseAi, or requireDoctorReview",
    };
  }

  if (UUID_RE.test(explicitAssignedDoctorId)) {
    patch.assigned_doctor_id = explicitAssignedDoctorId;
  }

  const needsFlagsMerge = snoozeMinutes != null || action === "resume_ai" || clearEscalation;
  if (needsFlagsMerge) {
    const { data: flagRow, error: flagErr } = await supabase
      ...
      .maybeSingle();
    if (flagErr) {
      return { ok: false, status: 500, error: "query_failed", message: flagErr.message };
    }
    const prevFlags =
    ...
    if (snoozeMinutes != null) {
      const untilIso = new Date(Date.now() + snoozeMinutes * 60 * 1000).toISOString();
      nextFlags.ai_snoozed_until = untilIso;
      nextFlags.ai_snooze_started_at = nowIso;
      nextFlags.ai_snooze_minutes = snoozeMinutes;
      nextFlags.ai_snooze_catchup_done = false;
      delete nextFlags.ai_snooze_catchup_at;
      if (params.actorId) nextFlags.ai_snoozed_by_doctor_id = String(params.actorId);
      timelineEvents.forEach((ev) => {
        if (ev.eventType === "ai_snoozed") {
          ev.meta = { ...(ev.meta || {}), until: untilIso, minutes: snoozeMinutes };
        }
      });
    }
    if (action === "resume_ai" || clearEscalation) {
      delete nextFlags.ai_snoozed_until;
      delete nextFlags.ai_snooze_started_at;
      delete nextFlags.ai_snooze_minutes;
      delete nextFlags.ai_snoozed_by_doctor_id;
      nextFlags.ai_snooze_catchup_done = false;
    }
    patch.operational_intake_flags = nextFlags;
  }
  ...
  const { data, error } = await supabase
    ...
    .eq("clinic_id", clinicId)
    .select(
      "id, coordination_mode, ai_mode, ai_paused, ai_escalation_required, human_takeover_at, assigned_coordinator_id, assigned_doctor_id, escalation_flags, responder_mode, primary_responder_type, operational_intake_flags",
    )
    .maybeSingle();
  ...
  const assignedDoctorForThread = data?.assigned_doctor_id
    ? String(data.assigned_doctor_id).trim()
    : "";
  if (assignedDoctorForThread && UUID_RE.test(assignedDoctorForThread)) {
    const { data: profRow } = await supabase
      .from("ai_coordinator_lead_profiles")
      .select("patient_id")
      .eq("id", profileId)
    ...
    if (profRow?.patient_id) {
      ...
      void syncPatientLeadThreadAssignedDoctor(
        String(profRow.patient_id),
        clinicId,
        assignedDoctorForThread,
      ).catch((e) =>
        console.warn("[patchInquiryDelegation] thread sync:", e?.message || e),
      );
    }
  }
  ...
  return {
    ...
    aiSnoozeActive: delegation.aiSnoozeActive,
    aiSnoozedUntil: delegation.aiSnoozedUntil,
