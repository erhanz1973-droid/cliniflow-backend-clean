...
function resolvesPendingConfirmation(message, durableState) {
  ...
  const action = durableState.awaitingAction || durableState.pending_action;
  if (action === BOOKING_PENDING_ACTIONS.CONFIRM_BOOKING) return true;
  if (String(durableState.stage || "") === "awaiting_slot_confirm") return true;
  if (durableState.selectedSlot?.startAt && durableState.appointmentOfferPending === true) {
    return true;
  }
  return false;
}
