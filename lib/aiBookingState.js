...
const BOOKING_AUDIT_EVENTS = Object.freeze({
  ...
  BOOKING_RESCHEDULE_STARTED: "BOOKING_RESCHEDULE_STARTED",
  BOOKING_RESCHEDULE_CONFIRMED: "BOOKING_RESCHEDULE_CONFIRMED",
  BOOKING_RESCHEDULE_COMPLETED: "BOOKING_RESCHEDULE_COMPLETED",
});
...
/** Pure affirmatives — must not be treated as rejection/alternate scheduling. */
const BOOKING_CONFIRM_ONLY_RE =
  /^(evet|tamam|olur|onayl\w*|onay|yes|ok|okay|sure|kabul|peki|confirm|confirmed)[\s!.?]*$/i;

/**
 * Rejection, alternate-day/time requests, exclusions — must block confirmation and auto-book.
 * @param {string} message
 */
function normalizeSchedulingIntentText(message) {
 ...
 * @param {string} message
 */
function patientHasNegativeSchedulingIntent(message) {
  const raw = String(message || "").trim();
  if (!raw || raw.length > 400) return false;
  if (BOOKING_CONFIRM_ONLY_RE.test(raw)) return false;

  const { isClinicServicesCatalogQuestion } = require("./conversationalTimeParse");
  if (isClinicServicesCatalogQuestion(raw)) return false;

  const t = normalizeSchedulingIntentText(raw);

  if (
    /\b(baska\s+gun|baska\s+saat|baska\s+bir\s+gun|baska\s+bir\s+saat)\b/.test(t)
  ) {
    return true;
  }
  if (/\b(farkli\s+gun|farkli\s+saat|alternatif)\b/.test(t)) {
    return true;
  }
  if (/\b(baska|farkli)\b/.test(t) && /\b(gun|saat|tarih|zaman|gunu)\b/.test(t)) {
    return true;
  }
  if (/\b(gun|saat|tarih|zaman)\b/.test(t) && /\b(baska|farkli)\b/.test(t)) {
    return true;
  }

  if (
    /\b(istemiyorum|istemedim|istemem|istemiyor|istemiyoruz|istemiyorlar|istemiyorsunuz)\b/.test(t)
  ) {
    return true;
  }
  if (/\b(olmaz|olmasin|uygun degil|musait degil|yer yok|dolu|mesgul)\b/.test(t)) {
    return true;
  }
  if (/\b(harici|haric|disinda|haric|except|excluding)\b/.test(t)) {
    if (/\b(hizmet\w*|tedavi\w*|service\w*|treatment\w*|prosedur\w*)\b/.test(t)) {
      return false;
    }
    return true;
  }
  if (/\b(degistir|ertele|iptal|yeniden planla|reschedule|postpone|cancel)\b/.test(t)) {
    return true;
  }

  if (
    /\b(do not want|don't want|dont want|do not need|cannot do|can't do|won't work|doesn't work|does not work|not available|not suitable|not monday|not tuesday|not wednesday|not thursday|not friday|not saturday|not sunday)\b/.test(
      t,
    )
  ) {
    return true;
  }
  if (/\b(different day|another day|other day|different time|another time|other time)\b/.test(t)) {
    return true;
  }
  if (/\b(requesting another|want another|need another|another day other than)\b/.test(t)) {
    return true;
  }
  if (/\b(i do not want|i don't want)\b/.test(t)) {
    return true;
  }

  const weekdayReject =
    /\b(pazartesi|sali|carsamba|persembe|cuma|cumartesi|pazar|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/;
  const rejectCue =
    /\b(harici|haric|disinda|except|istemiyorum|istemedim|olmaz|degil|not|do not want|don't want|dont want|uygun degil)\b/;
  if (weekdayReject.test(t) && rejectCue.test(t)) {
    return true;
  }

  if (/\bnot\b/.test(t) && weekdayReject.test(t)) {
    return true;
  }
...
/**
 * Alias — blocks all confirmation routing when true.
 * @param {string} message
 */
function patientBlocksBookingConfirmation(message) {
  return patientHasNegativeSchedulingIntent(message);
}
...
function isBookingConfirmationYesMessage(message, ctx = {}) {
  ...
  if (patientBlocksBookingConfirmation(t)) return false;
  ...
  if (schedulingOffered && BOOKING_CONFIRM_YES_RE.test(t)) return true;
  if (ctx.pendingConfirmation === true && BOOKING_CONFIRM_YES_RE.test(t)) return true;
...
/**
 * Confirming a new slot while a prior booking is still on file (reschedule / change time).
 * @param {string} message
 * @param {Record<string, unknown>|null|undefined} flags
 * @param {Array<{ role?: string, text?: string }>} [recentTurns]
 */
function isPendingBookingChangeConfirmation(message, flags, recentTurns = []) {
  if (patientBlocksBookingConfirmation(message)) return false;
  const durable = readDurableBookingState(flags);
  if (String(durable.stage || "") !== "awaiting_slot_confirm") return false;
  if (!durable.selectedSlot?.startAt) return false;
 ...
 * @param {Array<{ role?: string, text?: string }>} [recentTurns]
 */
function isPostBookingStaleActionMessage(message, flags = null, recentTurns = []) {
  const t = String(message || "").trim();
  if (!t) return false;
  if (flags && isPendingBookingChangeConfirmation(message, flags, recentTurns)) {
    return false;
  }
  if (flags && isPendingRescheduleConfirmation(message, flags, recentTurns)) {
    return false;
  }
  const { isSocialAcknowledgmentMessage } = require("./conversationRepetitionMemory");
  if (isSocialAcknowledgmentMessage(t)) return false;
  if (
    isBookingConfirmationYesMessage(t, {
      recentTurns,
      pendingConfirmation: true,
      schedulingOfferPending: coordinatorRecentlyOfferedScheduling(recentTurns),
    })
  ) {
    return true;
  }
...
/**
 * Reschedule confirmation pending (distinct from a brand-new booking).
 * @param {string} message
 * @param {Record<string, unknown>|null|undefined} flags
 * @param {Array<{ role?: string, text?: string }>} [recentTurns]
 */
function isPendingRescheduleConfirmation(message, flags, recentTurns = []) {
  const durable = readDurableBookingState(flags);
  if (durable.rescheduleMode !== true) return false;
  return isPendingBookingChangeConfirmation(message, flags, recentTurns);
}
...
/**
 * Patient-facing label from canonical booking — always derive from startAt + clinic TZ.
 * @param {Record<string, unknown>|null|undefined} canonical
 * @param {string} [lang]
 * @param {string} [timezone]
 */
function resolveCanonicalWhenLabel(canonical, lang = "tr", timezone = DEFAULT_BOOKING_CLINIC_TZ) {
  const startAt = canonical?.startAt ? String(canonical.startAt).trim() : "";
  if (startAt) {
    const { formatAppointmentDisplayWithTimezone } = require("./bookingTimezoneIntegrity");
    return formatAppointmentDisplayWithTimezone(startAt, lang, timezone);
  }
  if (canonical?.date && canonical?.time) {
    return `${canonical.date} ${String(canonical.time).slice(0, 5)}`;
  }
  return canonical?.label ? String(canonical.label) : startAt || "";
}
...
function buildCanonicalBookingFromAppointment(appt, flags = {}) {
  ...
  const tz =
    String(appt.timezone || flags?.clinicTimezone || DEFAULT_BOOKING_CLINIC_TZ).trim() ||
    DEFAULT_BOOKING_CLINIC_TZ;
  const locale = String(flags?.locale || "tr").slice(0, 5);
  let date = null;
  let time = null;
  let label = null;
  try {
    ...
    const { formatAppointmentDisplayWithTimezone } = require("./bookingTimezoneIntegrity");
    label = formatAppointmentDisplayWithTimezone(startAt, locale, tz);
  } catch {
    /* non-fatal */
  }
  return {
...
function hasCompletedCanonicalBooking(flags) {
  const f = flags && typeof flags === "object" ? flags : {};
  const activeStart = f.activeAppointment?.startAt;
  const abStage = String(f.aiBooking?.stage || "").toLowerCase();
  if (abStage === "expired") return false;
  if (activeStart) {
    const activeSt = String(f.activeAppointment?.status || "scheduled").toLowerCase();
    if (activeSt !== "cancelled" && activeSt !== "canceled") return true;
  }
  ...
  return TERMINAL_COMPLETED_STAGES.has(abStage) || !!activeStart;
}
...
function buildCanonicalStatusReply(lang, canonical) {
  ...
  const tz = String(canonical?.timezone || DEFAULT_BOOKING_CLINIC_TZ).trim() || DEFAULT_BOOKING_CLINIC_TZ;
  const when = resolveCanonicalWhenLabel(canonical, key, tz);
...
function buildClosedBookingPatch(params) {
  return {
    ...
    expiresAt: null,
    rescheduleMode: params.rescheduleMode === true ? true : false,
    rescheduleFromAppointmentId: params.rescheduleFromAppointmentId || null,
    rescheduleFromSlot: params.rescheduleFromSlot || null,
  };
}

/** Strip abandoned new-booking proposal fields before entering reschedule workflow. */
function buildRescheduleIsolationPatch() {
  return {
    offeredSlots: [],
    slotListId: null,
    selectedSlot: null,
 ...
 */
function appointmentStartAtSameInstant(a, b) {
...
/**
 * In-progress aiBooking proposal disagrees with activeAppointment (stale slot list vs booked appt).
 * @param {Record<string, unknown>|null|undefined} flags
 */
function hasStaleBookingProposalVsActiveAppointment(flags) {
  const f = flags && typeof flags === "object" ? flags : {};
  const appt = f.activeAppointment && typeof f.activeAppointment === "object" ? f.activeAppointment : null;
  const activeStart = appt?.startAt ? String(appt.startAt).trim() : "";
  if (!activeStart) return false;

  const d = readDurableBookingState(f);
  if (d.rescheduleMode === true) return false;

  const stage = String(d.stage || "idle").toLowerCase();
  if (stage === "idle" || TERMINAL_COMPLETED_STAGES.has(stage)) return false;

  const pendingStart = d.selectedSlot?.startAt ? String(d.selectedSlot.startAt).trim() : "";
  if (pendingStart && !appointmentStartAtSameInstant(pendingStart, activeStart)) {
    return true;
  }
  if (Array.isArray(d.offeredSlots) && d.offeredSlots.length > 0 && IN_PROGRESS_STAGES.has(stage)) {
    return true;
  }
  if (IN_PROGRESS_STAGES.has(stage) && d.bookingActive === true) {
    return true;
  }
  return false;
}
 ...
 */
function logBookingWorkflowState(label, flags, extra = {}) {
  const f = flags && typeof flags === "object" ? flags : {};
  const d = readDurableBookingState(f);
  const workflowType =
    d.rescheduleMode === true ? "reschedule" : d.bookingActive ? "booking" : "idle";
  console.log("[booking.workflow]", {
    label: String(label || "state"),
    workflow_type: workflowType,
    bookingActive: d.bookingActive === true,
    rescheduleMode: d.rescheduleMode === true,
    pendingAction: d.pendingAction || d.awaitingAction || null,
    stage: d.stage || null,
    selectedSlot: d.selectedSlot?.startAt || d.selectedSlot?.label || null,
    slotListId: d.slotListId || null,
    activeAppointment: f.activeAppointment?.startAt || null,
    ...extra,
  });
}
...
function mergeAiBookingPatch(prevAb, patch) {
  ...
  let rescheduleMode = p.rescheduleMode !== undefined ? p.rescheduleMode === true : prev.rescheduleMode === true;
  let rescheduleFromSlot =
    p.rescheduleFromSlot !== undefined ? p.rescheduleFromSlot : prev.rescheduleFromSlot || null;
  let rescheduleFromAppointmentId =
    p.rescheduleFromAppointmentId !== undefined
      ? p.rescheduleFromAppointmentId
      : prev.rescheduleFromAppointmentId || null;

  if (
    selectedSlot?.startAt &&
    rescheduleFromSlot?.startAt &&
    !appointmentStartAtSameInstant(selectedSlot.startAt, rescheduleFromSlot.startAt) &&
    rescheduleMode !== true &&
    p.rescheduleMode !== true
  ) {
...
function validateBookingGuardian(params) {
  ...
  if (Math.abs(expectedTs - actualTs) > 120000) {
    ...
    logBookingTimeMismatchBlocked({
      ...
      selected_slot: slot.startAt,
      booking_payload: startAt,
      calendar_write: null,
      delta_ms: mismatch.deltaMs,
    });
    return mismatch;
  }

  const tz = String(params.timezone || DEFAULT_BOOKING_CLINIC_TZ).trim() || DEFAULT_BOOKING_CLINIC_TZ;
  try {
