...
function looksClearlyEnglish(text) {
  ...
  if (!t || hasCyrillic(t) || hasGeorgianScript(t) || hasTurkishDiacritics(t)) return false;
  const enHits = countMarkerHits(t, EN_MARKERS);
  if (enHits >= 2) return true;
  if (enHits >= 1 && /\b(how|what|where|when|much|many|your|the|is|are|at|our|clinic|cost|price)\b/i.test(t)) {
    return true;
  }
  if (
    t.length >= 24 &&
...
function resolveConversationLanguage(params) {
  ...
  if (!lockedPrimary) {
    ...
    if (
      ...
      (detected.cyrillic || detected.georgianScript) &&
      ...
      (detected.cyrillic || detected.georgianScript || detected.messageLength >= SWITCH_MIN_CHARS)
    ) {
      conversationLanguage = detectedCode;
      languageSwitched = true;
    } else if (
...
function buildConversationLanguagePromptBlock(state) {
  ...
  const lines = [
    ...
    state.policy?.stickySessionLanguage
      ? `* This conversation started in ${langName}. Keep EVERY reply in ${langName} unless the patient clearly writes a long message in another language.`
      : null,
    state.phoneOnly
      ? `* Patient's latest message is ONLY a phone/WhatsApp number — reply in ${langName}; do NOT discuss your language skills.`
