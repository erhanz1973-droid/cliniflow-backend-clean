 ...
 * @param {{ expectedLang?: string|null, patientMessage?: string, logLabel?: string }} [opts]
 */
function enforcePatientReplyLanguage(replyText, opts = {}) {
  ...
  let expected = normalizeLangCode(opts.expectedLang);
  ...
  if (!expected || expected === "en") {
    if (isKnownEnglishCoordinatorFallback(reply) && patientDetect?.code && patientDetect.code !== "en") {
      expected = patientDetect.code;
    } else {
      return reply;
    }
