...
async function bumpDoctorUnreadForOmnichannelInbound(patientId, clinicId) {
  ...
  let doctorId = null;
  const canonical = await getCanonicalThread(pid, cid, {
    source: "unread_badge",
    repairClinic: false,
    ensureProfile: false,
  });
  const assigned = canonical.thread?.assigned_doctor_id;
  if (assigned && UUID_RE.test(String(assigned))) {
    doctorId = String(assigned).trim();
  }
  if (!doctorId) {
