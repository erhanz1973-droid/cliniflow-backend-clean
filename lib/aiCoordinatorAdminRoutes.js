...
const PROFILE_SELECT = `
  ...
  assigned_coordinator_id, human_takeover_at,
...
function registerAiCoordinatorAdminRoutes(app, deps) {
  ...
  router.get("/ai-leads", requireAdminAuth, async (req, res) => {
    try {
      ...
      const limit = Math.min(120, Math.max(1, parseInt(String(req.query.limit || "80"), 10) || 80));
  ...
  router.get("/ai-leads/:profileId", requireAdminAuth, async (req, res) => {
    try {
      ...
      const lead = enrichLeadRow(profile, mapProfileRow);
      ...
      const intakeFlags = lead.operationalIntakeFlags || {};
  ...
  router.patch("/ai-leads/:profileId/coordination", requireAdminAuth, async (req, res) => {
    try {
      const clinicId = getClinicId(req);
      const profileId = String(req.params.profileId || "").trim();
      const mode = String(req.body?.mode ?? req.body?.coordinationMode ?? "").trim();

      if (!UUID_RE.test(clinicId) || !UUID_RE.test(profileId)) {
        return res.status(400).json({ ok: false, error: "invalid_id" });
      }
      if (mode !== COORDINATION_AI && mode !== COORDINATION_HUMAN) {
        return res.status(400).json({
          ok: false,
          error: "invalid_mode",
          message: "mode must be ai_active or human_active",
        });
      }
      if (!isSupabaseEnabled()) {
        return res.status(503).json({ ok: false, error: "supabase_required" });
      ...
      const { data, error } = await supabase
        ...
        .eq("id", profileId)
        .eq("clinic_id", clinicId)
        .select("id, coordination_mode, human_takeover_at")
        .maybeSingle();
      ...
      const eventType = mode === COORDINATION_HUMAN ? "human_takeover" : "coordination_change";
      await insertTimelineEvent({
        profileId,
        eventType,
        eventMetadata: { mode, adminId },
      });
      ...
      return res.json({
        ...
        aiDisabled: data.coordination_mode === COORDINATION_HUMAN,
      });
    } catch (e) {
