...
function registerAiCoordinatorAdminRoutes(app, deps) {
  ...
  router.get("/ai-leads", requireAdminAuth, async (req, res) => {
    try {
      ...
      try {
        ...
        let qb = supabase.from("ai_coordinator_lead_profiles").select(PROFILE_SELECT).eq("clinic_id", clinicId);
        ...
        const { data: fallback, error } = await qb;
        if (error) {
          return res.status(500).json({ ok: false, error: "query_failed", message: error.message });
        }
        data = await attachPatientNamesToProfileRows(fallback || []);
        queueMeta = { fallback: true };
      }
  ...
  router.get("/ai-leads/:profileId", requireAdminAuth, async (req, res) => {
    try {
      ...
      const { data: profileFresh } = await supabase
        .from("ai_coordinator_lead_profiles")
        .select(PROFILE_SELECT)
        .eq("id", profileId)
        .eq("clinic_id", clinicId)
