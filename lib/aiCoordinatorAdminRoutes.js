 ...
 */
function attachDelegation(lead) {
...
function registerAiCoordinatorAdminRoutes(app, deps) {
  ...
  router.get("/ai-leads", requireAdminAuth, async (req, res) => {
    try {
      ...
      leads = leads.map((L) => {
        ...
        return attachDelegation(enriched);
      });
  ...
  router.patch("/ai-leads/:profileId/coordination", requireAdminAuth, async (req, res) => {
    try {
      const clinicId = getClinicId(req);
      const profileId = String(req.params.profileId || "").trim();
      const aiModeRaw = req.body?.aiMode ?? req.body?.ai_mode ?? null;
      const modeLegacy = String(req.body?.mode ?? req.body?.coordinationMode ?? "").trim();
      ...
      let aiMode = null;
      if (aiModeRaw != null && String(aiModeRaw).trim()) {
        aiMode = normalizeAiMode(aiModeRaw);
      } else if (modeLegacy === COORDINATION_AI || modeLegacy === COORDINATION_HUMAN) {
        aiMode =
          modeLegacy === COORDINATION_HUMAN ? AI_MODE.HUMAN_ONLY : AI_MODE.AI_ASSISTED;
      } else {
        return res.status(400).json({
          ok: false,
          error: "invalid_mode",
          message:
            "Provide aiMode (HUMAN_ONLY | AI_DRAFT | AI_ASSISTED | AI_ACTIVE | ESCALATION_REQUIRED) or legacy mode",
        });
      }
      ...
      const clearEscalation =
        req.body?.clearEscalation === true || req.body?.clear_escalation === true;
      const patch = buildAiModePatch(aiMode, {
        assignedCoordinatorId:
          aiMode === AI_MODE.HUMAN_ONLY || aiMode === AI_MODE.ESCALATION_REQUIRED
            ? adminId || undefined
            : undefined,
        clearEscalation,
      });

      if (aiMode === AI_MODE.HUMAN_ONLY || aiMode === AI_MODE.ESCALATION_REQUIRED) {
        patch.human_takeover_at = nowIso;
        if (adminId) patch.assigned_coordinator_id = adminId;
      }
      ...
      const { data, error } = await supabase
        ...
        .eq("clinic_id", clinicId)
        .select(
          "id, coordination_mode, ai_mode, ai_paused, ai_escalation_required, human_takeover_at, assigned_coordinator_id, assigned_doctor_id",
        )
        .maybeSingle();
      ...
      await insertTimelineEvent({
        profileId,
        eventType,
        eventMetadata: { aiMode, coordinationMode: data.coordination_mode, adminId },
      });
