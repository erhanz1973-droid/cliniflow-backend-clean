 ...
 * @param {string} [lang]
 */
function attachOperationalProjection(lead, lang = "en") {
  ...
  const timelinePreview = flags.latestMessagePreview
    ...
    : null;
  const projection = buildCoordinationProjection(lead, { timelinePreview }, lang);
...
function mapProfileRow(row) {
  ...
  const patientName = p ? resolvePatientDisplayLabel(p) : null;
 ...
 * @param {{ requireAdminAuth: Function, insertClinicMessageForLeadReply?: Function }} deps
 */
function registerAiCoordinatorAdminRoutes(app, deps) {
  const { requireAdminAuth, insertClinicMessageForLeadReply } = deps;
  ...
  router.get("/coordination/workspace", requireAdminAuth, async (req, res) => {
    try {
      ...
      let leads = (data || []).map((row) =>
        enrichLeadForCoordinatorUI(enrichLeadRow(row, mapProfileRow), uiLang),
      );
      ...
      if (refresh) {
        ...
        leads = (data || []).map((row) =>
          enrichLeadForCoordinatorUI(
            attachOperationalProjection(enrichLeadRow(row, mapProfileRow), uiLang),
            uiLang,
          ),
        );
      } else {
        leads = leads.map((L) => attachOperationalProjection(L, uiLang));
      }
  ...
  router.get("/ai-leads/queues", requireAdminAuth, async (req, res) => {
    try {
      ...
      return res.json({
        ok: true,
        intakeQueues: localizeIntakeQueues(
          INTAKE_QUEUES.map((q) => ({
            id: q.id,
            label: q.label,
  ...
  router.get("/ai-leads", requireAdminAuth, async (req, res) => {
    try {
      ...
      const allEnriched = baseLeads.map((L) => {
        ...
        return attachDelegationToLead(attachOperationalProjection(enriched, uiLang), clinicPolicy, uiLang);
      });
      ...
      return res.json({
        ...
        operationalFilters: localizeOperationalFilters(
        ...
        handlingFilters: localizeHandlingFilters(
  ...
  router.get("/ai-leads/:profileId", requireAdminAuth, async (req, res) => {
    try {
      ...
      return res.json({
        ok: true,
        lead: attachDelegationToLead(lead, clinicPolicy, uiLang),
        clinicPolicy,
        visitPlanDraft,
        documents,
        ...
        readiness: {
          ...
          missing: localizeReadinessMissingList(intakeFlags.readinessMissing || [], uiLang),
          journeyStage: intakeFlags.journeyStage,
          journeyStageLabel:
            localizeJourneyStageLabel(intakeFlags.journeyStage, uiLang) ||
            intakeFlags.journeyStageLabel,
        },
  ...
  router.post("/ai-leads/:profileId/send-reply", requireAdminAuth, async (req, res) => {
    try {
      const clinicId = getClinicId(req);
      const profileId = String(req.params.profileId || "").trim();
      ...
      if (typeof insertClinicMessageForLeadReply !== "function") {
        return res.status(503).json({ ok: false, error: "send_not_configured" });
      }

      const { data: profile, error: pErr } = await supabase
        .from("ai_coordinator_lead_profiles")
        .select("id, patient_id, clinic_id, primary_channel")
        .eq("id", profileId)
      ...
      if (!profile?.patient_id) {
        return res.status(404).json({ ok: false, error: "profile_or_patient_missing" });
      }

      const patientId = String(profile.patient_id).trim();
      const { sendClinicReplyWithExternalDelivery } = require("./omnichannel/outboundDelivery");
      const { touchProfileHumanReply } = require("./clinicalGuidanceSend");
      const { insertTimelineEvent } = require("./aiCoordinatorTimeline");

      const result = await sendClinicReplyWithExternalDelivery({
        patientId,
        clinicId,
        profileId,
        text,
        ...
        insertFn: insertClinicMessageForLeadReply,
      });
      ...
      await touchProfileHumanReply(profileId, text, { skipTimelineEvent: true });
      await insertTimelineEvent({
        profileId,
        eventType: "human_reply",
        aiReply: text,
        eventMetadata: {
          subtype: "coordinator_lead_inbox_send",
          channel: profile.primary_channel || null,
          delivery: result.delivery || null,
        },
        channel: profile.primary_channel || "in_app",
      });

      const delivery = result.delivery || null;
      const failed =
        delivery?.status === "failed" || delivery?.deliveryStatus === "failed";
      const httpOk = result.ok !== false && !failed;
      return res.status(httpOk ? 200 : 502).json({
        ok: httpOk,
        message: result.message || null,
        patientMessageId: result.patientMessageId || null,
        delivery,
        warning: failed
          ? "Message saved in clinic inbox but Messenger delivery failed — use Resend after fixing connection."
          : null,
      });
    } catch (e) {
      console.error("[POST /api/admin/ai-leads/:id/send-reply]", e?.message || e);
      return res.status(500).json({ ok: false, error: "send_failed", message: e?.message });
    }
  });

  router.post("/ai-leads/:profileId/resend-messenger", requireAdminAuth, async (req, res) => {
    try {
      const clinicId = getClinicId(req);
      const profileId = String(req.params.profileId || "").trim();
      const text = String(req.body?.text || req.body?.message || "").trim();
      const patientMessageId = String(
        req.body?.patientMessageId || req.body?.patient_message_id || "",
      ).trim();
      const outboundDeliveryId = String(
        req.body?.outboundDeliveryId || req.body?.outbound_delivery_id || "",
      ).trim();
      if (!UUID_RE.test(clinicId) || !UUID_RE.test(profileId)) {
        return res.status(400).json({ ok: false, error: "invalid_id" });
      }
      ...
      const { data: profile, error: pErr } = await supabase
        .from("ai_coordinator_lead_profiles")
        .select("id, patient_id, primary_channel")
        .eq("id", profileId)
        .eq("clinic_id", clinicId)
        .maybeSingle();
      if (pErr || !profile?.patient_id) {
        return res.status(404).json({ ok: false, error: "profile_not_found" });
      }
      const { resendMessengerClinicReply } = require("./omnichannel/outboundDelivery");
      const result = await resendMessengerClinicReply({
        patientId: String(profile.patient_id),
        clinicId,
        profileId,
        text,
        patientMessageId: patientMessageId || null,
        outboundDeliveryId: outboundDeliveryId || null,
        messageRole: "coordinator",
        senderName: "Care Team",
      });
      const delivery = result.delivery || null;
      ...
      return res.status(failed ? 502 : 200).json({
        ok: !failed,
        delivery,
        patientMessageId: result.patientMessageId || patientMessageId || null,
      });
    } catch (e) {
      console.error("[POST /api/admin/ai-leads/:id/resend-messenger]", e?.message || e);
      return res.status(500).json({ ok: false, error: "resend_failed", message: e?.message });
    }
  });
