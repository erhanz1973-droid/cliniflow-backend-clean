...
function planForDbColumn(plan) {
  const normalized = normalizePackageKey(plan);
  if (normalized === "PREMIUM") return "PRO";
  if (["FREE", "BASIC", "PRO"].includes(normalized)) return normalized;
  return "FREE";
}
...
async function updateClinicWithColumnPruning(supabase, clinicId, patch) {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    lastError = error;
    if (!error) {
      return { ok: false, error: "clinic_not_found", message: "No clinic row updated" };
    }

    if (!isMissingColumnError(error)) {
      const msg = String(error.message || "update_failed");
      if (/clinics_plan_check|check constraint/i.test(msg)) {
        return {
          ok: false,
          error: "invalid_plan_value",
          message:
            "Invalid plan for database constraint. Use Free, Starter (BASIC), or Professional (PRO). Enterprise maps to PRO.",
        };
      }
      return { ok: false, error: "update_failed", message: msg };
    }

    const missing = getMissingColumnName(error);
    if (!missing || !(missing in currentPatch)) {
      return { ok: false, error: "update_failed", message: error.message || "update_failed" };
    }
...
async function applySuperAdminOperationalAction(clinicId, body, supabase, helpers) {
  ...
  switch (action) {
    case "set_plan":
      if (!body?.plan) return { ok: false, error: "plan_required", message: "Select a plan first" };
      targetPlan = normalizePackageKey(body.plan);
      targetStatus = "ACTIVE";
  ...
  if (planExpiry) settings.planExpiry = planExpiry;
  settings.planSource = "ADMIN";

  const mappedPlan = planForDbColumn(targetPlan);
  const patch = {
    plan: mappedPlan,
  ...
  const updatedResult = await updateClinicWithColumnPruning(supabase, clinicId, patch);
      ...
      try {
        ...
        if (!result.ok) {
          const status =
            result.error === "clinic_not_found"
              ? 404
              : result.error === "invalid_action" || result.error === "plan_required"
                ? 400
                : 500;
          return res.status(status).json(result);
        }
