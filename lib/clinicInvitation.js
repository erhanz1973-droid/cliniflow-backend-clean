...
const IOS_STORE_URL =
  ...
  "https://apps.apple.com/app/id6738260000";
...
function buildInviteUrls(clinicCode, req) {
  ...
  return {
    ...
    appDeepLink: `clinifly://clinic-invite/${encodeURIComponent(code)}`,
...
function renderInviteLandingHtml({ clinic, urls }) {
...
<head>
  ...
  <style>
    ...
    .card{max-width:420px;width:100%;background:#1e293b;border-radius:16px;padding:28px;box-shadow:0 8px 32px rgba(0,0,0,.35);text-align:center;}
    h1{font-size:22px;margin:0 0 8px;}
    p{color:#94a3b8;font-size:15px;line-height:1.5;margin:0 0 20px;}
    .code{font-family:ui-monospace,monospace;background:#0f172a;padding:8px 14px;border-radius:8px;display:inline-block;margin-bottom:20px;font-size:14px;letter-spacing:.08em;}
...
<body>
  <div class="card">
    ...
    <p>You were invited by this clinic. Open the Clinifly app to create your account — your clinic code is saved automatically.</p>
    <div class="code">${escapeHtml(code)}</div>
    <a class="btn primary" id="openApp" href="${escapeHtml(urls.appDeepLink)}">Open Clinifly app</a>
    <div class="stores">
      <a class="btn secondary" href="${escapeHtml(urls.iosStoreUrl)}">App Store</a>
      <a class="btn secondary" href="${escapeHtml(urls.androidStoreUrl)}">Google Play</a>
    </div>
    <p class="hint">After installing, open this page again or scan the QR code to continue signup.</p>
  </div>
  <script>
    ...
    var webUrl = ${JSON.stringify(urls.webUrl)};
    setTimeout(function () {
    ...
    document.getElementById("openApp").addEventListener("click", function (e) {
      e.preventDefault();
      window.location.href = appLink;
      setTimeout(function () { window.location.href = webUrl; }, 2500);
    });
  </script>
</body>
</html>`;
...
function registerClinicInvitationRoutes(app, { requireAdminAuth }) {
  ...
  app.get("/invite/:code", async (req, res) => {
    try {
      ...
      return res.send(renderInviteLandingHtml({ clinic: preview.clinic, urls }));
    } catch (e) {
  ...
  app.get("/api/admin/clinic-invitation/qr.png", requireAdminAuth, async (req, res) => {
    try {
      ...
      const png = await QRCode.toBuffer(urls.webUrl, {
        type: "png",
        width: 512,
        margin: 2,
