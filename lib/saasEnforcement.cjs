 ...
 */
async function getBillingSnapshot(supabase, clinicRow, clinicCode, explicitClinicId) {
  ...
  const clinicId = fromExplicit || fromRow || fromFile || null;
  ...
  const [activeTreatments, monthlyUploads, referralInvites] = await Promise.all([
    ...
    saasUsage.countMonthlyUploadsForClinic(supabase, clinicId, period.start, period.end),
    saasUsage.countMonthlyReferralInvitesForClinic(supabase, clinicId, code, period.start, period.end),
  ]);
