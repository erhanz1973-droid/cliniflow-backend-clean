 ...
 */
async function getBillingSnapshot(supabase, clinicRow, clinicCode) {
  ...
  const clinicId = clinicRow?.id ? String(clinicRow.id) : null;
  const code = clinicCode || clinicRow?.clinic_code || null;

  const [activeTreatments, monthlyUploads, referralInvites] = await Promise.all([
    saasUsage.countActiveTreatmentsForClinic(supabase, clinicId),
    saasUsage.countMonthlyPatientFileUploadsForClinic(supabase, clinicId, period.start),
    saasUsage.countMonthlyReferralInvitesForClinic(supabase, clinicId, code, period.start),
  ]);

  const usage = { activeTreatments, monthlyUploads, referralInvites };
...
async function assertCanAddActiveTreatment(supabase, clinicRow) {
  ...
  const snap = await getBillingSnapshot(supabase, clinicRow, clinicRow?.clinic_code);
