...
function getBillingSnapshotWithZeroUsage(clinicRow) {
  ...
  return buildSnapshotFromUsage(
    clinicRow || {},
    { activeTreatments: 0, monthlyUploads: 0, referralInvites: 0, totalReferrers: 0 },
    period
  );
}
  ...
  if (r.status === 'fulfilled') {
    ...
    if (v && typeof v.value === 'number' && typeof v.error === 'boolean') {
      return {
        value: v.value,
        error: v.error,
        timeout: v.timeout === true,
        uniqueInviterCount: typeof v.uniqueInviterCount === 'number' ? v.uniqueInviterCount : undefined,
      };
    }
...
async function getBillingSnapshot(supabase, clinicRow, clinicCode, explicitClinicId, options) {
  try {
    ...
    const debugUsage = options && options.debugUsage === true;
    const skipCache = Boolean(options && (options.force === true || debugUsage));
    if (skipCache) {
    ...
    const countOpts = debugUsage ? { clinicCode: code, logRowsSample: true } : { clinicCode: code };
    const t = saasUsage.BILLING_COUNT_TIMEOUT_MS;
    const t0 = Date.now();
    const c = (p) => p.catch(() => ({ value: 0, error: true, timeout: false }));

    const results = await Promise.allSettled([
      saasUsage.withTimeout(c(saasUsage.countActiveTreatmentsForClinic(supabase, cid, countOpts)), t, 'treatments'),
    ...
    const snapshot = buildSnapshotFromUsage(
      clinicRow,
      {
        activeTreatments: tr.value,
        monthlyUploads: up.value,
        referralInvites: re.value,
        totalReferrers: re.uniqueInviterCount != null ? re.uniqueInviterCount : 0,
      },
      period
    );
    if (debugUsage && supabase && typeof supabase.from === 'function') {
      try {
        snapshot.usage_debug = await saasUsage.collectUsageDiagnostics(supabase, cid, code, period);
        snapshot.usage_debug.countsUsedInPayload = {
          activeTreatments: tr.value,
          monthlyUploads: up.value,
          referralInvites: re.value,
        };
        snapshot.usage_debug.countErrors = {
          treatments: tr.error,
          uploads: up.error,
          referrals: re.error,
        };
        console.log('[billing] usage_debug', cid, JSON.stringify(snapshot.usage_debug));
      } catch (e) {
        snapshot.usage_debug = { error: String(e && e.message ? e.message : e) };
      }
    }
    if (!debugUsage) {
