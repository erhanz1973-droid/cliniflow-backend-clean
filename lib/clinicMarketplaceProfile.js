...
const MARKETPLACE_SELECT_ATTEMPTS = [
  MARKETPLACE_SELECT,
  `id, name, clinic_code, country, city, city_code, settings, status, is_listed, logo_url, cover_photo_url,
  short_description, about_text,
  website, website_url, facebook_url, instagram_url, tiktok_url, youtube_url, linkedin_url, whatsapp,
...
const REPUTATION_COLUMN_STRIPS = [
  ["reputation_sources", "facebook_page_url", "facebook_recommendation_score", "facebook_recommendation_count"],
  ["trustpilot_url", "trustpilot_rating", "trustpilot_review_count"],
...
function hasSocialLink(row) {
  if (str(row?.facebook_page_url)) return true;
...
function buildMarketplaceProfilePatch(body, existing) {
  ...
  if (Object.keys(social).length) {
    ...
    if ("facebook" in social) {
      const fbSocial = normalizeUrl(social.facebook);
      patch.facebook_url = fbSocial;
      if (fbSocial && !str(existing?.facebook_page_url) && patch.facebook_page_url == null) {
        patch.facebook_page_url = fbSocial;
      }
    }
...
async function fetchClinicMarketplaceRow(supabase, clinicId) {
  let lastErr = null;
  for (let i = 0; i < MARKETPLACE_SELECT_ATTEMPTS.length; i++) {
    const sel = MARKETPLACE_SELECT_ATTEMPTS[i];
    ...
    if (!error) {
      if (i > 0) {
        console.warn(
          "[marketplace-profile/select] using fallback attempt",
          i + 1,
          "of",
          MARKETPLACE_SELECT_ATTEMPTS.length,
        );
      }
      return data;
    }
  ...
  throw new Error(lastErr?.message || "marketplace_fetch_failed");
}

async function updateClinicMarketplaceRow(supabase, clinicId, patch) {
  let attemptPatch = { ...patch };
  const maxAttempts = REPUTATION_COLUMN_STRIPS.length + 1;

  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const { error } = await supabase.from("clinics").update(attemptPatch).eq("id", clinicId);

    if (!error) return fetchClinicMarketplaceRow(supabase, clinicId);

    if (isMissingColumnError(error) && attempt < REPUTATION_COLUMN_STRIPS.length) {
      for (const key of REPUTATION_COLUMN_STRIPS[attempt]) {
        delete attemptPatch[key];
      }
      console.warn(
        "[marketplace-profile/update] retry without columns:",
        REPUTATION_COLUMN_STRIPS[attempt].join(", "),
      );
      continue;
    }
  ...
  return fetchClinicMarketplaceRow(supabase, clinicId);
}
...
function registerClinicMarketplaceAdminRoutes(app, deps) {
  ...
  app.put("/api/admin/marketplace-profile", requireAdminAuth, async (req, res) => {
    try {
      ...
      let data;
      try {
        data = await updateClinicMarketplaceRow(supabase, clinicId, patch);
      } catch (error) {
