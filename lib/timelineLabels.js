...
function timelineLabel(eventType, meta = {}) {
  ...
  switch (eventType) {
    ...
    case "system":
      if (m.subtype === "guidance_created") return "Doctor recorded clinical guidance";
      if (m.subtype === "ai_expanded") return "AI expanded patient-facing draft";
      if (m.subtype === "rewrite_applied") {
        return m.rewrite_action
          ? `Doctor applied rewrite: ${m.rewrite_action}`
          : "Doctor applied rewrite to draft";
      }
      if (m.subtype === "approved_by_doctor") return "Doctor approved message for patient";
      return m.summary || "System event";
    default:
