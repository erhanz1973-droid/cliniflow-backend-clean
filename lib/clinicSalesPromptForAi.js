 ...
 * @param {{ message: string, leadData?: Record<string, unknown>|null, clinicName?: string|null }} params
 */
async function buildClinicSalesPromptForAi(clinicId, params) {
  ...
  lines.push(
    `Patient intent: ${intent.isCommercialQuestion ? "commercial/pricing" : "general"}; topics: ${intent.topics.join(", ") || "unspecified"}; complexity: ${intakeClass.tier}.`,
  );

  lines.push(
    intent.isCommercialQuestion
      ? "RESPONSE ORDER: (1) Answer the commercial question directly using data below. (2) Brief coordinator tone. (3) Optional booking. (4) Do NOT deflect to consultation-only or x-ray upload before answering price/brand questions."
      : "When patient asks about price, brands, or duration — use clinic data below if available; do not refuse or defer to generic consultation scripts.",
  );
