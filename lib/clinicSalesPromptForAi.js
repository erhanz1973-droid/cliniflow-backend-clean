...
function selectRelevantTreatments(treatments, intent) {
  ...
  if (intent.asksBrand || intent.asksVariantComparison || intent.primaryTopic === "implant") {
  ...
  if (intent.asksDirectPrice || intent.asksVariantComparison) return rows.slice(0, 8);
 ...
 */
function summarizeVariantPriceSpread(variants) {
  ...
  let currency = "EUR";
  for (const v of rows) {
    if (v.priceMin != null) prices.push(Number(v.priceMin));
    if (v.priceMax != null) prices.push(Number(v.priceMax));
    if (v.currency) currency = String(v.currency);
  }
  if (!prices.length) return { count: rows.length, currency };

  const min = Math.min(...prices);
  const max = Math.max(...prices);
  const spread = max - min;
  const pct = min > 0 ? Math.round((spread / min) * 100) : null;

  let magnitude = "unknown";
  if (pct != null) {
    if (pct <= 15) magnitude = "modest";
    else if (pct <= 35) magnitude = "moderate";
    else magnitude = "significant";
  }

  return { count: rows.length, min, max, spread, pct, currency, magnitude };
}

function buildVariantComparisonPrompt(params, materials, intent, relevant, allTreatments, authority) {
  ...
  if (mem?.pricingAlreadyDiscussed) {
    lines.push(
      ...
      "* Do NOT repeat the same variant price list verbatim — add comparison insight or answer the new angle (count vs spread).",
    );
  }
  ...
  const categoryLabel = intent.primaryTopic || intent.topics[0] || "implant";
  lines.push(
    `Patient intent: same-category variant comparison (${categoryLabel}) — how many configured options and whether price differences are large.`,
    "RESPONSE ORDER:",
    "* (1) State how many implant/variant options this clinic has configured in admin (count brands/tiers below).",
    "* (2) Compare prices ONLY within that same treatment category — do not mix unrelated procedures.",
    "* (3) Comment whether the spread between lowest and highest configured option is modest, moderate, or significant — use the spread hints below.",
    "* (4) Briefly note tier/brand positioning (premium vs standard) if listed — not a medical recommendation.",
    "* 2–4 short sentences; coordinator tone; non-binding language (typically from, approximately).",
    "* Do NOT deflect to X-ray upload before answering.",
  );

  const treatmentRows = relevant.length ? relevant : allTreatments;
  const withVariants = treatmentRows.filter((t) => Array.isArray(t.variants) && t.variants.length > 0);

  if (withVariants.length) {
    lines.push("\nCONFIGURED VARIANTS (same category — use for count + comparison):");
    for (const t of withVariants.slice(0, 4)) {
      const label = String(t.name || t.treatmentCode || "Treatment").trim();
      const spread = summarizeVariantPriceSpread(t.variants);
      lines.push(`• ${label} — ${spread?.count || t.variants.length} option(s):`);
      lines.push(...formatTreatmentsForPrompt([t]).slice(1));
      if (spread?.min != null && spread?.max != null) {
        const pctNote = spread.pct != null ? ` (~${spread.pct}% spread)` : "";
        lines.push(
          `  Spread hint: ${spread.min}–${spread.max} ${spread.currency}${pctNote} → interpret as ${spread.magnitude} difference for patient commentary.`,
        );
      }
    }
  } else {
    const treatmentLines = formatTreatmentsForPrompt(treatmentRows.slice(0, 6));
    if (treatmentLines.length) {
      lines.push("\nCLINIC TREATMENT PRICING (no structured variants — compare base options if multiple rows):");
      lines.push(...treatmentLines);
    } else {
      lines.push(
        "\n(No structured variant prices in admin — explain options qualitatively and invite coordinator for exact figures.)",
      );
    }
  }

  if (authority.allowBrandNames) {
    ...
    if (brandLines.length) {
      lines.push("\nIMPLANT / MATERIAL BRANDS (context):");
...
async function buildClinicSalesPromptForAi(clinicId, params) {
  ...
  if (intent.asksVariantComparison && !intent.asksDirectPrice) {
    return buildVariantComparisonPrompt(params, materials, intent, relevant, treatments, authority);
  }
