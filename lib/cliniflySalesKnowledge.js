...
async function loadActiveSalesKbEntries(opts = {}) {
  if (!isSupabaseEnabled()) {
    const bundled = getBundledCliniflySalesKb();
...
async function retrieveCliniflySalesKnowledge(params) {
  ...
  const contextBlock = lines.length
    ? `AUTHORITATIVE SALES KB — FACTS ONLY. Transform into Problem → Benefit → Solution → CTA (default: free clinic registration at ${registerUrl} — no credit card, not demo). Never use /sign-up or clinifly.net for clinic signup. Never paste KB lines word-for-word. Never mention clinic codes or patient-app enrollment.\n${lines.join("\n")}`
    : "";
