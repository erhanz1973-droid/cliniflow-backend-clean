  ...
  const lines = selected.map((s, idx) => {
    ...
    const proofs =
      e.proofPoints?.length > 0
        ? ` Proof: ${e.proofPoints.join("; ")}.`
        : "";
    const cta = e.cta ? ` Suggested CTA: ${e.cta}.` : "";
    return `[${idx + 1}] (${e.id}, score=${s.score}) ${e.answerShort}${proofs}${cta}`;
  });

  const contextBlock = lines.length
    ? `AUTHORITATIVE SALES KB (use these facts; do not contradict; never mention clinic codes or patient-app enrollment unless KB says so):\n${lines.join("\n")}`
    : "";
