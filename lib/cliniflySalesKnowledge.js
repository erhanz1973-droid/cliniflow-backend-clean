...
async function retrieveCliniflySalesKnowledge(params) {
  ...
  const contextBlock = lines.length
    ? `AUTHORITATIVE SALES KB — FACTS ONLY. Transform into Problem → Benefit → Solution → CTA (default: free clinic registration, no credit card — not demo). Never paste these lines word-for-word. Never mention clinic codes or patient-app enrollment.\n${lines.join("\n")}`
    : "";
