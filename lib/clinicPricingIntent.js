...
function detectPatientCommercialIntent(message, leadData = {}) {
  ...
  const asksPrice =
    ...
    /\b(fiyat\w*|ücret\w*|ucret\w*|ne\s+kadar|kaç\s+para|kac\s+para|maliyet|tutar\w*|bedel\w*|kaça|kaca|ne\s+kadar\w*)\b/i.test(
      text,
    ) ||
    /\b(pahalı|pahali|ucuz|expensive|cheap|affordable|uygun\s+fiyat)\b/i.test(text) ||
    /\b\d+\s*(€|eur|usd|\$|try|tl|₺|lira)\b/i.test(text);
