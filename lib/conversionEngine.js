...
function analyzeConversationForConversion(params) {
  ...
  if (pricingBlocker) {
    ...
    priorities.push("mention_referral_if_clinic_has_program");
...
function buildConversionStrategyPromptBlock(strategy, config, analysis) {
  ...
  const lines = [
    ...
    "* Keep the conversation moving — do not stop after a one-line price answer.",
    "* Answer the patient's question first, then add context and an optional next step.",
    "",
    "Pricing example:",
    ...
    "",
    "Referral / discount sensitivity:",
    '* When REFERRAL PROGRAM context is provided and the patient shows price sensitivity, you may add ONE soft sentence about friend-referral savings — never only "contact the clinic".',
    '* Do not mention referral if REFERRAL PROGRAM says inactive or passive-only for this turn.',
    "",
  ...
  if (strategy.suggestXray) {
    lines.push("* Gently invite imaging (panoramic X-ray) when aligned with clinic next-step preference — after answering the question.");
  }
