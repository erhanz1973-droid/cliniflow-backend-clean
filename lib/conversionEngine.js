...
function analyzeConversationForConversion(params) {
  ...
  const pricingBlocker =
    commercial.asksDirectPrice &&
    (HESITATION_PATTERNS.some((re) => re.test(combined)) || leadData.budgetSignal === "low");
  const trustGap =
    TRUST_GAP_PATTERNS.some((re) => re.test(combined)) ||
    (msgCount <= 2 && commercial.asksDirectPrice);
  ...
  } else if (commercial.asksDirectPrice) {
    priorities.push("answer_pricing_then_guide");
  } else if (commercial.asksCostSensitivity) {
    priorities.push("answer_cost_qualitatively_no_numbers");
  }
...
function buildConversionStrategyPromptBlock(strategy, config, analysis) {
  ...
  const lines = [
    ...
    "",
    "Pricing discipline:",
    '* Do NOT mention prices, cost ranges, or currency amounts unless the patient asked for a specific amount (fiyat ne kadar, kaç lira, how much).',
    '* «Pahalı mı» / is it expensive → qualitative only (varies by case), zero numbers.',
    'Direct price question: "Implant treatments vary by brand and bone condition. In our clinic, prices usually start around $700 — final quote after clinical assessment."',
    'Cost sensitivity only: "It depends on your case — brand and bone condition matter. If you want a price range, ask how much in lira."',
    "",
