...
function createOfferNotifications(ctx) {
  ...
  async function countDoctorOfferUnreadTotal(doctorKey, clinicId) {
    ...
    try {
      ...
      const { data: offers, error } = await q.limit(400);
  ...
  async function enqueueOfferMessagePush({ actor, offerId, tr, messageRow, doctorRow }) {
    if (!messageRow?.id) return;
    const stableId = `offer_msg:${String(messageRow.id)}`;
    if (!(await tryClaimChatPushDispatch(stableId))) {
    ...
    try {
      if (actor.kind === "patient") {
        const doctorId = String(doctorRow?.doctor_id || doctorRow?.id || "").trim();
        if (!doctorId || !UUID_RE.test(doctorId)) {
          logOfferEvent("offer_push_skipped_no_doctor", { offerId });
          return;
        }
        const clinicId = tr?.clinic_id ? String(tr.clinic_id).trim() : null;
        const badge = await getDoctorCombinedUnreadForPushBadge(doctorId, clinicId);
        ...
        await sendExpoToEntity("doctor", doctorId, {
          ...
          data: {
            ...
            patientId: tr?.patient_id ? String(tr.patient_id) : null,
            senderRole: "patient",
            preview,
            unreadBadge: String(badge),
            enrolled: canonical.enrolled,
            route: canonical.route,
            lead_thread_is_lead: canonical.lead_thread_is_lead,
          },
        });
