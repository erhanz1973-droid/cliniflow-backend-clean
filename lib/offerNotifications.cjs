  ...
  async function enqueueOfferMessagePush({
    ...
    try {
      if (actor.kind === "patient") {
        ...
        const canonical = await resolveOfferPushCanonicalMeta(tr);
        const clinicIdForLink = tr?.clinic_id ? String(tr.clinic_id).trim() : "";
        const senderLabel = String(senderName || "Clinic").trim().slice(0, 80) || "Clinic";
        const mainChatUrl =
          canonical.route === "patient_chat" && clinicIdForLink
            ? `/(patient)/messages?clinicId=${encodeURIComponent(clinicIdForLink)}&clinic_id=${encodeURIComponent(clinicIdForLink)}`
            : canonical.route === "patient_chat"
              ? "/(patient)/messages"
              : `/offer-chat?offerId=${encodeURIComponent(String(offerId))}&otherName=${encodeURIComponent(senderLabel)}`;
        const pushData = buildMessagePushDataPayload({
          type: "offer_message",
          messageId: String(messageRow.id),
          threadId: offerId,
