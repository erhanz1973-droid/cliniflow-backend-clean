...
function createOfferNotifications(ctx) {
  ...
  async function enqueueOfferMessagePush({
    ...
    const inboundStableId =
      actor.kind === "patient" && sourceMessageStableId
        ? String(sourceMessageStableId).trim()
        : offerMsgStableId;
    const stableId = actor.kind === "patient" ? inboundStableId : offerMsgStableId;
    const recipientKind = actor.kind === "patient" ? "doctor" : "patient";
    const recipientId =
      actor.kind === "patient"
        ? String(doctorRow?.doctor_id || doctorRow?.id || "").trim()
        : String(tr?.patient_id || "").trim();
    const notificationType =
      actor.kind === "patient" ? PATIENT_INBOUND_DOCTOR_PUSH_TYPE : "offer_message";
