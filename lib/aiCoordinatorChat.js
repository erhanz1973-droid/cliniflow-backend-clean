...
router.post("/chat", aiChatLimiter, async (req, res) => {
  try {
    ...
    const aiEnabled = sessionId ? await isAiAutoReplyEnabled(sessionId) : true;
    ...
    if (!aiEnabled) {
      reply =
        "Thank you for your message. A human coordinator from the clinic will respond shortly.";
    ...
    if (aiEnabled && clinicUuid && leadPipeline.profileId) {
      try {
    ...
    return res.json({
      ...
      coordination: {
        mode: aiEnabled ? "ai_active" : COORDINATION_HUMAN,
        aiAutoReplyEnabled: aiEnabled,
      },
    });
  } catch (err) {
