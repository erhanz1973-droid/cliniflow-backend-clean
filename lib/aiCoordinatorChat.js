...
const { formatLeadSummaryForHumans } = require("./leadSummaryHuman");
const { uiLangFromRequest, normalizeUiLang } = require("./i18n/uiLocale");
...
const { analyzeInternationalCoordinatorTurn } = require("./internationalPatientCoordinator");
...
const { getClinicIntakeRules, buildClinicIntakeRulesPromptBlock } = require("./clinicIntakeRules");
const {
...
const { buildSmileAnalysisPromptBlock } = require("./smileAnalysisContext");
const { retrieveCliniflyDentalTourismKnowledge } = require("./cliniflyDentalTourismKnowledge");
...
router.post("/chat", aiChatLimiter, async (req, res) => {
  try {
    ...
    const smileAnalysisContext = buildSmileAnalysisPromptBlock(
      req.body?.smileAnalysisContext ?? req.body?.smile_analysis_context,
    );
    ...
    let internationalCoordinatorContext = analyzeInternationalCoordinatorTurn({
      message,
      leadData: priorLeadData || {},
    ...
    let preparationIntakeContext = null;
    if (isTreatmentGuide) {
      ...
      const prevPrep = normalizeConversationalIntake(priorFlags?.conversationalIntake);
      const rulesConfig = clinicUuid ? await getClinicIntakeRules(clinicUuid) : null;
      const treatmentInterest =
        priorLeadData?.treatmentInterest || priorLeadData?.treatment_interest || message;
      const clinicRulesBlock = buildClinicIntakeRulesPromptBlock(rulesConfig, treatmentInterest);
      const missingCritical = listMissingCriticalFields(prevPrep, rulesConfig, treatmentInterest);
      preparationIntakeContext = buildPreparationIntakePromptBlock({
        conversationalIntake: prevPrep,
        missingCritical,
        clinicRulesBlock,
        treatmentInterest,
      });
    ...
    let turnConversationalIntake = null;
    let languageState = null;
    let conversationLanguagePolicy = null;
    let patientQuestionAnchoringPrompt = null;
    let whatsappCollectionContext = null;
    let conversionStrategyContext = null;
    let referralAwarenessContext = "";
    let waEval = { candidate: false };
    let profileRowForWa = null;
    /** Set inside clinic branch when resolving WhatsApp from patient turn. */
    let preWhatsapp = null;
    ...
    let referralAwarenessResult = null;
    ...
    if (clinicUuid) {
      ...
      if (sessionId && isSupabaseEnabled()) {
        ...
        patientContextStrategyPrompt =
          buildPatientContextStrategyPromptBlock(patientContextStrategy);
        internationalCoordinatorContext = analyzeInternationalCoordinatorTurn({
          message,
          leadData: priorLeadData || {},
          profileRow: profileRowForLang,
          travelContextDetected: patientContextStrategy.travel_context_detected,
      ...
      if (profileRowForLang?.id) {
        const flagsForLang =
          profileRowForLang.operational_intake_flags &&
          typeof profileRowForLang.operational_intake_flags === "object"
            ? profileRowForLang.operational_intake_flags
            : {};
        await persistConversationLanguage(profileRowForLang.id, languageState, flagsForLang);
        profileRowForLang = {
          ...profileRowForLang,
      ...
      patientQuestionAnchoringPrompt = buildPatientQuestionAnchoringPromptBlock(message);
      ...
      if (!isTreatmentGuide && profileRowForWa?.id) {
        ...
        const flags =
          ...
          typeof profileRowForWa.operational_intake_flags === "object"
            ? profileRowForWa.operational_intake_flags
            : {};
        const waTurns = profileRowForWa.id
          ? await fetchRecentCoordinatorTurns(String(profileRowForWa.id), { maxTurns: 10 })
          : historyForModel;
        preWhatsapp = resolveWhatsappFromPatientTurn(message, {
          recentTurns: waTurns,
          flags,
          profileRow: profileRowForWa,
        });
        if (preWhatsapp?.declined) {
          ...
          flags.whatsappCollectionStage = "collected";
        ...
        const knownWa =
          normalizeWhatsappNumber(profileRowForWa.whatsapp_number) || preWhatsapp?.number || null;
        ...
        if (knownWa && phoneOnlyTurn) {
          ...
          (flags.whatsappCollectionPrompted === true ||
            coordinatorRecentlyAskedForWhatsapp(waTurns))
        ) {
          whatsappCollectionContext = buildWhatsappAcknowledgmentPromptBlock(
            null,
      ...
      if (
        ...
        const { data: bookingProfile } = await supabase
          ...
          .eq("id", profileRowForLang.id)
          .maybeSingle();
        if (bookingProfile) {
          bookingPrep = await prepareAiAppointmentBookingTurn({
            clinicId: clinicUuid,
            patientId,
            profileRow: bookingProfile,
            patientMessage: message,
            leadData: priorLeadData || {},
            locale: languageState?.conversationLanguage || "en",
          });
          if (bookingPrep.promptBlock) {
            documentIntakeContext = documentIntakeContext
              ? `${bookingPrep.promptBlock}\n\n${documentIntakeContext}`
              : bookingPrep.promptBlock;
          }
        }
      }
      ...
      if (profileRowForLang?.id) {
        ...
        const memFlags =
          ...
          typeof profileRowForWa.operational_intake_flags === "object"
            ? profileRowForWa.operational_intake_flags
            : priorFlags;
        topicContext = resolveActiveConversationTopic({
          patientMessage: message,
          conversationSummary,
          recentTurns: historyForModel,
          persistedTopic: readConversationTopicFromFlags(memFlags),
        });
        if (topicContext.referral_topic_locked && !isTreatmentGuide) {
      ...
      if (clinicUuid && patientId && !isTreatmentGuide) {
        referralAwarenessResult = await buildReferralAwarenessForAi({
          clinicId: clinicUuid,
          patientId,
          message,
          leadData: priorLeadData || {},
          messageCount: profileRowForLang?.message_count,
          pricingBlocker: conversionStrategy.meta?.signals?.pricingBlocker === true,
          topicContext,
          recentTurns: historyForModel,
        });
        referralAwarenessContext = assembleReferralAwarenessContext(
          topicContext || resolveActiveConversationTopic({ patientMessage: message, conversationSummary }),
          referralAwarenessResult,
        );
      }
    }

    if (!clinicUuid && isTreatmentGuide) {
      const patientLang = patientId ? await loadPatientAppLanguage(patientId) : null;
      const bodyConv = normalizeLangCode(
        req.body?.conversationPrimaryLanguage ?? req.body?.conversation_primary_language,
      );
      languageState = resolveConversationLanguage({
        message,
        conversationPrimaryLanguage: bodyConv,
        patientAppLanguage: patientLang,
        profilePreferredLanguage: null,
        clinicPrimaryLanguage: "en",
        enabledLanguageCodes: ["en", "tr", "ru", "ka", "ar"],
        messageCount: 0,
      });
      conversationLanguagePolicy = buildConversationLanguagePromptBlock(languageState);
      patientQuestionAnchoringPrompt = buildPatientQuestionAnchoringPromptBlock(message);
    ...
    if (!runAi) {
    ...
    } else if (bookingPrep.directReply && autoReply) {
      reply = bookingPrep.directReply;
      turnLeadData = priorLeadData || null;
    } else {
      let dentalTourismContext = "";
      const tourismLocale =
        languageState?.conversationLanguage ||
        normalizeLangCode(
          req.body?.conversationPrimaryLanguage ?? req.body?.conversation_primary_language,
        ) ||
        uiLangFromRequest(req);
      const tourismKb = await retrieveCliniflyDentalTourismKnowledge({
        message,
        locale: tourismLocale,
      });
      if (tourismKb.contextBlock) {
        dentalTourismContext = tourismKb.contextBlock;
      }
      ...
      const result = await coordinatorChatReply({
        ...
        conversationPrimaryLanguage: languageState?.conversationPrimaryLanguage || null,
        conversationLanguagePolicy,
        whatsappCollectionContext,
        conversionStrategyContext,
        patientContextStrategyPrompt,
        internationalCoordinatorContext: isTreatmentGuide ? "" : internationalCoordinatorContext,
        repetitionSuppressionPrompt,
        referralAwarenessContext,
        preparationIntakeContext,
        patientQuestionAnchoringPrompt,
        smileAnalysisContext: smileAnalysisContext || null,
        dentalTourismContext,
      });
      let guardedReply = applyConversationRepetitionGuardrails(result.reply, {
        patientMessage: message,
        discussionMemory,
        conversationLanguage: languageState?.conversationLanguage || null,
      });
      guardedReply = repairGenericDeflectionReply(guardedReply, message, {
        conversationLanguage: languageState?.conversationLanguage || null,
      });
      guardedReply = enforcePatientReplyLanguage(guardedReply, {
        expectedLang: languageState?.conversationLanguage || null,
        lockedConversationLanguage: languageState?.conversationPrimaryLanguage || null,
        patientMessage: message,
        logLabel: "aiCoordinatorChat",
      });
      guardedReply = repairPhoneNumberTurnReply(guardedReply, message, {
        lang: resolvePhoneAcknowledgmentLanguage({
          languageState,
          profileRow: profileRowForWa,
          message,
          recentTurns: historyForModel,
        ...
        recentTurns: historyForModel,
        whatsappNumber:
          normalizeWhatsappNumber(profileRowForWa?.whatsapp_number) ||
          preWhatsapp?.number ||
          null,
      });
      coordinatorDraft = guardedReply;
      if (autoReply) {
        reply = guardedReply;
      } else {
        reply = coordinatorNotifiedReply(
          resolveCoordinatorReplyLang({
            lang: languageState?.conversationLanguage,
            patientMessage: message,
          }),
      ...
      turnConversationalIntake = result.conversationalIntake || null;
    }
    ...
    if (leadPipeline.profileId) {
      ...
      const memoryAfterTurn =
        discussionMemory && autoReply
          ...
          : discussionMemory;
      const topicAfterTurn =
        topicContext && autoReply
          ? updateConversationTopicAfterTurn(topicContext, {
              patientMessage: message,
              aiReply: reply,
              referralState: referralAwarenessResult?.state || null,
            })
          : topicContext;
      const syncResult = await syncOperationalIntakeFlags(
        ...
        profileDocsForJourney.length ? profileDocsForJourney : intakeDocuments,
        {
          ...
          conversationalIntake: turnConversationalIntake,
      ...
      if (autoReply && reply) {
        void markAppointmentOfferInAiReply(leadPipeline.profileId, reply).catch(() => {});
      }
      void finalizeAiAppointmentBookingTurn({
        profileId: leadPipeline.profileId,
        patientMessage: message,
      }).catch((e) => console.warn("[aiAppointmentBooking] ai_chat finalize:", e?.message || e));
    }
    ...
    const summaryLang = languageState?.conversationLanguage
      ? normalizeUiLang(languageState.conversationLanguage)
      : uiLangFromRequest(req);
    const leadSummary = leadDataHasSignals(leadData)
      ? formatLeadSummaryForHumans(leadData, summaryLang)
      : { lines: [], paragraph: "", tagSummaries: [] };

    return res.json({
      success: true,
      reply,
      leadData,
      leadSummaryLines: leadSummary.lines,
      leadSummarySections: leadSummary.sections,
      leadSummaryParagraph: leadSummary.paragraph,
      conversationSummary: nextSummary,
      ...
      conversationalIntake: operationalIntakeFlags?.conversationalIntake || null,
