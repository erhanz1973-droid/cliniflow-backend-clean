...
function registerClinicAiCommunicationRoutes(app, deps) {
  ...
  app.get("/api/admin/clinic/ai-communication", requireAdminAuth, async (req, res) => {
    try {
      ...
      const clinicHours = resolveClinicWorkingHours(profile.logistics);
      return res.json({
        ok: true,
        clinicPolicy: policy,
        aiReplies,
  ...
  app.patch("/api/admin/clinic/ai-communication", requireAdminAuth, async (req, res) => {
    try {
      ...
      const aiReplies = {
        ...
        messageBufferPreset: (() => {
          if (req.body?.messageBufferPreset != null) {
            return normalizeMessageBufferPreset(req.body.messageBufferPreset);
          }
          if (req.body?.messageBufferDelay != null) {
            return normalizeMessageBufferPreset(req.body.messageBufferDelay);
          }
          return prev.messageBufferPreset || messageBufferPresetFromMs(prev.messageBufferDelayMs);
        })(),
        messageBufferDelayMs: (() => {
          if (req.body?.messageBufferDelayMs != null) {
            return Math.max(0, Math.min(15000, Math.round(Number(req.body.messageBufferDelayMs))));
          }
          if (req.body?.messageBufferPreset != null || req.body?.messageBufferDelay != null) {
            return messageBufferMsFromPreset(
              req.body.messageBufferPreset ?? req.body.messageBufferDelay,
            );
          }
          if (req.body?.omnichannelInstantDelayMs != null) {
            return Math.max(0, Math.min(15000, Math.round(Number(req.body.omnichannelInstantDelayMs))));
          }
          return prev.messageBufferDelayMs ?? resolveMessageBufferDelayMs(prev);
        })(),
      ...
      aiReplies.omnichannelInstantDelayMs = aiReplies.messageBufferDelayMs;
      ...
      let logisticsPatch;
      const hasHoursPatch =
        req.body?.clinicWeekdayStart != null ||
        req.body?.clinicWeekdayEnd != null ||
        req.body?.clinicTimezone != null ||
        req.body?.weekdayStart != null ||
        req.body?.weekdayEnd != null ||
        req.body?.timezone != null ||
        req.body?.saturdayEnabled != null ||
        req.body?.enableSaturday != null ||
        req.body?.saturdayStart != null ||
        req.body?.saturdayEnd != null ||
        req.body?.sundayEnabled != null ||
        req.body?.enableSunday != null ||
        req.body?.sundayStart != null ||
        req.body?.sundayEnd != null ||
        req.body?.weekday24Hours != null ||
        req.body?.saturday24Hours != null ||
        req.body?.sunday24Hours != null ||
        req.body?.open24_7 != null ||
        req.body?.weeklySchedule != null;

      if (hasHoursPatch) {
        logisticsPatch = mergeLogisticsWorkingHours(profile.logistics, req.body || {});
      }
      ...
      const saved = await upsertClinicAiSettings(clinicId, {
        ...
        ...(logisticsPatch ? { logistics: logisticsPatch } : {}),
      });
      if (!saved.ok) {
      ...
      const savedAiReplies = withFallbackDerivedFields(
        normalizeAiRepliesConfig(saved.profile?.communicationPolicy),
      );
      const synced = {
        ...savedAiReplies,
        omnichannelInstantDelayMs: savedAiReplies.messageBufferDelayMs,
      };
      return res.json({
        ok: true,
        aiReplies: synced,
        aiBooking: normalizeAiBookingConfig(saved.profile?.communicationPolicy),
        clinicHours: resolveClinicWorkingHours(saved.profile?.logistics),
