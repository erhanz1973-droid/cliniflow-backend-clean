...
const { resolveClinicWorkingHours, normalizeHm } = require("./clinicWorkingHours");
  ...
  app.patch("/api/admin/clinic/ai-communication", requireAdminAuth, async (req, res) => {
    try {
      ...
      if (
        ...
        const start =
          normalizeHm(req.body.clinicWeekdayStart) ||
          normalizeHm(req.body.weekdayStart) ||
          hours.weekdays.start;
        const end =
          normalizeHm(req.body.clinicWeekdayEnd) ||
          normalizeHm(req.body.weekdayEnd) ||
          hours.weekdays.end;
        const timezone =
          String(req.body.clinicTimezone || req.body.timezone || hours.timezone).trim() ||
          hours.timezone;
        logisticsPatch = {
          ...(profile.logistics && typeof profile.logistics === "object" ? profile.logistics : {}),
          timezone,
          weekdayHours: `${start} – ${end}`,
          workingHours: { timezone, weekdays: { start, end } },
        };
      }
