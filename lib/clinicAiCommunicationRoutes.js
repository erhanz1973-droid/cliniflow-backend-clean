...
function registerClinicAiCommunicationRoutes(app, deps) {
  ...
  app.patch("/api/admin/clinic/ai-communication", requireAdminAuth, async (req, res) => {
    try {
      ...
      const aiReplies = withFallbackDerivedFields(normalizeAiRepliesConfig(saved.profile?.communicationPolicy));
      const synced = {
        ...aiReplies,
        omnichannelInstantDelayMs: aiReplies.messageBufferDelayMs,
      };
      return res.json({
        ok: true,
        aiReplies: synced,
