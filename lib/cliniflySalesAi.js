...
function buildCliniflySalesSystemPrompt() {
  return `You are Clinifly AI on Messenger — helpful for dental consumers (especially Facebook/Instagram ad visitors) and for clinic owners discussing the Clinifly platform.
...
async function cliniflySalesChatReply({
  ...
  if (isSalesPhotoUploadInbound(userText)) {
    latestBlock +=
      "\n\nINTERPRETATION: Visitor uploaded a photo. Smile Score analysis is only in the Clinifly app — use the Smile Score conversion reply (photo upload → Smile Score → fast AI insights → App Store + Google Play). Do NOT analyze the photo in Messenger. Do NOT ask clinic representative or clinic code.";
  } else if (isSalesDentalConsumerQuestion(userText)) {
    latestBlock +=
      "\n\nINTERPRETATION: Dental/smile consumer question from an ad visitor. Smile Score conversion ONLY — short reply: upload smile photo in Clinifly app → AI Smile Score → fast personalized insights → App Store + Google Play links. Do NOT explain veneers, implants, orthodontics, whitening, or treatments in Messenger. Do NOT ask clinic representative or clinic code. Set salesLead.visitorType=patient.";
  } else if (isSalesGreetingOnlyMessage(userText) && shouldUseConsumerAdMessengerFlow(userText, { recentTurns: turns })) {
    latestBlock +=
      "\n\nINTERPRETATION: Greeting from Facebook/Instagram ad consumer. You are Clinifly AI — warm welcome + offer to help with teeth/smile questions. Do NOT ask clinic representative. Do NOT ask clinic code. Do NOT redirect to the app on greeting alone.";
...
async function runCliniflySalesInboundReply(params) {
  ...
  if (isSalesPhotoUploadInbound(message)) {
    replyText = buildPhotoUploadAppReply(lang);
    ...
    replyText = buildSmileAnalysisAppDownloadReply(lang);
  ...
  if (!String(replyText || "").trim() && detectSalesHumanEscalation(message)) {
    ...
    if (isSalesProfileIdentityMessage(message)) {
      ...
      replyText = buildConsumerAdWelcomeReply(lang);
  ...
  if (
    ...
    if (fixed?.reply) {
      ...
      const substantive = resolveSalesSubstantiveFallbackReply(message, lang);
