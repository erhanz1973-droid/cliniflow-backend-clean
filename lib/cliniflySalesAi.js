...
PRIMARY GOAL: For ad consumers — answer teeth/smile questions naturally (no diagnosis), then gently encourage the Clinifly app. Never ask clinic representative or clinic code. Do not redirect immediately. For clinic owners discussing Clinifly (registration, AI training, patient acquisition, pricing), help them register and try Clinifly.
...
• Do NOT re-ask clinic representative vs information seeker when the visitor asked a dental/treatment question — answer the question first.
• No diagnosis, prescriptions, or guarantees — but DO give brief helpful general dental education when asked (prosthesis, implants, veneers, etc.).
...
async function cliniflySalesChatReply({
  ...
  if (isSalesPhotoUploadInbound(userText)) {
    latestBlock +=
      "\n\nINTERPRETATION: Visitor uploaded a photo. Thank them; explain AI photo analysis is only in the Clinifly app. Provide App Store + Google Play links. Do NOT analyze the photo in Messenger. Do NOT ask clinic representative or clinic code.";
  } else if (isSalesDentalConsumerQuestion(userText)) {
    latestBlock +=
      "\n\nINTERPRETATION: Dental/treatment consumer question. Answer the question directly with helpful general information first (no diagnosis). Do NOT ask clinic representative vs information seeker. Do NOT ask for a clinic code. Assume regular user. Set salesLead.visitorType=patient. After answering, one gentle Clinifly app line (download, upload photos, connect with clinics).";
...
async function runCliniflySalesInboundReply(params) {
  ...
  if (
    ...
    (looksLikeRoleQualificationReply(replyText) || salesIntent === "visitor_discovery")
  ) {
    replyText = buildDentalConsumerQuestionFallbackReply(message, lang);
    salesIntent = "dental_consumer_question";
    salesLead = mergeVisitorTypeIntoSalesLead(salesLead, "patient");
  } else if (
