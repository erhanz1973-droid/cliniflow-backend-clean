...
PRIMARY GOAL: Help clinic owners see how Clinifly grows their clinic, then register and try the platform free (no credit card) via the canonical clinic registration URL — not explain software like a manual or push meetings on every turn.
...
COMMERCIAL BEHAVIOR:
• Warm, confident, consultative — default forward motion: free clinic registration at admin-register.html (self-service, no credit card, immediate access).
...
async function cliniflySalesChatReply({
  ...
  if (isSalesProfileIdentityMessage(userText)) {
...
async function runCliniflySalesInboundReply(params) {
  ...
  if (detectSalesHumanEscalation(message)) {
    ...
    if (lang === "ka") {
      replyText =
        `გამარჯობა — Clinifly-ის ასისტენტი ვარ. თქვენ შეგიძლიათ უფასოდ დაარეგისტრიროთ თქვენი კლინიკა საკრედიტო ბარათის გარეშე:\n${regUrl}\nრეგისტრაციას მხოლოდ რამდენიმე წუთი სჭირდება.`;
    } else if (lang === "tr") {
      replyText =
        `Merhaba — Clinifly ekibindenim. Kliniğinizi ücretsiz kaydedin, kredi kartı gerekmez: ${regUrl}`;
    } else {
      replyText =
        `Hello — I'm the Clinifly assistant. Register your clinic free with no credit card: ${regUrl}`;
    }
    }
  } else {
    salesIntent = detectSalesConversationIntent(message);
    const kbLimit =
      salesIntent === "demo" ? 2 : salesIntent === "profile_identity" ? 1 : 4;
    ...
    try {
      const ai = await cliniflySalesChatReply({
        ...
        salesKbContext: salesIntent === "profile_identity" ? "" : kbRetrieval.contextBlock,
        salesPlaybook,
        antiRepeatHint,
        ctaRepeatGuard,
        pricingFollowUpHint,
      ...
      if (salesIntent === "profile_identity" || isSalesProfileIdentityMessage(message)) {
