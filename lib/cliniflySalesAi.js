...
async function cliniflySalesChatReply({
  ...
  if (isSalesGreetingOnlyMessage(userText)) {
    ...
    latestBlock +=
      "\n\nINTERPRETATION: Visitor says you already said that / stop repeating. Apologize briefly (one line). Do NOT restart Stage 1 qualification. Do NOT ask clinic vs patient again. Do NOT paste the registration URL again unless they explicitly ask how to register. Offer ONE helpful follow-up question on the current sales topic.";
    ...
    latestBlock +=
      "\n\nINTERPRETATION: Visitor corrects you — videos DO exist. Apologize briefly, share YouTube @Clinifly link. Do NOT restart introduction or ask clinic vs patient.";
...
async function runCliniflySalesInboundReply(params) {
  ...
  if (
    ...
    /temsilcisi|representative|patient\?|პაციენტ/i.test(String(replyText || ""))
  ) {
