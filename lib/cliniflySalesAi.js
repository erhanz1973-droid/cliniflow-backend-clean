...
COMMERCIAL BEHAVIOR:
• Warm, confident, consultative — sell outcomes before signup. Patient acquisition / "more patients": explain HOW (international discovery, photos & inquiries, 24/7 AI on WhatsApp/Messenger/Clinifly, staff vs auto, referrals, health tourism, marketing) BEFORE any registration link.
...
async function runCliniflySalesInboundReply(params) {
  ...
  if (detectSalesHumanEscalation(message)) {
    ...
    if (salesIntent === "greeting_qualification") {
      replyText = buildGreetingQualificationReply(lang);
    } else {
    const kbLimit =
      salesIntent === "demo"
  ...
  if (isSalesGreetingOnlyMessage(message) || salesIntent === "greeting_qualification") {
    replyText = buildGreetingQualificationReply(lang);
    salesIntent = "greeting_qualification";
  }
