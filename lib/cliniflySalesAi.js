...
PRIMARY GOAL: First understand who the visitor is (clinic representative, visitor seeking information, partner) and what they need — then help clinic owners register and try Clinifly. Never send a long sales pitch after a simple hello. In greeting questions never ask "are you a patient" — use "looking for information" instead (covers dental, aesthetic, and beauty visitors).
...
• No clinic codes (e.g. CEM), patient-app enrollment steps, quoting THIS visitor's clinic treatment prices, or clinical advice.
...
async function cliniflySalesChatReply({
  ...
  if (isSalesSmileAnalysisAdIntent(userText, { recentTurns: turns })) {
    latestBlock +=
      "\n\nINTERPRETATION: Smile analysis / consumer ad visitor (Facebook or Instagram). Guide to Clinifly patient app: upload teeth photo, AI recommendations, optional clinic connection. Do NOT ask clinic representative vs information seeker. Do NOT pitch clinic B2B registration.";
  } else if (isSalesGreetingOnlyMessage(userText)) {
    latestBlock +=
      "\n\nINTERPRETATION: Greeting only (Stage 1). Reply with short welcome + ask clinic representative vs visitor seeking information. Never say 'patient' or 'hasta'. No product pitch.";
...
async function runCliniflySalesInboundReply(params) {
  ...
  if (detectSalesHumanEscalation(message)) {
    ...
    } else if (isSalesSmileAnalysisAdIntent(message, { recentTurns })) {
  ...
  replyText = sanitizeSalesRegistrationUrls(replyText, {
    preserveAppStoreLinks: isSalesMobileAppDownloadQuery(message),
  });
