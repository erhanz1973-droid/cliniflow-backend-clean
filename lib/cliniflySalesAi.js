...
const { enforcePatientReplyLanguage, buildConversationLanguagePromptBlock } = require("./conversationLanguage");
...
const {
  ...
  buildConsumerAdMessengerRulesBlock,
...
const { buildMobileAppDownloadPromptRules } = require("./patientClinicEnrollment");
const {
...
function buildCliniflySalesSystemPrompt(channelMetadata) {
  const partnerClinic = resolveFacebookAdPartnerClinic(channelMetadata);
  return `You are Clinifly AI on Messenger — helpful for dental consumers (especially Facebook/Instagram ad visitors from ${partnerClinic}) and for clinic owners discussing the Clinifly platform.

${buildConsumerAdMessengerRulesBlock(partnerClinic)}

PRIMARY GOAL: For ad consumers — Smile Score conversion only: invite smile photo upload in the Clinifly app, mention AI Smile Score and fast results, then App Store + Google Play links. No long dental education in Messenger. Never ask clinic representative or clinic code. For clinic owners discussing Clinifly (registration, AI training, patient acquisition, pricing), help them register and try Clinifly.
...
CLINIC AI TRAINING (core product fact — affirm when asked):
• YES — clinics train their Clinifly AI assistant with clinic-specific knowledge in Admin: Treatment Price List (pricing), AI Training Center (policies, tone, FAQs, workflows), working hours, materials/brands, international visitor procedures, transfer/accommodation, and clinic policies.
• If asked whether clinics can teach prices, treatment processes, FAQs, or workflows to the AI: explain YES and how (Treatment Price List + AI Training Center). NEVER say this is impossible or that Clinifly does not allow it.

HARD BANS (this sales chat only — not product capabilities):
• Do NOT ask for clinic codes in Messenger unless the visitor is explicitly enrolling with a code they already have.
• Do NOT re-ask clinic representative vs information seeker when the visitor asked a dental/treatment question — use Smile Score app conversion instead.
• No diagnosis, prescriptions, or guarantees. Do NOT explain veneers, implants, orthodontics, whitening, or prosthetics in Messenger — direct to app photo upload and Smile Score.
• No quoting specific clinic treatment prices. No "our clinic / our doctors" procedure booking tone.
...
• Short messages with only a clinic and/or person name (e.g. "Bright Dental. Sara K.") are identity/profile info — thank them, save salesLead, do not answer as a product FAQ.
• NEVER invent or assume clinic/contact names. Only use names the visitor explicitly typed. Images/stickers/emoji-only messages are NOT identity — do not thank them for a clinic or name they did not send.
 ...
 */
function mergeSalesLeadMetadata(salesLead, prevMeta = {}, statedIdentity = null) {
  const baseMeta =
    statedIdentity && statedIdentity.clinicNames && statedIdentity.contactNames
      ? pruneUnverifiedSalesLeadMetadata(prevMeta, statedIdentity)
      : prevMeta;
  const prev =
    baseMeta.clinifly_sales_lead && typeof baseMeta.clinifly_sales_lead === "object"
      ? { ...baseMeta.clinifly_sales_lead }
      : {};
  if (!salesLead) return { ...baseMeta, clinifly_sales_lead: prev };
  const patch = { ...prev };
  const identityKeys = new Set(["contactName", "clinicName"]);
  for (const key of [
    "contactName",
    "clinicName",
    "email",
    ...
    if (identityKeys.has(key) && statedIdentity) {
      const set = key === "clinicName" ? statedIdentity.clinicNames : statedIdentity.contactNames;
      if (!set.has(String(v).trim())) continue;
    }
    patch[key] = v;
  }
  patch.updated_at = new Date().toISOString();
  return { ...baseMeta, clinifly_sales_lead: patch };
}
 ...
 * @param {{ clinicNames: Set<string>, contactNames: Set<string> }} statedIdentity
 */
function buildKnownLeadHint(channelMetadata, statedIdentity) {
  const lead =
  ...
  if (!lead) return "";
  const parts = [];
  const clinic = String(lead.clinicName || "").trim();
  const contact = String(lead.contactName || "").trim();
  if (clinic && statedIdentity?.clinicNames?.has(clinic)) parts.push(`clinic: ${clinic}`);
  if (contact && statedIdentity?.contactNames?.has(contact)) parts.push(`contact: ${contact}`);
  if (lead.country) parts.push(`country: ${lead.country}`);
  if (!parts.length) return "";
  return (
    `INTERNAL — visitor-stated lead on file: ${parts.join("; ")}. ` +
    "Do NOT address the visitor by clinic/contact name unless they typed it themselves in this chat."
  );
}
...
async function cliniflySalesChatReply({
  ...
  const partnerClinic = resolveFacebookAdPartnerClinic(channelMetadata);
  const partnerPromptBlock = buildFacebookAdPartnerPromptBlock(conversationLanguage, partnerClinic);
  ...
  if (isSalesPhotoUploadInbound(userText)) {
    latestBlock +=
      `\n\nINTERPRETATION: Visitor uploaded a photo (from ${partnerClinic} Facebook ad). Smile Score analysis is only in the Clinifly app — tie reply to ${partnerClinic} + Smile Score conversion (photo upload → Smile Score → fast AI insights → App Store + Google Play). Do NOT analyze the photo in Messenger. Do NOT ask clinic representative or clinic code.`;
  } else if (isSalesDentalConsumerQuestion(userText)) {
    latestBlock +=
      `\n\nINTERPRETATION: Dental/smile/aesthetic consumer question from ${partnerClinic} Facebook ad visitor. Tie reply to ${partnerClinic} + Smile Score conversion ONLY — short reply: upload smile photo in Clinifly app → AI Smile Score → fast personalized insights → App Store + Google Play links. Do NOT explain veneers, implants, orthodontics, whitening, or treatments in Messenger. Do NOT ask clinic representative or clinic code. Set salesLead.visitorType=patient.`;
  } else if (isSalesGreetingOnlyMessage(userText) && shouldUseConsumerAdMessengerFlow(userText, { recentTurns: turns, channelMetadata })) {
    latestBlock +=
      `\n\nINTERPRETATION: Greeting from ${partnerClinic} Facebook/Instagram ad consumer. You are Clinifly AI — warm welcome tied to ${partnerClinic}'s ad + offer to help with smile/aesthetic questions. Do NOT ask clinic representative. Do NOT ask clinic code. Do NOT redirect to the app on greeting alone.`;
  } else if (isSalesGreetingOnlyMessage(userText)) {
    latestBlock +=
      "\n\nINTERPRETATION: Greeting only (clinic B2B). Reply with short welcome + ask clinic representative vs visitor seeking information. Never say 'patient' or 'hasta'. No product pitch.";
  } else if (isSalesAcknowledgmentMessage(userText)) {
    latestBlock +=
      "\n\nINTERPRETATION: Short acknowledgment/agreement only (e.g. mantıklı, bence de, ok). Continue the SAME topic from recent messages. Do NOT greet again. Do NOT re-ask clinic representative vs information seeker. Do NOT echo their phrase as the main subject.";
  } else if (isSalesRepetitionComplaintMessage(userText)) {
    latestBlock +=
      "\n\nINTERPRETATION: Visitor says you already said that / stop repeating. Apologize briefly (one line). Do NOT restart Stage 1 qualification. Do NOT re-ask clinic representative vs information seeker. Do NOT paste the registration URL again unless they explicitly ask how to register. Offer ONE helpful follow-up question on the current sales topic.";
  } else if (isSalesTutorialVideoQuery(userText)) {
    latestBlock +=
      "\n\nINTERPRETATION: Tutorial/how-to video question. Confirm YES — share YouTube @Clinifly link from KB. NEVER say videos do not exist.";
  } else if (isSalesTutorialVideoPushbackMessage(userText, recentTurns)) {
    latestBlock +=
      "\n\nINTERPRETATION: Visitor corrects you — videos DO exist. Apologize briefly, share YouTube @Clinifly link. Do NOT restart introduction or re-ask clinic representative vs information seeker.";
  ...
  } else if (isSalesLanguageSwitchMessage(userText)) {
    const req = detectSalesRequestedLanguage(userText) || conversationLanguage || "en";
    latestBlock +=
      `\n\nINTERPRETATION: Visitor asks to switch language to ${req}. Apologize briefly if you replied in the wrong language before. Reply ONLY in ${req} from this turn — do NOT explain language settings or stored preferences. Continue helping on the same topic.`;
  }
  let systemPrompt = buildCliniflySalesSystemPrompt(channelMetadata);
  systemPrompt = `${systemPrompt}\n\n${partnerPromptBlock}`;
  const langBlock = buildConversationLanguagePromptBlock({
    conversationLanguage: conversationLanguage || "en",
    detectedInputLanguage: conversationLanguage || "en",
    confidence: 0.9,
    languageSwitched: isSalesLanguageSwitchMessage(userText),
    policy: { stickySessionLanguage: false },
  });
  systemPrompt = `${systemPrompt}\n\n${langBlock}`;
  ...
  safeReply = sanitizePartnerClinicLanguageLeak(safeReply, conversationLanguage);
  safeReply = sanitizeSalesRegistrationUrls(safeReply, {
    preserveAppStoreLinks: isSalesMobileAppDownloadQuery(userText),
  });
  return {
    reply: safeReply,
...
async function runCliniflySalesInboundReply(params) {
  ...
  if (isSalesLanguageAmbiguousInbound(message)) {
    console.log("[cliniflySalesAi] ambiguous inbound — resolved language:", {
      lang,
      profileLang: profileRow.conversation_primary_language || profileRow.preferred_language,
      fromRecentTurns: inferLanguageFromRecentTurns(recentTurns),
      messagePreview: message.slice(0, 32),
    });
  }
  ...
  if (isSalesPhotoUploadInbound(message)) {
    replyText = buildPhotoUploadAppReply(lang, prevMetaEarly);
    salesIntent = "photo_upload_app";
    salesLead = mergeVisitorTypeIntoSalesLead(salesLead, "patient");
  } else if (
    isSalesDentalConsumerQuestion(message) &&
    shouldUseConsumerAdMessengerFlow(message, { recentTurns, channelMetadata: prevMetaEarly })
  ) {
    replyText = buildSmileAnalysisAppDownloadReply(lang, prevMetaEarly);
    salesIntent = "smile_score_conversion";
    salesLead = mergeVisitorTypeIntoSalesLead(salesLead, "patient");
  } else if (isSalesMediaOnlyInbound(message)) {
    if (shouldUseConsumerAdMessengerFlow(message, { recentTurns, channelMetadata: prevMetaEarly })) {
      replyText = buildSmileAnalysisAppDownloadReply(lang, prevMetaEarly);
      salesIntent = "smile_score_conversion";
      salesLead = mergeVisitorTypeIntoSalesLead(salesLead, "patient");
    } else {
      return { sent: false, reason: "media_only_no_reply" };
    }
  }

  const statedIdentity = collectUserStatedIdentity(recentTurns, message);

  if (!String(replyText || "").trim() && detectSalesHumanEscalation(message)) {
    replyText = buildSalesHumanHandoffReply(lang);
    escalated = true;
  } else if (!String(replyText || "").trim() && !isOpenAIConfigured()) {
    if (isSalesProfileIdentityMessage(message)) {
      ...
      const duplicate =
        assistantRecentlyAckedProfileIdentity(recentTurns) ||
        hasKnownSalesLeadIdentity(prevMetaEarly);
      replyText = buildProfileIdentityAckReply(lang, id, { duplicate });
      salesIntent = "profile_identity";
    } else if (isSalesAiClinicTrainingQuery(message)) {
      replyText = buildAiClinicTrainingReply(lang);
      salesIntent = "ai_clinic_knowledge_training";
    ...
    } else if (isSalesInternationalPatientsMessage(message)) {
      replyText = buildInternationalPatientsDirectReply(lang);
      salesIntent = "international_patients";
    } else if (isSalesLocationQuestion(message)) {
      replyText = buildSalesLocationReply(lang);
      salesIntent = "general";
    } else if (isSalesMobileAppDownloadQuery(message)) {
      replyText = buildMobileAppDownloadReply(lang);
      salesIntent = "mobile_app_download";
      salesLead = mergeVisitorTypeIntoSalesLead(salesLead, "patient");
    } else if (isSalesDoctorRegistrationQuery(message)) {
      replyText = buildDoctorRegistrationReply(lang);
      salesIntent = "doctor_registration";
    } else if (
      isSalesGreetingOnlyMessage(message) &&
      shouldUseConsumerAdMessengerFlow(message, { recentTurns, channelMetadata: prevMetaEarly })
    ) {
      replyText = buildConsumerAdWelcomeReply(lang, prevMetaEarly);
      salesIntent = "consumer_ad_welcome";
      salesLead = mergeVisitorTypeIntoSalesLead(salesLead, "patient");
    ...
    const direct = resolveSalesDirectReply(message, lang, prevMeta, recentTurns);
    if (direct?.skip) {
      return { sent: false, reason: direct.intent || "non_substantive_skip" };
    }
    if (direct?.reply) {
      replyText = direct.reply;
      salesIntent = direct.intent || "general";
      salesLead = mergeVisitorTypeIntoSalesLead(salesLead, direct.visitorType || null);
    ...
    } else if (salesIntent === "profile_identity") {
      const parsed = parseProfileIdentityFromMessage(message);
      salesLead = mergeParsedIdentityIntoSalesLead(parsed, salesLead);
      const duplicate =
        assistantRecentlyAckedProfileIdentity(recentTurns) || hasKnownSalesLeadIdentity(prevMeta);
      replyText = buildProfileIdentityAckReply(lang, parsed, { duplicate });
    } else if (salesIntent === "consumer_ad_welcome") {
      replyText = buildConsumerAdWelcomeReply(lang, prevMetaEarly);
      salesLead = mergeVisitorTypeIntoSalesLead(salesLead, "patient");
    } else if (salesIntent === "photo_upload_app") {
      replyText = buildPhotoUploadAppReply(lang, prevMeta);
      salesLead = mergeVisitorTypeIntoSalesLead(salesLead, "patient");
    } else if (salesIntent === "greeting_qualification" && !pastQualification) {
      replyText = buildGreetingQualificationReply(lang);
    } else {
      ...
      salesIntent === "demo"
        ...
        : salesIntent === "profile_identity"
          ...
          :           salesIntent === "greeting_qualification" ||
              ...
              salesIntent === "smile_analysis_app" ||
              salesIntent === "dental_consumer_question" ||
              salesIntent === "consumer_ad_welcome" ||
              salesIntent === "photo_upload_app" ||
              salesIntent === "mobile_app_download" ||
              salesIntent === "doctor_registration"
    ...
    const salesPlaybook = buildSalesPlaybookBlock(salesIntent, lang, message, prevMeta);
    ...
    const knownLeadHint = buildKnownLeadHint(prevMeta, statedIdentity);
    const pastQualHint = buildPastQualificationHint(recentTurns, nextSummary, prevMeta);
    ...
    try {
      const ai = await cliniflySalesChatReply({
        ...
        ctaRepeatGuard,
        pricingFollowUpHint,
        knownLeadHint: [knownLeadHint, pastQualHint].filter(Boolean).join("\n\n"),
        onboardingKbContext,
        channelMetadata: prevMeta,
      });
      replyText = ai.reply;
      nextSummary = ai.conversationSummary || nextSummary;
      salesLead = sanitizeSalesLeadIdentityFields(ai.salesLead, statedIdentity);
      if (salesIntent === "profile_identity") {
        salesLead = mergeParsedIdentityIntoSalesLead(
          parseProfileIdentityFromMessage(message),
          salesLead,
      ...
      const detectedVisitorType =
        salesIntent === "dental_consumer_question"
          ? "patient"
          : detectVisitorTypeFromMessage(message);
      salesLead = mergeVisitorTypeIntoSalesLead(salesLead, detectedVisitorType);
    } catch (e) {
  ...
  const lastAssistantTurn = [...(recentTurns || [])]
    ...
    .slice(-1)[0];
  if (lastAssistantTurn && repliesAreNearDuplicate(replyText, String(lastAssistantTurn.text || ""))) {
    return { sent: false, reason: "near_duplicate_reply" };
  }
  ...
  const metaForGuard =
    profileRow.channel_metadata && typeof profileRow.channel_metadata === "object"
      ? profileRow.channel_metadata
      : {};
  const pastQualificationGuard = conversationPastQualificationStage(
    recentTurns,
    nextSummary,
    metaForGuard,
  );
  if (
    isSalesRepetitionComplaintMessage(message) &&
    /temsilcisi|representative|bilgi mi arıyorsunuz|looking for information|პაციენტ|hasta mısınız/i.test(String(replyText || ""))
  ) {
    replyText = buildRepetitionComplaintReply(lang);
    salesIntent = "general";
  } else if (looksLikeSalesMisnamedReply(message, replyText)) {
    const fixed = resolveSalesDirectReply(message, lang, metaForGuard, recentTurns);
    if (fixed?.reply) {
      replyText = fixed.reply;
      salesIntent = fixed.intent || salesIntent;
      salesLead = mergeVisitorTypeIntoSalesLead(salesLead, fixed.visitorType || null);
    }
  } else if (
    isSalesTutorialVideoQuery(message) &&
    (looksLikeDeniesTutorialVideos(replyText) ||
      !String(replyText || "").includes(getCliniflyTutorialYoutubeUrl()))
  ) {
    replyText = buildTutorialVideosReply(lang);
    salesIntent = "tutorial_videos";
  } else if (
    looksLikeDeniesClinicAiTraining(replyText) &&
    conversationAskedClinicAiTraining(recentTurns, message)
  ) {
    replyText = buildAiClinicTrainingReply(lang);
    salesIntent = "ai_clinic_knowledge_training";
  } else if (
    shouldBlockQualificationRestartReply(recentTurns, nextSummary, metaForGuard) &&
    looksLikeGreetingQualificationRestartReply(replyText)
  ) {
    const fixed = resolveSalesDirectReply(message, lang, metaForGuard, recentTurns);
    if (fixed?.reply) {
      replyText = fixed.reply;
      salesIntent = fixed.intent || "general";
      salesLead = mergeVisitorTypeIntoSalesLead(salesLead, fixed.visitorType || null);
    } else {
      const substantive = resolveSalesSubstantiveFallbackReply(message, lang, metaForGuard);
      if (substantive?.reply) {
        replyText = substantive.reply;
        salesIntent = substantive.intent || "general";
      } else {
        replyText = buildMidChatContinuationReply(lang, recentTurns);
        salesIntent = "general";
      }
    }
  } else if (
    isSalesDentalConsumerQuestion(message) &&
    shouldUseConsumerAdMessengerFlow(message, { recentTurns, channelMetadata: metaForGuard }) &&
    (looksLikeRoleQualificationReply(replyText) ||
      salesIntent === "visitor_discovery" ||
      salesIntent === "dental_consumer_question" ||
      looksLikeDentalEducationSalesReply(replyText))
  ) {
    replyText = buildSmileAnalysisAppDownloadReply(lang, metaForGuard);
    salesIntent = "smile_score_conversion";
    salesLead = mergeVisitorTypeIntoSalesLead(salesLead, "patient");
  } else if (
    isSalesSmileAnalysisAdIntent(message, {
      recentTurns,
      channelMetadata: metaForGuard,
    }) &&
    (looksLikeGreetingQualificationRestartReply(replyText) ||
      salesIntent === "greeting_qualification")
  ) {
    ...
    replyMentionsUnverifiedIdentity(replyText, statedIdentity, metaForGuard) ||
    (looksLikeProfileIdentityEchoReply(replyText) &&
      (assistantRecentlyAckedProfileIdentity(recentTurns) || hasKnownSalesLeadIdentity(metaForGuard)))
  ) {
    if (isSalesMediaOnlyInbound(message)) {
      return { sent: false, reason: "duplicate_identity_echo_skip" };
    }
    const substantive = resolveSalesSubstantiveFallbackReply(message, lang, metaForGuard);
    if (substantive?.reply) {
      replyText = substantive.reply;
  ...
  replyText = enforceSalesAppDownloadReply(message, replyText, lang);
  replyText = sanitizePartnerClinicLanguageLeak(replyText, lang);
  replyText = sanitizeSalesRegistrationUrls(replyText, {
    preserveAppStoreLinks:
      isSalesMobileAppDownloadQuery(message) || isSalesPhotoUploadInbound(message),
  });
  ...
  const channelMetadata = {
    ...mergeSalesLeadMetadata(salesLead, prevMeta, statedIdentity),
    clinifly_sales_kb: {
