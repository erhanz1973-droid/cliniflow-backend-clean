...
function buildTreatmentIntakeGuidanceBlock(classification) {
  ...
  if (tier === "high") {
    ...
    lines.push(
      "* If the patient asks about price or brands, answer from clinic pricing data FIRST (see pricing block), then mention imaging if still needed for planning.",
    );
    lines.push(
      "* Panoramic X-ray / imaging may be needed before final planning — mention after giving ranges, not instead of answering commercial questions.",
    );
