...
const SELF_SERVICE_REGISTER_URL = {
  ka: "https://www.clinifly.net/ka",
  tr: "https://www.clinifly.net/tr",
  en: "https://www.clinifly.net",
  ru: "https://www.clinifly.net",
};
...
function buildPrimaryCtaGuidance(lang) {
  ...
  const url = SELF_SERVICE_REGISTER_URL[key] || SELF_SERVICE_REGISTER_URL.en;

  if (key === "ka") {
    return `PRIMARY CTA (use when closing — do NOT ask for demo or meeting time unless visitor explicitly asked for a demo):
თქვენ შეგიძლიათ უფასოდ დაამატოთ თქვენი კლინიკა Clinifly-ში საკრედიტო ბარათის გარეშე და თავად გამოსცადოთ სისტემა.
გადადით: ${url}
აირჩიეთ „კლინიკის დამატება“ და დაარეგისტრირეთ თქვენი კლინიკა რამდენიმე წუთში.
Emphasize: free trial, no credit card, self-service onboarding, immediate access.`;
  }
  if (key === "tr") {
    return `PRIMARY CTA (default — no demo push):
Kliniğinizi ücretsiz ekleyin — kredi kartı gerekmez; sistemi kendiniz deneyin.
${url} → "Klinik ekle" ile birkaç dakikada kayıt.
Vurgu: ücretsiz deneme, self-servis, anında erişim.`;
  }
  if (key === "ru") {
    return `PRIMARY CTA (default — no demo push):
Добавьте клинику бесплатно без карты и попробуйте сами: ${url}
Акцент: бесплатный trial, self-service, мгновенный доступ.`;
  }
  return `PRIMARY CTA (default — do NOT ask for demo or meeting unless visitor explicitly requested a demo):
Add your clinic free — no credit card — and explore Clinifly yourself.
Go to: ${url}
Choose "Add clinic" and register in a few minutes.
Emphasize: free trial, no credit card, self-service onboarding, immediate access.`;
}
...
4. CTA — default: invite free self-service registration (no credit card, try the platform yourself, immediate access). Include the registration URL from PRIMARY CTA when you close the message.
