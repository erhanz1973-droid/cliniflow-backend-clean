...
const { CLINIFLY_PATIENT_WEB_URL } = require("./patientClinicEnrollment");
const {
...
/**
 * Consumer smile-analysis ad traffic (Facebook / Instagram) — not clinic B2B qualification.
 * @param {string} message
...
function isSalesSmileAnalysisAdIntent(message, ctx = {}) {
  const m = String(message || "").trim();
  if (!m) return false;
  if (VISITOR_CLINIC_RE.test(m) || VISITOR_PARTNER_RE.test(m) || PATIENT_ACQUISITION_RE.test(m)) {
    return false;
  }
  if (SMILE_ANALYSIS_AD_RE.test(m) || SMILE_AI_CONSUMER_AD_RE.test(m)) return true;
  const recentTurns = ctx.recentTurns || [];
  const pastQual = conversationPastQualificationStage(
    recentTurns,
    null,
    ctx.channelMetadata,
  );
  if (pastQual) return false;
  const lang = inferSalesConversationLanguage(m);
  if (
...
function buildSmileAnalysisAppDownloadReply(lang) {
  ...
  if (key === "ka") {
    return (
      "გამარჯობა! 👋\n\n" +
      "🦷 გსურთ გაიგოთ, რა შეიძლება გაუმჯობესდეს თქვენს ღიმილში?\n\n" +
      "საუკეთესო გამოცდილებისთვის გირჩევთ ჩამოტვირთოთ Clinifly აპლიკაცია.\n\n" +
      "📸 აპლიკაციაში შეგიძლიათ ატვირთოთ თქვენი კბილების ფოტო, მიიღოთ AI-ის პერსონალიზებული რეკომენდაციები და სურვილის შემთხვევაში დაუკავშირდეთ კლინიკებს.\n\n" +
      `📲 ჩამოტვირთეთ Clinifly და დაიწყეთ უფასოდ: ${url}`
    );
  }
  if (key === "tr") {
    return (
      "Merhaba! 👋\n\n" +
      "🦷 Gülüşünüzü nasıl geliştirebileceğinizi öğrenmek ister misiniz?\n\n" +
      "En iyi deneyim için Clinifly uygulamasını indirmenizi öneririz.\n\n" +
      "📸 Uygulamada diş fotoğrafınızı yükleyebilir, yapay zekadan kişisel öneriler alabilir ve isterseniz kliniklerle iletişime geçebilirsiniz.\n\n" +
      `📲 Clinifly'ı indirin ve ücretsiz başlayın: ${url}`
    );
  }
  if (key === "ru") {
    return (
      "Здравствуйте! 👋\n\n" +
      "🦷 Хотите узнать, что можно улучшить в вашей улыбке?\n\n" +
      "Для лучшего опыта рекомендуем скачать приложение Clinifly.\n\n" +
      "📸 В приложении вы можете загрузить фото зубов, получить персональные рекомендации от ИИ и при желании связаться с клиниками.\n\n" +
      `📲 Скачайте Clinifly и начните бесплатно: ${url}`
    );
  }
  return (
    "Hello! 👋\n\n" +
    "🦷 Would you like to learn what could improve your smile?\n\n" +
    "For the best experience, we recommend downloading the Clinifly app.\n\n" +
    "📸 In the app you can upload photos of your teeth, get personalized AI recommendations, and connect with clinics if you wish.\n\n" +
    `📲 Download Clinifly and start for free: ${url}`
  );
}
...
function resolveSalesDirectReply(message, lang, channelMetadata, recentTurns = []) {
  const m = String(message || "").trim();
  if (!m) return null;
  if (isSalesSmileAnalysisAdIntent(m, { recentTurns, channelMetadata })) {
    return {
      reply: buildSmileAnalysisAppDownloadReply(lang),
...
function detectSalesConversationIntent(message, ctx = {}) {
  ...
  if (isSalesSmileAnalysisAdIntent(m, ctx)) return "smile_analysis_app";
...
• Assume they want smile insights via the Clinifly patient app.
• Guide: upload teeth photo → AI personalized recommendations → optional clinic connection.
• Encourage app install: ${url}
• Keep 3–5 short sentences; no clinic registration URL, no B2B pitch.`,
...
• Clinifly hasta uygulamasına yönlendir: fotoğraf yükle → AI önerileri → isteğe bağlı klinik.
• Uygulama: ${url}
• Kısa ve aksiyon odaklı; B2B kayıt linki yok.`,
    ka: `ACTIVE PLAYBOOK — ღიმილის ანალიზის რეკლამა (მომხმარებელი):
Facebook/Instagram ღიმილის/AI რეკლამიდან მოვიდა.
• არ ჰკითხოთ კლინიკის წარმომადგენელი ხართ თუ კლინიკას ეძებთ.
• Clinifly აპში: ფოტო → AI რეკომენდაციები → სურვილისამებრ კლინიკა.
• ჩამოტვირთვა: ${url}`,
    ru: `ACTIVE PLAYBOOK — РЕКЛАМА АНАЛИЗА УЛЫБКИ:
Перешли по рекламе улыбки / AI в Facebook или Instagram.
• НЕ спрашивайте представитель клиники или ищет клинику.
• Направьте в приложение Clinifly: фото → рекомендации ИИ → клиники по желанию.
• Скачать: ${url}`,
  };
  return blocks[key] || blocks.en;
}
...
Stage 1 — Qualification: pure hello/hi/salam only → welcome + ask clinic representative vs visitor seeking information (or how can we help). NO product pitch. Never use the word "patient" in the greeting question.
Stage 2 — Discovery: learn visitor type (clinic owner, dentist, manager, visitor seeking a clinic, partner) and what they need. ONE question at a time. NO long feature lists.
...
If visitor is seeking a clinic (not a clinic representative) → never sell the clinic platform; redirect to their clinic or the Clinifly app with clinic code.`;
