...
const SMILE_ANALYSIS_AD_RE =
  /(?:ღიმილ|სრულყოფ|რა\s*აკლია|გეტყვით|გეტყვი|გაიგოთ.*ღიმილ|gul[uü]ş|gul[uü]msem|g[uü]l[uü]ş|g[uü]l[uü]msem|smile\s*(analysis|perfect|improv|makeover)|perfect\s*smile|what.*(missing|improve|wrong).*smile|improve\s*(my\s*)?smile|улучшить\s*улыб|идеальн.*улыб|улыбк.*соверш|анализ\s*улыб)/i;
 ...
 * @param {string} lang
 */
function buildSmileAnalysisAppDownloadReply(lang) {
  return buildSmileAnalysisMessengerConversionReply(normalizeLocale(lang));
}
...
function resolveSalesDirectReply(message, lang, channelMetadata, recentTurns = []) {
  ...
  if (
    ...
    return {
      reply: buildConsumerAdWelcomeReply(lang),
 ...
 * @param {string} lang
 */
function buildConsumerAdWelcomeReply(lang) {
  const key = normalizeLocale(lang);
  if (key === "ka") {
    return "გამარჯობა 👋 მე Clinifly AI ვარ. შემიძლია დაგეხმაროთ კბილებისა და ღიმილის შესახებ კითხვებში. რით შემიძლია დაგეხმაროთ?";
  }
  if (key === "tr") {
    return "Merhaba 👋 Ben Clinifly AI. Dişleriniz ve gülüşünüz hakkındaki sorularınızda yardımcı olabilirim. Size nasıl yardımcı olabilirim?";
  }
  if (key === "ru") {
    return "Здравствуйте 👋 Я Clinifly AI. Могу помочь с вопросами о зубах и улыбке. Чем могу помочь?";
  }
  return "Hello 👋 I'm Clinifly AI. I can help answer questions about your teeth and smile. What would you like to know?";
}
 ...
 * @param {string} lang
 */
function buildPhotoUploadAppReply(lang) {
  const key = normalizeLocale(lang);
  const intros = {
    ka: "ფოტოს გამოგზავნისთვის გმადლობთ. AI-ის დაფუძნებული ფოტო ანალიზი Clinifly აპლიკაციაში ხდება — Messenger-ში არა.",
    tr: "Fotoğrafınız için teşekkürler. AI destekli fotoğraf analizi Clinifly uygulamasında yapılır — Messenger içinde değil.",
    ru: "Спасибо за фото. Анализ с помощью ИИ доступен в приложении Clinifly — не в Messenger.",
    en: "Thanks for sending a photo. AI-powered photo analysis is available inside the Clinifly app — not in Messenger.",
  };
  return buildMobileAppDownloadReply(key, { introLine: intros[key] || intros.en });
}
 ...
 * @param {string} lang
 */
function buildDentalConsumerQuestionFallbackReply(message, lang) {
  const key = normalizeLocale(lang);
  const m = String(message || "").trim();
  const topic = detectPatientTreatmentTopic(m);
  const valplast = /valplast/i.test(m);
  const singleTooth = /single\s+tooth|one\s+tooth|tek\s+diş|bir\s+diş/i.test(m);
  const upperJaw = /upper\s+jaw|üst\s+çene|ust\s+cene|upper\s+arch/i.test(m);
  const guidance = buildDentalConsumerAppGuidanceLine(key);

  if (valplast || (topic?.id === "prosthetic" && (valplast || singleTooth))) {
    if (key === "tr") {
      return (
        "Evet — birçok durumda üst çenede tek eksik diş için Valplast esnek parsiyel protez kullanılabilir. Uygunluk; eksik dişin konumu, çevre dişlerin durumu ve ağız sağlığınıza bağlıdır. Kesin öneri için bir diş hekimi muayenesi gerekir.\n\n" +
        guidance
      );
    }
    if (key === "ka") {
      return (
        "დიახ — ხშირად ზედა ხეოში ერთი აკლებული კბილის შემთხვევაში შესაძლებელია Valplast მოქნილი ნაწილობრივი პროთეზი. შესაბამისობა დამოკიდებულია კბილის მდებარეობაზე, გარშემო კბილების მდგომარეობაზე და პირის ჯანმრთელობაზე — დიაგნოზს მხოლოდ სტომატოლოგი დაადგენს.\n\n" +
        guidance
      );
    }
    if (key === "ru") {
      return (
        "Да — во многих случаях для одного отсутствующего зуба на верхней челюсти можно использовать гибкий частичный протез Valplast. Пригодность зависит от положения зуба, состояния соседних зубов и здоровья полости рта — окончательную рекомендацию даст стоматолог.\n\n" +
        guidance
      );
    }
    const jawHint = upperJaw ? " in the upper jaw" : "";
    return (
      `Yes, in many cases a Valplast flexible partial denture can be used to replace a single missing tooth${jawHint}. However, suitability depends on the location of the missing tooth, the condition of surrounding teeth, and your oral health. A dentist would need to evaluate your situation to provide a recommendation.\n\n` +
      guidance
    );
  }

  const body = topicAnchoredFallbackReply(m, key);
  return `${body}\n\n${guidance}`;
}
 ...
 * @param {string} lang
 */
function buildDentalConsumerQuestionPlaybook(lang) {
  const key = normalizeLocale(lang);
  const guidance = buildDentalConsumerAppGuidanceLine(key);
  const blocks = {
    en: `ACTIVE PLAYBOOK — DENTAL CONSUMER QUESTION:
The visitor asked a dental/treatment question (NOT Clinifly clinic platform setup).
• Answer the dental question FIRST in 2–4 sentences with helpful general information. No diagnosis or guarantees.
• Do NOT ask if they are a clinic representative, patient, or looking for information.
• Do NOT ask for a clinic code or admin registration.
• Assume they are a regular user (especially from ads) unless they clearly said they are a clinic owner/manager.
• After answering, ONE gentle Clinifly app line (no clinic code): "${guidance}"
• Set salesLead.visitorType=patient. NEVER admin-register.html.`,
    tr: `ACTIVE PLAYBOOK — DİŞ TEDAVİSİ SORUSU (tüketici):
Önce diş/tedavi sorusunu yanıtlayın — genel bilgilendirme, teşhis yok.
• Klinik temsilcisi / hasta / bilgi mi SORMAYIN. Klinik kodu istemeyin.
• Reklamdan gelenleri normal kullanıcı kabul edin.
• Sonra kısa Clinifly uygulama satırı: "${guidance}"
• salesLead.visitorType=patient. admin-register YOK.`,
    ka: `ACTIVE PLAYBOOK — სტომატოლოგიური კითხვა:
ჯერ პასუხი კბილის/მკურნალობის შესახებ — ზოგადი ინფო, დიაგნოზი არა.
• არ ჰკითხოთ წარმომადგენელი/პაციენტი. კლინიკის კოდი არა.
• ბოლოს: "${guidance}" — visitorType=patient.`,
    ru: `ACTIVE PLAYBOOK — СТОМАТОЛОГИЧЕСКИЙ ВОПРОС:
Сначала ответ на вопрос о лечении — общая информация, без диагноза.
• Не спрашивайте представитель клиники / пациент. Без кода клиники.
• В конце: "${guidance}" — visitorType=patient.`,
  };
 ...
 * @param {string} lang
 */
function buildConsumerAdWelcomePlaybook(lang) {
  const key = normalizeLocale(lang);
  const example = buildConsumerAdWelcomeReply(key);
  const blocks = {
    en: `ACTIVE PLAYBOOK — CONSUMER AD WELCOME (Facebook/Instagram):
Pure hello from an ad visitor interested in teeth/smile — NOT clinic B2B.
• You are Clinifly AI. Warm welcome + offer to help with teeth/smile questions.
...
მხოლოდ გამარჯობა — Clinifly AI. კბილები/ღიმილი. არა კლინიკის წარმომადგენელი. არა აპის ბმული ამ ტურზე.`,
    tr: `ACTIVE PLAYBOOK — REKLAM SELAMI:
Sadece merhaba — Clinifly AI. Diş/gülüş soruları. Klinik temsilcisi sorma. Bu turda uygulama linki yok.`,
    ru: `ACTIVE PLAYBOOK — ПРИВЕТСТВИЕ С РЕКЛАМЫ:
Clinifly AI. Вопросы о зубах/улыбке. Без вопроса про клинику. Без ссылки на приложение в приветствии.`,
  };
  return blocks[key] || blocks.en;
}
...
• Thank them; explain AI photo analysis is only in the Clinifly app.
• Provide App Store + Google Play links.
...
function resolveSalesSubstantiveFallbackReply(message, lang) {
  const m = String(message || "").trim();
  if (!m) return null;
  if (isSalesDentalConsumerQuestion(m)) {
    return {
      reply: buildDentalConsumerQuestionFallbackReply(m, lang),
...
function buildSalesPlaybookBlock(intent, lang, message = "") {
  ...
  if (intent === "dental_consumer_question") {
    return buildDentalConsumerQuestionPlaybook(lang);
  }
  if (intent === "consumer_ad_welcome") {
    return buildConsumerAdWelcomePlaybook(lang);
  }
...
const CONSUMER_AD_MESSENGER_RULES = `FACEBOOK / INSTAGRAM AD VISITORS — You are Clinifly AI:
Users from ads are usually interested in improving their smile or learning about their teeth.
1. Answer dental questions naturally and helpfully — general information only, no diagnosis.
2. Do NOT ask if they are a clinic representative.
3. Do NOT ask for a clinic code.
4. Do NOT immediately redirect elsewhere — answer first, then gently mention the app.
5. After answering, encourage installing the Clinifly app (use the recommended closing line for their language).
6. If they ask where to download the app → App Store + Google Play links.
7. If they upload a photo → explain photo/AI analysis is in the Clinifly app + download links. Never analyze photos in Messenger.`;
...
• If the visitor asks a dental or treatment question (implants, prosthesis, Valplast, veneers, smile, etc.) → answer the question FIRST with brief general education. No diagnosis.
...
• After answering, use the recommended app encouragement line for their language (Georgian: უფრო პერსონალიზებული რეკომენდაციების მისაღებად, შეგიძლიათ ჩამოტვირთოთ Clinifly აპლიკაცია, ატვირთოთ თქვენი კბილების ფოტო და მიიღოთ AI-ის რეკომენდაციები.).
...
module.exports = {
  ...
  buildConsumerAdMessengerRulesBlock: () => CONSUMER_AD_MESSENGER_RULES,
