 ...
 * @param {string} lang
 */
function buildDentalConsumerQuestionFallbackReply(_message, lang) {
  return buildSmileAnalysisMessengerConversionReply(normalizeLocale(lang));
}
...
function buildDentalConsumerQuestionPlaybook(lang) {
  const key = normalizeLocale(lang);
  const example = buildSmileAnalysisMessengerConversionReply(key);
  const blocks = {
    en: `ACTIVE PLAYBOOK — SMILE SCORE CONVERSION (Facebook/Instagram ad consumer):
The visitor asked about their smile/teeth. Do NOT explain veneers, implants, orthodontics, whitening, or other treatments in Messenger.
• Goal: move them into the Clinifly app Smile Score flow.
• Short reply only (about 4–6 lines): invite smile photo upload → AI Smile Score → fast personalized insights → App Store + Google Play links.
...
Diş/gülüş sorusu — tedavi eğitimi YOK (kaplama, implant, ortodonti vb.).
• Kısa yanıt: fotoğraf yükle → Smile Score → AI analiz → uygulama linkleri.
• Klinik temsilcisi/kodu SORMA. visitorType=patient.`,
    ka: `ACTIVE PLAYBOOK — SMILE SCORE კონვერსია (რეკლამიდან):
ღიმილი/კბილის კითხვა — არა გრძელი სტომატოლოგიური ლექცია.
• მოკლე პასუხი: ფოტო → Smile Score → AI → აპის ბმულები.
• visitorType=patient.`,
    ru: `ACTIVE PLAYBOOK — SMILE SCORE (реклама):
Вопрос об улыбке — без лекций о винирах, имплантах, ортодонтии.
• Короткий ответ: фото → Smile Score → AI → ссылки на приложение.
• visitorType=patient.`,
  };
...
const CONSUMER_AD_MESSENGER_RULES = `FACEBOOK / INSTAGRAM AD VISITORS — You are Clinifly AI (Smile Score conversion):
Users from ads want to know what could improve their smile. Your job is to move them into the Clinifly app — NOT to teach dentistry in Messenger.
...
3. Short reply only: invite smile photo upload → Clinifly AI Smile Score → fast personalized insights → App Store + Google Play links.
