...
const { localizeOperationalStatusLabel } = require("./i18n/localizeCoordination");
...
const STATUS_LABELS = {
  waiting_for_patient: "Waiting on user",
 ...
 * @param {string} [lang]
 */
function buildCoordinationProjection(lead, ctx = {}, lang = "en") {
  ...
  return {
    ...
    operationalStatusLabel:
      localizeOperationalStatusLabel(operationalStatus, L) ||
      STATUS_LABELS[operationalStatus] ||
      operationalStatus,
...
async function projectCoordinationState(profileId, opts = {}) {
  ...
  if (opts.force !== true) {
    const { data: freshFlagsRow } = await supabase
      .from("ai_coordinator_lead_profiles")
      .select("operational_intake_flags")
      .eq("id", profileId)
      .maybeSingle();
    const flagsEarly =
      freshFlagsRow?.operational_intake_flags &&
      typeof freshFlagsRow.operational_intake_flags === "object"
        ? freshFlagsRow.operational_intake_flags
        : {};
    const projectedAt = flagsEarly.projectionUpdatedAt || flagsEarly.projection_updated_at;
    if (projectedAt) {
      const ageMs = Date.now() - new Date(String(projectedAt)).getTime();
      if (Number.isFinite(ageMs) && ageMs >= 0 && ageMs < 120_000) {
        return { ok: true, skipped: true, operationalIntakeFlags: flagsEarly };
      }
    }
  }
...
async function attachPatientNamesToProfileRows(rows) {
  ...
  const { data: patients } = await supabase
    .from("patients")
    .select("id, full_name, name, first_name, last_name, email, phone, external_name, display_name_override")
    .in("id", ids);
