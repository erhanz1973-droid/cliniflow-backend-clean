...
function buildNameCollectionPromptBlock(evalResult, contextMode = "coordinator", lang = "tr") {
  ...
  return `USER NAME COLLECTION (${ch} — ask naturally, once):
* You do not know the user's real name yet. After briefly answering their question, politely ask for their name once this turn.
* Example (${code}): ${examples}
* WhatsApp contacts already show a display name — do NOT use this block on WhatsApp. This user is on ${ch}.
...
function buildNameAcknowledgmentPromptBlock(name, lang = "tr") {
  ...
  const code = String(lang || "tr").slice(0, 2).toLowerCase();
  if (code === "en") {
    return `USER NAME RECEIVED: The user shared their name (${n}). Thank them briefly (e.g. "Thank you, ${n.split(" ")[0]}!") and continue the conversation. Do not ask for their name again.`;
  }
  if (code === "ru") {
    return `ИМЯ ПОЛУЧЕНО: Пользователь назвал себя (${n}). Кратко поблагодарите и продолжайте — больше не спрашивайте имя.`;
  }
  if (code === "ka") {
    return `სახელი მიღებულია: მომხმარებელმა თქვა (${n}). მოკლედ მადლობა უთხარით და გააგრძელეთ — სახელი აღარ ჰკითხოთ.`;
  }
  return `İSİM ALINDI: Kullanıcı adını paylaştı (${n}). Kısaca teşekkür edin (ör. "Teşekkürler ${n.split(" ")[0]}!") ve konuşmaya devam edin — adını tekrar sormayın.`;
}
