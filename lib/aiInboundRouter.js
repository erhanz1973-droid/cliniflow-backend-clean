...
function buildStaleSchedulingResetPatch(flags, message, timezone = "Europe/Istanbul") {
  ...
  const conflictsWithStale =
    ...
    (d.selectedDate && String(d.selectedDate) !== newDate) ||
    (Array.isArray(d.offeredSlots) &&
      d.offeredSlots.length > 0 &&
      d.offeredSlots.some((s) => s?.dateYmd && String(s.dateYmd) !== newDate));

  if (!conflictsWithStale && stalePreferred === newDate && String(d.selectedDate || "") === newDate) {
    return null;
  }

  if (!conflictsWithStale && !stalePreferred && !d.selectedDate && !d.offeredSlots?.length) return null;
