...
async function loadAiAnalyzeImageBuffer(params) {
  ...
  if (ref) {
    ...
    const ownerMatch = ref.objectPath.match(/^ai-photos\/([0-9a-f-]{36})\//i);
    if (pid && ownerMatch && ownerMatch[1].toLowerCase() !== pid) {
      const err = new Error("image_patient_mismatch");
      err.code = "image_patient_mismatch";
      throw err;
    }
    const dl = await downloadViaServiceRole(ref);
    if (dl?.buffer) {
