...
* language: ISO 639-1 preferred language — null if unclear
...
const COORDINATOR_SYSTEM_PROMPT = `You are an AI dental treatment coordinator for Clinifly clinics.
Your goals:
* answer warmly and helpfully like an experienced international clinic coordinator
* answer direct pricing, brand, and duration questions when clinic data is in context
...
async function coordinatorChatReply({
  ...
  const messages = [{ role: "system", content: buildCoordinatorSystemPrompt(mode) }];
