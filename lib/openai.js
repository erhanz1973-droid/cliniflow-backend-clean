...
* when Dental Tourism KB is in context and no clinic quotation exists: explain cost factors and encourage treatment request + photo upload — never invent exact prices
...
* Reinforce when appropriate (in the user's language): final clinical evaluation is performed by licensed dental professionals.
...
Dental tourism (when Dental Tourism KB is in context):
* Help users understand options, compare clinics, and submit treatment requests with photos.
* Do not invent exact prices — encourage personalized clinic quotations.
* Never guarantee outcomes or call one clinic the best.
 ...
 *   smileAnalysisContext?: string|null,
 *   dentalTourismContext?: string|null,
...
async function coordinatorChatReply({
  ...
  smileAnalysisContext = null,
  dentalTourismContext = null,
  maxTokens = null,
}) {
  ...
  const smileCtx = smileAnalysisContext ? String(smileAnalysisContext).trim() : "";
  if (smileCtx) latestBlock += `${smileCtx}\n\n`;
  ...
  if (smileCtx && mode === "treatment_guide") {
    systemPrompt = `${systemPrompt}\n\n${smileCtx}`;
  }
  const tourismCtx = dentalTourismContext ? String(dentalTourismContext).trim() : "";
  if (tourismCtx) {
    systemPrompt = `${systemPrompt}\n\n${tourismCtx}`;
  }
  ...
  const finishCoordinatorTurn = (content, modelName) => {
    ...
    let safeReply = applyReplyGuardrails(parsed.reply, {
      userMessage: userText,
      conversationLanguage: conversationLanguage || parsed.leadData?.language || null,
    });
    ...
    if (mode === "treatment_guide") {
      safeReply = applyTreatmentGuideDisclaimer(
        safeReply,
        conversationLanguage || parsed.leadData?.language || null,
      );
    }
