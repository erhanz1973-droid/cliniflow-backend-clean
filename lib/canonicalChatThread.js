...
/** Hot-path reads — no merge, archive, clinic repair, or profile creation. */
const CANONICAL_THREAD_READ_OPTS = {
  readOnly: true,
  repairClinic: false,
  ensureProfile: false,
 ...
 */
function logCanonicalPerf(tag, ms, payload) {
  try {
    console.log(
      tag,
      JSON.stringify({
...
async function archiveCrossClinicStaleThreads(patientId, canonicalClinicId) {
  ...
  if (!candidates.length) {
    logCanonicalPerf("[PERF_ARCHIVE_STALE]", Date.now() - t0, {
      patient_id: pid.slice(0, 8),
...
async function getCanonicalThread(patientId, clinicId, opts = {}) {
  ...
  const mergeDuplicates = readOnly ? false : opts.mergeDuplicates !== false;
  const archiveCrossClinicStale = readOnly ? false : opts.archiveCrossClinicStale === true;
  const repairClinic = readOnly ? false : opts.repairClinic !== false;
  const ensureProfile = readOnly ? false : opts.ensureProfile !== false;
  ...
  if (mergeDuplicates && operationalClinic) {
  ...
  if (archiveCrossClinicStale && operationalClinic) {
    await archiveCrossClinicStaleThreads(pid, operationalClinic);
  }

  let clinicRepair = { repaired: false };
  if (repairClinic && operationalClinic) {
    clinicRepair = await repairPatientClinicConsistency(pid, operationalClinic, source);
  }

  let profile = { profileId: null, created: false };
  if (ensureProfile && operationalClinic) {
    profile = await ensureCoordinatorProfile(pid, operationalClinic, { source });
  }

  const ms = Date.now() - t0;
  if (ms >= 50 || !readOnly) {
    logCanonicalPerf("[PERF_CANONICAL_THREAD]", ms, {
      patient_id: pid.slice(0, 8),
      source,
      readOnly,
      reason: resolved.reason,
      thread_id: resolved.threadId ? String(resolved.threadId).slice(0, 8) : null,
      clinic_repaired: clinicRepair.repaired === true,
      profile_created: profile.created === true,
    });
  }
...
async function resolveCanonicalChatThread(opts) {
  ...
  const readOnly = opts.readOnly === true;
  const mergeDupes = readOnly ? false : opts.mergeDuplicates !== false;
  if (!UUID_RE.test(pid)) {
  ...
  if (UUID_RE.test(clinicHint) && mergeDupes) {
  ...
  if (!chosen && operationalClinic && !readOnly) {
    try {
