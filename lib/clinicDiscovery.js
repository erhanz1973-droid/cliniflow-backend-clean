...
const { computeMarketplaceCompleteness, resolveDisplayMediaGallery } = require("./clinicMarketplaceProfile");
const { mapReputationForDiscovery } = require("./clinicReputation");
const { cityMatchesQuery } = require("./citySearchNormalize.cjs");
...
const DISCOVERY_SELECT_ATTEMPTS = [
  // Full marketplace profile (all migrations applied)
  `id, name, clinic_code, city, country, status, is_listed, logo_url, cover_photo_url, address, phone, email,
   ...
   trustpilot_url, google_reviews_url, facebook_page_url, facebook_recommendation_score,
   facebook_recommendation_count, reputation_sources, settings,
   ...
   working_hours, media_gallery`,
  // Core discovery + social/reputation (20260606120000) — skip extended trust columns if missing
  `id, name, clinic_code, city, country, status, is_listed, logo_url, cover_photo_url, address, phone, email,
   short_description, about_text, website, website_url, facebook_url, instagram_url, tiktok_url, linkedin_url, youtube_url,
   whatsapp, google_maps_url, google_rating, google_review_count, trustpilot_rating, trustpilot_review_count,
   trustpilot_url, google_reviews_url, is_verified, languages, specialties, services, technologies,
   working_hours, media_gallery`,
  // Social without youtube (if 20260607120000 not applied)
  `id, name, clinic_code, city, country, status, is_listed, logo_url, address, phone, email,
   short_description, about_text, website, website_url, facebook_url, instagram_url, tiktok_url, linkedin_url,
   whatsapp, google_maps_url, google_rating, google_review_count, trustpilot_rating, trustpilot_review_count,
   trustpilot_url, google_reviews_url, is_verified, languages, specialties`,
  // Minimal legacy fallback (no social columns; no latitude — column may be absent)
  `id, name, clinic_code, city, country, status, is_listed, logo_url, address, phone, email,
   short_description, website, website_url, google_maps_url, google_rating, google_review_count,
   is_verified, languages, specialties`,
...
function resolveFacebookUrl(row) {
  const social = row?.facebook_url != null ? String(row.facebook_url).trim() : "";
  const page = row?.facebook_page_url != null ? String(row.facebook_page_url).trim() : "";
  return social || page || null;
}
...
function parseMediaGallery(raw, row = null) {
  if (row) return resolveDisplayMediaGallery(row);
...
function computeDiscoveryRankScore(clinic) {
  const rating =
    clinic.primaryReputationScore ?? clinic.googleRating ?? clinic.trustpilotRating ?? 0;
  const reviews = Math.max(
    clinic.googleReviewCount ?? 0,
    clinic.trustpilotReviewCount ?? 0,
    clinic.facebookRecommendationCount ?? 0,
  );
...
function normalizeDiscoveryClinic(row, opts = {}) {
  ...
  const reputation = mapReputationForDiscovery(row);
  ...
  const base = {
    ...
    facebookUrl: resolveFacebookUrl(row),
  ...
  return {
    ...
    mediaGallery: parseMediaGallery(row?.media_gallery, row),
  };
}
...
function cityMatchesSimple(query, cityStr) {
  return cityMatchesQuery(query, cityStr);
}
...
function buildCompareHighlights(clinics) {
  ...
  const bestFacebook = clinics.reduce((best, c) => {
    if (c.facebookRecommendationScore == null) return best;
    if (
      !best ||
      (c.facebookRecommendationScore ?? 0) > (best.facebookRecommendationScore ?? 0)
    ) {
      return c;
    }
    return best;
  ...
  if (bestFacebook) {
    highlights.push({
      key: "best_facebook_recommendations",
      clinicId: bestFacebook.id,
      label: bestFacebook.name,
    });
  }
...
async function fetchListedClinicRows(supabase) {
  let lastErr = null;
  for (let i = 0; i < DISCOVERY_SELECT_ATTEMPTS.length; i++) {
    const sel = DISCOVERY_SELECT_ATTEMPTS[i];
    ...
    if (!error) {
      if (i > 0) {
        console.warn("[discovery/select] list using fallback attempt", i + 1, "of", DISCOVERY_SELECT_ATTEMPTS.length);
      }
  ...
  app.get("/api/discovery/clinics/compare", async (req, res) => {
    try {
      ...
      const criteria = clinics.map((c) => ({
        ...
        facebookRecommendationScore: c.facebookRecommendationScore,
        facebookRecommendationCount: c.facebookRecommendationCount,
        trustpilotRating: c.trustpilotRating,
        trustpilotReviewCount: c.trustpilotReviewCount,
        trustpilotUrl: c.trustpilotUrl,
        reputationSources: c.reputationSources,
        reputationDisplayLines: c.reputationDisplayLines,
    ...
    try {
      ...
      for (let i = 0; i < DISCOVERY_SELECT_ATTEMPTS.length; i++) {
        ...
        if (!error && data) {
          if (i > 0) {
            console.warn(
              "[discovery/select] profile using fallback attempt",
              i + 1,
              "of",
              DISCOVERY_SELECT_ATTEMPTS.length,
              "clinicId",
              clinicId.slice(0, 8),
            );
          }
