...
function normalizeAdminTimelineEventForIngest(evt, opts = {}) {
  ...
  const idNorm =
    out.id != null && trimStr(out.id) !== ""
      ? normalizeTimelineProcedureId(trimStr(out.id))
      : "";
  const procNorm =
    out.procedureId != null && trimStr(out.procedureId) !== ""
      ? normalizeTimelineProcedureId(trimStr(out.procedureId))
      : "";

  if (idNorm) out.id = idNorm;
  if (procNorm) out.procedureId = procNorm;
  // Canonical identity fallback: timeline correlation/dedupe relies on stable procedureId.
  if (!trimStr(out.procedureId) && idNorm) out.procedureId = idNorm;
...
function finalizeAdminTimelineAfterIso(evt, iso, tsParsed) {
  ...
  if (!trimStr(out.procedureId) && trimStr(out.id)) {
    out.procedureId = normalizeTimelineProcedureId(trimStr(out.id));
  }
