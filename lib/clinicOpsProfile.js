...
async function resolveClinicOpsKnowledge(clinicId) {
  const [settings, catalog, hotels, protocols] = await Promise.all([
    ...
    listProtocolsByClinic(clinicId, { activeOnly: true }),
  ]);
