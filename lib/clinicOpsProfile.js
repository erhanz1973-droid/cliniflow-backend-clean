...
const { buildMultilingualOrchestrationBlock } = require("./clinicMultilingual");
...
async function getClinicOpsProfile(clinicId) {
  ...
  return {
    ...
    sections: {
      ...
      conversionCoordinator: settings.knowledgeBase?.conversionEngine || null,
    },
    counts: {
...
async function patchClinicOpsSection(clinicId, sectionId, body) {
  ...
  switch (id) {
    ...
    case "conversion-coordinator":
      patch.knowledgeBase = { conversionEngine: body };
      break;
    case "ai-safety":
...
async function resolveClinicOpsKnowledge(clinicId) {
  ...
  const multilingual = buildMultilingualOrchestrationBlock(settings.tone);
  ...
  return {
    ...
    identity: {
      ...
      languages: settings.tone.enabledLanguageCodes || [],
      primaryLanguage: settings.tone.primaryLanguage,
      supportedLanguages: settings.tone.supportedLanguages,
