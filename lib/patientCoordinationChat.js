...
/**
 * Patient joined this clinic (shared-care) — offer_messages thread is archived; use patient_messages.
 */
...
async function ensureCoordinationOfferForRequest(requestId, opts = {}) {
  ...
  if (enrolled) {
    await ensureLeadWorkspaceForClinic(patientId, clinicId, {
      source: "coordination_chat_enrolled",
      leadStatus: tr.lead_status || LEAD_STATUS.CONVERTED_TO_PATIENT,
    ...
    try {
      const { data: cl } = await supabase
        ...
        .maybeSingle();
      clinicCode =
        cl?.clinic_code != null && String(cl.clinic_code).trim()
          ? String(cl.clinic_code).trim().toUpperCase()
          : null;
    } catch (_) {
      /* non-fatal */
    }
