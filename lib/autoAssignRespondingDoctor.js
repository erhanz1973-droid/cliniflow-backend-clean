 ...
 */
async function assignClinicDutyDoctorToPatient(patientId, clinicId, dutyDoctorIdRaw) {
  const pid = String(patientId || "").trim();
  const cid = String(clinicId || "").trim();
  if (!UUID_RE.test(pid) || !UUID_RE.test(cid)) return { ok: false, reason: "invalid_ids" };
...
async function assignDoctorOnPatientClinicJoin(patientId, clinicId) {
  ...
  /** Lead inbox routing (Burhan) wins over clinic duty doctor (Serap) when admin configured fixed_doctor. */
  const leadRoutingDoctorId = await resolveLeadRoutingDoctorForMembership(cid);
  if (leadRoutingDoctorId) {
    const result = await assignClinicDutyDoctorToPatient(pid, cid, leadRoutingDoctorId);
    if (result.ok) {
      return {
        ok: true,
        path: "clinic_lead_routing_fixed",
        doctorId: leadRoutingDoctorId,
        ...result,
      };
    }
  }
