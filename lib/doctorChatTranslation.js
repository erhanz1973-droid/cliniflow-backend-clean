 ...
 * from lib/mirrorOmnichannelPatientMessage.js
 * @param {string} messageId
 */
async function loadOmnichannelMirrorMessageByPublicId(messageId) {
  const id = String(messageId || "").trim();
  ...
  for (const sel of pmSelects) {
    ...
    if (row) return { ...row, storageTable: "patient_messages" };
  }
  ...
  const winStart = new Date(tsMs - 180000).toISOString();
  const winEnd = new Date(tsMs + 180000).toISOString();
  ...
  for (const sel of channelSelects) {
    ...
    for (const row of data || []) {
      ...
      if (meta.mirror_message_id === id || meta.omni_public_id === id) {
        const patientId = await resolvePatientIdFromCoordinatorProfile(row.profile_id);
        return {
          id,
          coordinatorSource: "channel",
      ...
      patientRows.push({ row, meta, body });
    }

    if (patientRows.length === 1) {
      const { row, meta, body } = patientRows[0];
      const patientId = await resolvePatientIdFromCoordinatorProfile(row.profile_id);
      return {
        id,
        coordinatorSource: "channel",
...
async function loadPatientMessageByPublicId(messageId) {
  ...
  if (id.startsWith("omni_")) {
    const omni = await loadOmnichannelMirrorMessageByPublicId(id);
    if (omni) return omni;
  }
