...
async function loadPatientMessageByPublicId(messageId) {
  ...
  const byUuid = await supabase.from("patient_messages").select(baseSelect).eq("id", id).maybeSingle();
  ...
  const byMessageId = await supabase
    ...
    .eq("message_id", id)
    .maybeSingle();
  return byMessageId.data || null;
}

function extractMessageBody(row) {
  const candidates = [row?.text, row?.message, row?.message_text, row?.content, row?.body];
...
async function translateDoctorChatMessage(params) {
  ...
  const translated = await translateChatMessageText(body, targetLang, {
