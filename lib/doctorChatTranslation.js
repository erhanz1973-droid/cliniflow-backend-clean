 ...
 */
async function resolveProfileIdsForPatient(patientId, clinicId) {
  ...
  if (UUID_RE.test(cid)) {
    const { data: strictRows } = await supabase
      ...
      .limit(12);
    for (const row of strictRows || []) add(row);
  }
  ...
  for (const row of byPatientRows || []) {
    ...
    if (!UUID_RE.test(cid) || !pc || pc === cid) add(row);
  }
  if (!profileIds.size) {
    for (const row of byPatientRows || []) add(row);
  }

  return [...profileIds];
}

function channelRowToOmniTranslateRow(id, row) {
...
/**
 * Omnichannel mirror ids: omni_whatsapp_{epochMs}_{suffix} / omni_messenger_{epochMs}_{suffix}
 * @param {string} messageId
 * @param {{ patientId?: string, clinicId?: string }} [opts]
 */
async function loadOmnichannelMirrorMessageByPublicId(messageId, opts = {}) {
  const id = String(messageId || "").trim();
  if (!id.startsWith("omni_") || !isSupabaseEnabled()) return null;

  const patientHint = String(opts.patientId || "").trim();
  const pmSelects = [
    "id, message_id, patient_id, clinic_id, thread_id, text, message, message_text, content, body, translation, from_role, created_at",
    "id, message_id, patient_id, clinic_id, text, message, message_text, body, from_role, created_at",
    "id, message_id, patient_id, text, message, from_role",
  ];

  const tryPmRow = (row) => {
    if (!row) return null;
    return { ...row, storageTable: "patient_messages" };
  };

  for (const sel of pmSelects) {
    let q = supabase.from("patient_messages").select(sel).eq("message_id", id).limit(3);
    if (UUID_RE.test(patientHint)) {
      q = q.eq("patient_id", patientHint);
    }
    const { data, error } = await q;
    if (error) {
      if (isMissingColumnError(error)) continue;
      console.warn("[doctorChatTranslation] omni patient_messages:", error.message);
      break;
    }
    const row = Array.isArray(data) ? data[0] : null;
    if (row) return tryPmRow(row);
  }

  const parsed = /^omni_(whatsapp|messenger)_(\d+)_([a-z0-9]+)$/i.exec(id);
  if (!parsed) return null;

  const channel = String(parsed[1]).toLowerCase();
  const tsMs = Number(parsed[2]);
  const suffix = String(parsed[3] || "").trim();
  ...
  if (suffix) {
    let suffixQ = supabase
      .from("patient_messages")
      .select(
        "id, message_id, patient_id, clinic_id, text, message, message_text, body, translation, from_role, created_at",
      )
      .like("message_id", `%_${suffix}`)
      .order("created_at", { ascending: false })
      .limit(12);
    if (UUID_RE.test(patientHint)) {
      suffixQ = suffixQ.eq("patient_id", patientHint);
    }
    const { data: suffixRows } = await suffixQ;
    for (const row of suffixRows || []) {
      if (String(row.message_id || "").trim() === id) {
        return tryPmRow(row);
      }
    }
  }

  const profileIds = UUID_RE.test(patientHint)
    ? await resolveProfileIdsForPatient(patientHint, opts.clinicId)
    : [];

  const channelSelects = [
    "id, profile_id, body, message_role, channel, metadata, created_at",
    "id, profile_id, body, message_role, created_at",
  ];

  for (const profileId of profileIds) {
    for (const sel of channelSelects) {
      let metaQ = supabase
        .from("ai_coordinator_channel_messages")
        .select(sel)
        .eq("profile_id", profileId)
        .filter("metadata->>mirror_message_id", "eq", id)
        .limit(3);
      let { data: metaRows, error: metaErr } = await metaQ;
      if (metaErr && isMissingColumnError(metaErr)) continue;
      if (!metaErr && Array.isArray(metaRows) && metaRows[0]) {
        const out = channelRowToOmniTranslateRow(id, metaRows[0]);
        if (out) {
          out.patient_id = await resolvePatientIdFromCoordinatorProfile(profileId);
          return out;
        }
      }
      ...
      metaQ = supabase
        ...
        .select(sel)
        .eq("profile_id", profileId)
        .filter("metadata->>omni_public_id", "eq", id)
        .limit(3);
      ({ data: metaRows, error: metaErr } = await metaQ);
      if (!metaErr && Array.isArray(metaRows) && metaRows[0]) {
        const out = channelRowToOmniTranslateRow(id, metaRows[0]);
        if (out) {
          out.patient_id = await resolvePatientIdFromCoordinatorProfile(profileId);
  ...
  const winStart = new Date(tsMs - 86_400_000).toISOString();
  const winEnd = new Date(tsMs + 86_400_000).toISOString();
  ...
  for (const sel of channelSelects) {
    ...
    const profilesToScan = profileIds.length ? profileIds : [null];
    for (const profileId of profilesToScan) {
      let pq = q;
      if (profileId) {
        pq = supabase
          .from("ai_coordinator_channel_messages")
      ...
      if (error) {
        if (isMissingColumnError(error)) continue;
        console.warn("[doctorChatTranslation] omni channel fallback:", error.message);
        continue;
      }
      ...
      for (const row of data || []) {
        if (!coordinatorChannelRoleIsPatient(row.message_role)) continue;
        const body = String(row.body || "").trim();
        if (!body) continue;
        const meta =
          row.metadata && typeof row.metadata === "object" && !Array.isArray(row.metadata)
            ? row.metadata
            : {};
        if (meta.mirror_message_id === id || meta.omni_public_id === id) {
          const out = channelRowToOmniTranslateRow(id, row);
          if (out) {
            out.patient_id = await resolvePatientIdFromCoordinatorProfile(row.profile_id);
            return out;
          }
        }
        const createdMs = Date.parse(String(row.created_at || ""));
        if (Number.isFinite(createdMs)) {
          const delta = Math.abs(createdMs - tsMs);
          if (delta < bestDelta) {
            bestDelta = delta;
            bestMeta = { row, meta, body };
            bestTime = createdMs;
          }
        }
      }

      if (bestMeta && bestDelta < 3_600_000) {
        const out = channelRowToOmniTranslateRow(id, bestMeta.row);
        if (out) {
          out.patient_id = await resolvePatientIdFromCoordinatorProfile(bestMeta.row.profile_id);
          console.log("[doctorChatTranslation] omni channel time match", {
            id: id.slice(0, 40),
            delta_ms: bestDelta,
          });
          return out;
  ...
  console.warn("[doctorChatTranslation] omni mirror not found:", id.slice(0, 56), {
    patient_hint: patientHint ? patientHint.slice(0, 8) : null,
 ...
 * @param {{ patientId?: string, clinicId?: string }} [opts]
 */
async function loadPatientMessageByPublicId(messageId, opts = {}) {
  const id = String(messageId || "").trim();
  if (!id || !isSupabaseEnabled()) return null;

  if (id.startsWith("omni_")) {
    const omni = await loadOmnichannelMirrorMessageByPublicId(id, opts);
    if (omni) return omni;
  }
  ...
  for (const sel of selects) {
    const { data: byUuidRows, error: byUuidErr } = await supabase
      .from("patient_messages")
      .select(sel)
      .eq("id", id)
      .limit(1);
    if (byUuidErr) {
      if (isMissingColumnError(byUuidErr)) continue;
      console.warn("[doctorChatTranslation] load by id:", byUuidErr.message);
    } else if (Array.isArray(byUuidRows) && byUuidRows[0]) {
      return { ...byUuidRows[0], storageTable: "patient_messages" };
    }

    const { data: byMsgRows, error: byMsgErr } = await supabase
      .from("patient_messages")
      .select(sel)
      .eq("message_id", id)
      .limit(1);
    if (byMsgErr) {
      if (isMissingColumnError(byMsgErr)) continue;
      console.warn("[doctorChatTranslation] load by message_id:", byMsgErr.message);
    } else if (Array.isArray(byMsgRows) && byMsgRows[0]) {
      return { ...byMsgRows[0], storageTable: "patient_messages" };
    }
  }
