...
} = require("./schemaStartupValidation");

const UUID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

/** @type {boolean|null} */
let variantsClinicIdColumnSupported = null;

function setVariantsClinicIdColumnSupported(supported) {
  variantsClinicIdColumnSupported = supported === true;
}

function isVariantsTableMissing(error) {
  return isTableOrSchemaCacheError(error, "treatment_price_variants");
}

function variantsTableMissingPayload(error) {
  const message = String(error?.message || error || "table unavailable");
  return {
    ok: false,
    error: "variants_table_missing",
    message,
    table: "public.treatment_price_variants",
    migration: `supabase/migrations/${VARIANTS_ENSURE_MIGRATION}`,
    hint:
      "Apply the migration in Supabase SQL editor, then wait ~60s or run NOTIFY pgrst, 'reload schema'; restart the API if errors persist.",
  };
}
...
function variantBodyToRow(body, priceId, clinicId) {
  ...
  if (variantsClinicIdColumnSupported === true && clinicId && UUID_RE.test(clinicId)) {
  ...
  for (let i = 0; i < incoming.length; i++) {
    ...
    if (v.id && UUID_RE.test(String(v.id))) {
      ...
      const { error } = await supabase
        ...
        .eq("treatment_price_id", priceId);
      if (error) {
        if (isVariantsTableMissing(error)) return variantsTableMissingPayload(error);
        if (String(error.message || "").includes("clinic_id")) {
          variantsClinicIdColumnSupported = false;
      ...
      if (error) {
        if (isVariantsTableMissing(error)) return variantsTableMissingPayload(error);
        if (String(error.message || "").includes("clinic_id") && payload.clinic_id) {
          variantsClinicIdColumnSupported = false;
          delete payload.clinic_id;
          const retry = await supabase
            .from("treatment_price_variants")
...
module.exports = {
  ...
  setVariantsClinicIdColumnSupported,
  variantsTableMissingPayload,
};
