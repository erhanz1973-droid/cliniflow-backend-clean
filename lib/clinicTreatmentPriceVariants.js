 ...
 */
function variantBodyToRow(body, priceId) {
 ...
 */
async function listVariantsByPriceIds(priceIds) {
...
async function syncVariantsForPrice(clinicId, priceId, variants) {
  ...
  for (let i = 0; i < incoming.length; i++) {
    const v = incoming[i];
    if (!String(v.brandName || "").trim()) continue;

    const payload = variantBodyToRow(
      { ...v, sortOrder: v.sortOrder != null ? v.sortOrder : i * 10, isDefault: v.isDefault === true && !defaultSet },
      priceId,
    );
