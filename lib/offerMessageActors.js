...
async function resolveClinicSideOfferSender(offerId, hints = {}) {
  ...
  return {
    senderId,
    /** UI labels AI as care team — not the assigned doctor. */
...
/**
 * Doctor-approved human message (clinical guidance send) — not AI coordinator.
 * @param {{
 *   offerId: string,
 *   message: string,
 *   senderName?: string,
 ...
 * }} params
 */
async function buildDoctorHumanOfferMessageInsert(params) {
  const offerId = String(params.offerId || "").trim();
  const text = String(params.message || "").trim();
  const doctorId = pickUuid(params.doctorId);
  if (!UUID_RE.test(offerId) || !text || !doctorId) {
    return { row: null, error: { message: "doctor_offer_insert_invalid" } };
  }
  const senderName = String(params.senderName || "Doctor").trim() || "Doctor";
  const messageSource =
    params.sendMode === "direct" || params.messageSource === "doctor_direct"
      ? "doctor_direct"
      : "doctor";
  const row = {
    offer_id: offerId,
  ...
  return {
    row,
    actor: { senderId: doctorId, senderRole: "doctor", actorKind: "doctor" },
    error: null,
  };
}
