...
const PROTOCOLS_TABLE_MISSING_HINT =
  "Treatment protocols table is not in Supabase yet. Run migration 20260518160000_clinic_treatment_protocols_ensure.sql in the SQL editor, then wait ~1 minute for the API schema cache to refresh.";
...
function protocolErrorResult(error, fallbackCode = "db_error") {
  if (isProtocolsTableMissingError(error)) {
    return {
      ...
      message: PROTOCOLS_TABLE_MISSING_HINT,
      tableMissing: true,
    };
  }
  return {
...
async function listProtocolsByClinic(clinicId, opts = {}) {
  ...
  if (error) {
    if (isProtocolsTableMissingError(error)) {
      console.warn("[clinicTreatmentProtocols] list: table missing");
      const err = new Error(PROTOCOLS_TABLE_MISSING_HINT);
