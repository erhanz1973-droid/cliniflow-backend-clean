...
const BOOKING_WHATSAPP_CONFIRM_PROMPT = {
  tr: "Hasta zaten WhatsApp üzerinden yazıyor — numarasını TEKRAR İSTEME. Gerekirse tek soru: «İletişim için bu WhatsApp numaranızı kullanmaya devam edelim mi? Farklı bir numara isterseniz yazabilirsiniz.» Gönderen numara zaten sistemde.",
  en: "The patient is already on WhatsApp — do NOT ask for their WhatsApp number again. If needed, ask once: «Should we continue using this WhatsApp number for communication?» The sender number is already on file.",
  ru: "Пациент уже пишет в WhatsApp — не просите номер снова. При необходимости: «Продолжим использовать этот номер WhatsApp для связи?»",
  ka: "პაციენტი უკვე WhatsApp-ზე წერს — არ მოითხოვოთ ნომერი თავიდან. საჭიროების შემთხვევაში: «გავაგრძელოთ ამ WhatsApp ნომრის გამოყENება?»",
};
 ...
 */
function buildContactRequiredPrompt(lang = "en", opts = {}) {
  const key = String(lang || "en").slice(0, 2).toLowerCase();
  if (opts?.whatsappChannel === true) {
    return BOOKING_WHATSAPP_CONFIRM_PROMPT[key] || BOOKING_WHATSAPP_CONFIRM_PROMPT.en;
  }
...
/**
 * @param {string} [lang]
 * @param {string|null} [knownPhone]
 */
function buildContactConfirmationDirectReply(lang = "tr", knownPhone = null) {
  const key = String(lang || "tr").slice(0, 2).toLowerCase();
  const display = knownPhone ? String(knownPhone) : "";
  if (key === "tr") {
    return display
  ...
  if (key === "ka") {
    return display
      ? `გავაგრძელოთ ამ WhatsApp ნომრის (${display}) გამოყENება?`
      : "გავაგრძელოთ ამ WhatsApp ნომრის გამოყENება?";
  }
  return display
    ? `Should we continue using this WhatsApp number (${display}) for communication? Share a different number if you prefer.`
    : "Should we continue using this WhatsApp number for communication? Share a different number if you prefer.";
}
 ...
 * @param {{ whatsappChannel?: boolean, knownPhone?: string|null }} [opts]
 */
function buildContactHintForSlotOffer(lang = "tr", opts = {}) {
  const key = String(lang || "tr").slice(0, 2).toLowerCase();
  if (opts?.whatsappChannel === true) {
    if (opts?.knownPhone) {
      return key === "tr"
        ? `\n\nİletişim için bu WhatsApp numaranızı (${opts.knownPhone}) kullanmaya devam edelim mi?`
        : key === "ru"
          ? `\n\nПродолжим использовать этот номер WhatsApp (${opts.knownPhone})?`
          : `\n\nShould we continue using this WhatsApp number (${opts.knownPhone})?`;
    }
    return key === "tr"
      ? "\n\nİletişim için bu WhatsApp numaranızı kullanmaya devam edelim mi?"
      : key === "ru"
        ? "\n\nПродолжим использовать этот номер WhatsApp?"
        : "\n\nShould we continue using this WhatsApp number for communication?";
  }
  if (key === "tr") {
  ...
  if (key === "ka") {
    return "\n\nჩაწერის დასასრულებლად გთხოვთ, გაგვიზიაროთ ტელეფონის ან WhatsApp-ის ნომერი.";
  }
...
function buildSlotOfferPromptBlock(params) {
  ...
  if (!params.hasContact) {
    lines.push(
      buildContactRequiredPrompt(params.lang, {
        whatsappChannel: params.whatsappChannel === true,
        knownPhone: params.knownPhone || null,
      }),
    );
  }
...
function buildSlotOfferDirectReply(slots, lang = "tr", opts = {}) {
  ...
  if (!list.length) {
    ...
    if (key === "ka") {
      return "ამ მომენტში კალენდარში თავისუფალი დრო არ ჩანს. მოგვწერეთ სხვა დღე ან საათი — შევამოწმებთ, ან გუნდი დაგიკავშირდებათ ვარიანტებით.";
    }
  ...
  const intro =
    ...
    (key === "tr"
      ...
      : key === "ru"
        ...
        : key === "ka"
          ? "თქვენთვის შესაფერისი თავისუფალი დროები:"
  ...
  const pickHint =
    key === "tr"
      ...
      : key === "ru"
        ...
        : key === "ka"
          ? "აირჩიეთ ვარიანტის ნომერი სიიდან (მაგ. «1» = პირველი ხაზი, არა 01:00). შეგიძლიათ დაწეროთ დრო «17:00»-ზე. ჩაწერა მხოლოდ თქვენი დადასტურების შემდეგ შეიქმნება."
  ...
  const contactHint = opts.needContact
    ? buildContactHintForSlotOffer(key, {
        whatsappChannel: opts.whatsappChannel === true,
  ...
  const nameHint = opts.needName
    ? key === "tr"
      ...
      : key === "ru"
        ...
        : key === "ka"
          ? "\n\nჩაწერისთვის გთხოვთ, მიუთითოთ თქვენი სახელი."
