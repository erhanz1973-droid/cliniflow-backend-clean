...
/**
 * «Mayıs ayı 3. hafta», «mayisin 3. haftasi» — month + week-of-month hint (not a single day).
 * @param {string} message
 * @param {string} [timezone]
 * @param {Date} [refDate]
 */
function parseMonthWeekHintFromMessage(message, timezone = "Europe/Istanbul", refDate = new Date()) {
  const t = normalizeTurkishMonthTokens(normalizeDateText(message));
  if (!t) return null;
  const weekMatch =
    t.match(/\b(\d{1,2})\s*\.\s*hafta\b/) ||
    t.match(/\b(\d{1,2})\s*hafta\b/) ||
    t.match(/\bhafta\s*(\d{1,2})\b/);
  if (!weekMatch) return null;
  const week = Number(weekMatch[1] || weekMatch[2] || weekMatch[3]);
  if (!Number.isFinite(week) || week < 1 || week > 5) return null;

  let monthNum = null;
  for (const [name, num] of Object.entries(TR_MONTHS)) {
  ...
  const zonedRef = toZonedTime(refDate instanceof Date ? refDate : new Date(refDate), tz);
  const year = Number(formatInTimeZone(zonedRef, tz, "yyyy"));
  return { month: monthNum, week, year, timezone: tz };
}
 ...
 * @param {Date} [refDate]
 */
function isDateYmdBeforeClinicToday(ymd, timezone = "Europe/Istanbul", refDate = new Date()) {
  const d = String(ymd || "").trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) return false;
  const tz = timezone || "Europe/Istanbul";
  const todayYmd = formatInTimeZone(
    toZonedTime(refDate instanceof Date ? refDate : new Date(refDate), tz),
    tz,
    "yyyy-MM-dd",
  );
  return d < todayYmd;
...
/**
 * End-of-week YMD for calendar week-of-month (Mon–Sun buckets, approximate).
 * @param {{ month?: number|null, week: number, year: number }} hint
 */
function monthWeekHintEndYmd(hint) {
  if (!hint?.week || !hint?.year) return null;
  const month = hint.month;
  if (!month || month < 1 || month > 12) return null;
  const weekEndDay = Math.min(hint.week * 7, new Date(hint.year, month, 0).getDate());
  return `${hint.year}-${String(month).padStart(2, "0")}-${String(weekEndDay).padStart(2, "0")}`;
}
 ...
 * @param {Date} [refDate]
 */
function assessSchedulingRequestTiming(message, timezone = "Europe/Istanbul", refDate = new Date()) {
  const tz = timezone || "Europe/Istanbul";
  const zonedRef = toZonedTime(refDate instanceof Date ? refDate : new Date(refDate), tz);
  const todayYmd = formatInTimeZone(zonedRef, tz, "yyyy-MM-dd");

  const preferredYmd = parsePreferredDateFromMessage(message, tz, refDate);
  if (preferredYmd && isDateYmdBeforeClinicToday(preferredYmd, tz, refDate)) {
    return {
      inPast: true,
      reason: "date_in_past",
      requestedYmd: preferredYmd,
      todayYmd,
      monthWeek: null,
    };
  }

  const monthWeek = parseMonthWeekHintFromMessage(message, tz, refDate);
  if (monthWeek) {
    const endYmd = monthWeekHintEndYmd(monthWeek);
    if (endYmd && isDateYmdBeforeClinicToday(endYmd, tz, refDate)) {
      return {
        inPast: true,
        reason: "month_week_in_past",
        requestedYmd: endYmd,
        todayYmd,
        monthWeek,
      };
    }
    if (monthWeek.month == null) {
      return {
        inPast: false,
        reason: "month_week_vague",
        requestedYmd: null,
        todayYmd,
        monthWeek,
      };
  ...
  return {
    inPast: false,
    reason: preferredYmd ? "future_date" : monthWeek ? "month_week_future" : "none",
    requestedYmd: preferredYmd,
    todayYmd,
    monthWeek,
  };
...
module.exports = {
  ...
  parseMonthWeekHintFromMessage,
  isDateYmdBeforeClinicToday,
