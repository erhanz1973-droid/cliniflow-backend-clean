...
function mapHotelRow(row) {
  return {
    ...
    pricePerNight:
      row.price_per_night != null && row.price_per_night !== ""
        ? Number(row.price_per_night)
        : null,
    vipTransfer: row.vip_transfer === true,
    distanceMinutes:
...
function hotelBodyToRow(body, clinicId) {
  ...
  return {
    ...
    price_per_night:
      body.pricePerNight != null && body.pricePerNight !== "" && Number.isFinite(Number(body.pricePerNight))
        ? Number(body.pricePerNight)
        : null,
    vip_transfer: body.vipTransfer === true,
    distance_minutes:
