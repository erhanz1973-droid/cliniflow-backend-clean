...
function isGreetingOnlyMessage(text) {
  ...
  if (GREETING_ONLY_RES.some((re) => re.test(t))) return true;
  return /^(how are you|how are u|hows it going|how's it going|whats up|what's up)(?:[\s!.?,…—-]|$)*/i.test(
    t,
  );
}
...
function detectGreetingLanguage(text) {
  ...
  if (
    ...
    /\b(merhaba|selam|selamlar|gunaydin|iyi\s+aksam|iyi\s+gece|nasilsin|nasılsın)\b/.test(t)
  ) {
    return "tr";
  }
  if (
    /\b(hello|hi|hey|good\s+morning|good\s+evening|good\s+night|how are you|how are u|how's it going|whats up|what's up)\b/.test(
      t,
    )
  ) {
    return "en";
  }
  if (/^[a-z0-9\s.,!?'"-]+$/i.test(t) && t.length >= 5 && t.length <= 64) {
    return "en";
  }
  return "tr";
}
...
function buildGreetingDirectReply(patientMessage, preferredLang, options = {}) {
  ...
  const lang =
    fromMessage === "en" || fromMessage === "ka" || fromMessage === "tr"
      ? fromMessage
      : pref === "ka" || pref === "en" || pref === "tr"
        ? pref
        : fromMessage;
  const greet = echoPatientGreeting(patientMessage, lang);
  const bookingContinuity = options?.bookingContinuity === true;

  if (/\bhow are you\b|\bhow are u\b/.test(t)) {
    if (lang === "ka") {
      return "გამარჯობა! კარგად ვარ, გმადლობთ. რით შემიძლია დაგეხმაროთ?";
    }
    if (lang === "en") {
      return "Hello! I'm here and happy to help — how can I assist you with your dental questions today?";
    }
    return "Merhaba! İyiyim, teşekkürler. Size nasıl yardımcı olabilirim?";
  }
