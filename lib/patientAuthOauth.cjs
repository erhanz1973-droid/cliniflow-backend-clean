...
const { getPatientByEmail, updatePatient } = require("./supabase.js");
...
async function resolvePatientOAuthSession({ supabase, accessToken, declaredProvider, clinicCode }) {
  ...
  let patient =
    ...
    (emailNorm ? await getPatientByEmail(emailNorm) : null) ||
  ...
  const patch = {
    ...
    auth_provider: prov,
    provider_subject: picked.subject || patient.provider_subject || null,
    avatar_url: picked.avatarUrl || patient.avatar_url || null,
  };
  if (emailNorm && !String(patient.email || "").trim()) {
    patch.email = emailNorm;
  }
  const displayName = String(patient.name || "").trim() || picked.fullName;
