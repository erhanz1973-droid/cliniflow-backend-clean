...
const { CANONICAL_THREAD_READ_OPTS } = require("./canonicalChatThread");
...
async function insertMirrorRow(row) {
  const { guardPatientFacingMessagePayload, auditMessageInsert } = require("./patientMessageThreadGuard");
  const guarded = await guardPatientFacingMessagePayload(row, {
    caller: "backfillLeadCoordinatorHistoryToPatientMessages",
    insertFn: "insertMirrorRow",
    source: "backfill_mirror",
    threadIdHint: row.thread_id || null,
    clinicIdHint: row.clinic_id || null,
  });
  if (!guarded.ok) return { ok: false, reason: guarded.error?.message || "guard_failed" };
  const payloads = [guarded.payload, { ...guarded.payload, clinic_id: undefined }];
  for (const payload of payloads) {
    const pr = await insertIntoTableWithColumnPruning("patient_messages", payload);
    if (!pr.error && pr.data) {
      auditMessageInsert({
        patientId: payload.patient_id,
        clinicId: guarded.clinicId || payload.clinic_id || null,
        threadId: guarded.persistedThreadId,
        resolvedThreadId: guarded.resolvedThreadId,
        persistedThreadId: guarded.persistedThreadId,
        messageId: payload.message_id || pr.data?.id || null,
        via: "backfill_mirror",
        insertFn: "insertMirrorRow",
        caller: "backfillLeadCoordinatorHistoryToPatientMessages",
      });
