...
const { attachHumanLeadSummary } = require("./leadSummaryHuman");
...
/**
 * Patient-reported tag match after humanization strips raw slugs from API payloads.
 * @param {Record<string, unknown>} lead
 * @param {string} tagSlug
 */
function leadMatchesPatientTag(lead, tagSlug) {
  const flags = lead.operationalIntakeFlags || {};
  const rawTags = flags.patientReportedTags || flags.patient_reported_tags || [];
  if (Array.isArray(rawTags) && rawTags.includes(tagSlug)) return true;

  const summaries = [
    ...(lead.patientReportedTagSummaries || []),
    ...(flags.patientReportedTagSummaries || []),
  ].map((s) => String(s || "").toLowerCase());

  const slugPatterns = {
    implant_interest: /implant|იმპლანტ/i,
    cosmetic_goal: /cosmetic|appearance|estetik|ესთეტიკ/i,
    veneer_interest: /veneer|lamina|ვინირ/i,
    whitening_interest: /whiten|bleach|beyazlat|გათეთრ/i,
    orthodontic_interest: /orthodont|braces|aligner|ortodont/i,
  };
  const re = slugPatterns[tagSlug] || new RegExp(tagSlug.replace(/_/g, "[\\s-]"), "i");
  if (summaries.some((line) => re.test(line))) return true;

  const sections = lead.leadSummarySections;
  if (Array.isArray(sections)) {
    const conversation = sections.find((s) => s && s.id === "conversation");
    if (conversation && Array.isArray(conversation.bullets)) {
      return conversation.bullets.some((b) => re.test(String(b)));
    }
  }

  const treatmentLine = String(lead.treatmentInterestSummary || "").toLowerCase();
  if (tagSlug === "implant_interest" && /implant|იმპლანტ/.test(treatmentLine)) return true;
...
function matchesIntakeQueue(lead, query) {
  ...
  switch (queue) {
    ...
    case "implant_interest":
      return tags.includes("implant_interest") || leadMatchesPatientTag(lead, "implant_interest");
    case "tag_cosmetic":
    case "cosmetic_goal":
    case "cosmetic_goals":
      return (
        ...
        leadMatchesPatientTag(lead, "cosmetic_goal") ||
        leadMatchesPatientTag(lead, "veneer_interest") ||
        leadMatchesPatientTag(lead, "whitening_interest") ||
        leadMatchesPatientTag(lead, "orthodontic_interest")
      );
    default:
      return true;
  }
...
function deriveBlockingReasonMeta(flags) {
  ...
  if (f.journeyStage) return { key: "journey_stage", stageKey: String(f.journeyStage) };
...
function enrichLeadForCoordinatorUI(lead, lang = "en") {
  ...
  const enriched = {
    ...
    linkedByPhoneNumber:
      ...
      !!(
        flags.linkedByPhoneNumber ||
        flags.linkedVia === "messenger_phone_link" ||
        flags.linkedVia === "messenger_whatsapp_match"
      ),
    linkedByPhoneAt: flags.linkedByPhoneAt || flags.linkedAt || null,
  ...
  return attachHumanLeadSummary(localizeLeadLabels(enriched, L), L);
}
