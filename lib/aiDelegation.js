...
const { opsT } = require("./i18n/localizeCoordination");
...
/**
 * Routine dental chief complaint (not ER) — AI should still reply and offer appointment.
 * @param {string} text
 */
function isRoutineDentalChiefComplaint(text) {
  const t = String(text || "").toLowerCase();
  if (!t || detectMedicalEmergency(text)) return false;
  if (/\b(şiddetli|dayanamıyorum|dayanamiyorum|unbearable|excruciating|acil|ambulans|911)\b/i.test(t)) {
    return false;
  }
  return (
    /\b(diş|dis)\s*(ağr|agr)/i.test(t) ||
    /\b(ağrım|agrim)\s+var\b/i.test(t) ||
    /\b(dişim|disim)\s+ağr/i.test(t) ||
    /\b(toothache|tooth\s+pain|dental\s+pain)\b/i.test(t)
  );
}
...
/**
 * Temporary doctor snooze — AI auto-replies blocked until timestamp (no permanent ai_paused).
 * @param {Record<string, unknown>|null|undefined} row
 */
function isAiSnoozeActive(row) {
  const until = getAiSnoozeUntilMs(row);
  return until != null && Date.now() < until;
}

/**
 * Temporary doctor snooze ended (not permanent Devral).
 * @param {Record<string, unknown>|null|undefined} row
 */
function hadSnoozeThatExpired(row) {
  const flags =
    row?.operational_intake_flags && typeof row.operational_intake_flags === "object"
      ? row.operational_intake_flags
      : {};
  const started = flags.ai_snooze_started_at || flags.aiSnoozeStartedAt;
  if (!started) return false;
  const untilMs = getAiSnoozeUntilMs(row);
...
function doctorOwnsConversation(row) {
  if (hadSnoozeThatExpired(row)) return false;
  const aiSnoozeActive = isAiSnoozeActive(row);
  const aiPaused = row.ai_paused === true || aiSnoozeActive;
...
function resolveInquiryDelegation(profileRow, ctx) {
  ...
  const snoozeExpiredResume = hadSnoozeThatExpired(row);
  const aiSnoozeActive = isAiSnoozeActive(row);
  let aiPaused = row.ai_paused === true || aiSnoozeActive;
  if (snoozeExpiredResume) {
    aiPaused = false;
    if (aiMode === AI_MODE.HUMAN_ONLY) {
      aiMode = capModeToClinicCeiling(AI_MODE.AI_ACTIVE, clinicCeiling);
    }
  }
  ...
  let aiEscalationRequired = row.ai_escalation_required === true;
  if (medicalEmergency || signals.emergency) {
    aiEscalationRequired = true;
  } else if (msg) {
    if (signals.severePain && !routineDental) {
      aiEscalationRequired = true;
    } else if (routineDental) {
      aiEscalationRequired = false;
    } else if (!medicalEmergency && !signals.emergency) {
      aiEscalationRequired =
        row.ai_escalation_required === true && flags.emergency === true;
    }
  } else if (flags.emergency === true) {
    aiEscalationRequired = true;
  }

  if (medicalEmergency || (aiEscalationRequired && !routineDental && signals.emergency)) {
    aiMode = AI_MODE.ESCALATION_REQUIRED;
    aiEscalationRequired = true;
  } else if (aiEscalationRequired && aiMode !== AI_MODE.HUMAN_ONLY && !routineDental) {
    aiMode = AI_MODE.ESCALATION_REQUIRED;
  } else if (routineDental && aiMode === AI_MODE.ESCALATION_REQUIRED) {
  ...
  const lang = normalizeUiLang(ctx?.lang || "en");
  const statusLabel = doctorOwns
    ? opsT(lang, "delegationStatus.doctorHandling")
    : autoReplyAllowed
      ? opsT(lang, "delegationStatus.aiHandling")
      : statusLabelForMode(aiMode, aiPaused, aiEscalationRequired, lang);

  const snoozeUntilMs = getAiSnoozeUntilMs(row);
  ...
  return {
    ...
    aiPaused,
    aiSnoozeActive,
    aiSnoozedUntil: snoozeUntilMs != null ? new Date(snoozeUntilMs).toISOString() : null,
    aiEscalationRequired,
 ...
 * @param {boolean} aiEscalationRequired
 */
function statusLabelForMode(aiMode, aiPaused, aiEscalationRequired, lang = "en") {
  const L = normalizeUiLang(lang);
  if (aiEscalationRequired || aiMode === AI_MODE.ESCALATION_REQUIRED) {
    return opsT(L, "delegationStatus.humanReview");
  }
  if (aiPaused || aiMode === AI_MODE.HUMAN_ONLY) return opsT(L, "delegationStatus.humanOnly");
  if (aiMode === AI_MODE.AI_DRAFT) return opsT(L, "delegationStatus.aiDraft");
  if (aiMode === AI_MODE.AI_ACTIVE) return opsT(L, "delegationStatus.aiCoordinating");
  if (aiMode === AI_MODE.AI_ASSISTED) return opsT(L, "delegationStatus.aiAssisting");
  return opsT(L, "delegationStatus.humanOnly");
}
...
function buildEscalationPatch(flags, messageText) {
  ...
  if (!force || isRoutineDentalChiefComplaint(messageText)) return null;
