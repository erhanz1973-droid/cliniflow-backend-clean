...
function resolveInquiryDelegation(profileRow, ctx) {
  ...
  const primaryResponder = String(row.primary_responder_type || "").toLowerCase();
  /** Doctor took over: no auto-reply, but clinical-guidance expand → patient draft still allowed. */
  const assignedDoctorId = String(row.assigned_doctor_id || "").trim();
  const doctorLedDraftAssist =
    aiPaused &&
    !aiEscalationRequired &&
    aiMode === AI_MODE.HUMAN_ONLY &&
    (primaryResponder === "doctor" || UUID_RE.test(assignedDoctorId));
